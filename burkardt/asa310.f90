...
!  Parameters:
!
!    Input, real ( kind = rk ) X, P, the parameters.
!    0 <= X, and 0 < P.
!
!    Output, integer IFAULT, error flag.
!    0, no error.
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, B, the shape parameters.
!    0 <= A, 0 <= B.
!
!    Input, real ( kind = rk ) LAMBDA, the noncentrality parameter.
!    0 <= LAMBDA.
!
!    Input, real ( kind = rk ) X, the value at which the CDF is desired.
!
!    Input, real ( kind = rk ) ERRMAX, the precision tolerance.
!
!    Output, integer IFAULT, error flag.
!    0, no error occurred.
!    1, X is 0 or 1.
...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ), external :: alngam
  real ( kind = rk ) b
  real ( kind = rk ) beta
  real ( kind = rk ), external :: betain
  real ( kind = rk ), external :: betanc
  real ( kind = rk ) c
  real ( kind = rk ) ebd
  real ( kind = rk ) errbd
  real ( kind = rk ) errmax
...
!
    errbd = ( 1.0D+00 - gammad ( c, real ( iter1, kind = rk ), ifault ) ) &
    * ( temp + s )
