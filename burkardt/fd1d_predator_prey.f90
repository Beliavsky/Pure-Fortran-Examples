...
!
!    User input, real ( kind = rk ) A, B, the left and right endpoints
!    of the interval.
!
!    User input, real ( kind = rk ) H, the "space step", the desired
!    spacing between
!    nodes in [A,B].
!
!    User input, real ( kind = rk ) T, the final time.  The problem is to
...
!
  ...
  real ( kind = rk ) alpha
  real ( kind = rk ) b
  real ( kind = rk ), allocatable, dimension ( :, : ) :: b1
  real ( kind = rk ), allocatable, dimension ( :, : ) :: b2
  real ( kind = rk ) beta
  integer bigj
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: u_rhs
  real ( kind = rk ), allocatable, dimension ( : ) :: v
  real ( kind = rk ), allocatable, dimension ( : ) :: v_rhs
...
!
!    Input, real ( kind = rk ) A(3,N), the D3 matrix.
!
...
!    N must be at least 2.
!
!    Input, real ( kind = rk ) A_LU(3,N), the LU factors from D3_NP_FA.
!
