...
!*****************************************************************************80
!
!! cvt_basis_flow() uses CVT clustering on a set of fluid flow solution vectors.
!
!  Discussion:
!
...
!
!    20 June 2002: Lee requested real ( kind = rk ) calculations.
...
!
!    Input, integer ENERGY_IT_MAX, the maximum number of energy
!    iterations.
!
!    Output, real ( kind = rk ) ENERGY(CLUSTER_HI), contains in entries
!    CLUSTER_LO through CLUSTER_HI the estimated minimum energy over
!    all clusterings of this size.
!
!    Output, real ( kind = rk ) CLUSTER_CENTER(DIM_NUM,CLUSTER_HI), contains the
...
!
!    Input, integer NODE(NPE,ELEMENT_NUM), the nodes that make 
!    up each element.
!
...
!
!    Input, real ( kind = rk ) X(N), Y(N), the vectors whose distance is
!    to be computed.
!
!    Output, real ( kind = rk ) DISTANCE_NORMAL_SQ, the square of the
...
!
  ...
  real ( kind = rk ) cosine
  real ( kind = rk ) distance_normal_sq
  real ( kind = rk ) x(n)
  real ( kind = rk ) x_norm
  real ( kind = rk ) y(n)
  real ( kind = rk ) y_norm
...
!
  ...
  integer output_unit
  real ( kind = rk ), parameter :: x = 1.0D+00
...
!    Input, character ( len = * ) STRING, the prompt string.
!
!    Output, integer VALUE1, VALUE2, the values entered
!    by the user.
!
!    Output, integer IERROR, an error flag, which is zero 
!    if no error occurred.
...
!    Input, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the data points.
!
!    Input/output, integer CLUSTER(POINT_NUM), the cluster to 
...
!
!    Input, real ( kind = rk ) XY(2,NODE_NUM), the nodes.
!
!    Output, real ( kind = rk ) A(BANDWIDTH+1,NODE_NUM), the mass matrix.
!
  implicit none
  ...
  real ( kind = rk ) a(bandwidth+1,node_num)
  real ( kind = rk ) area
  integer element
  real ( kind = rk ) eta
  integer ip
  ...
  real ( kind = rk ) refqbf
  integer rule
  real ( kind = rk ) weight(nquad)
  real ( kind = rk ) wi
  real ( kind = rk ) wj
  real ( kind = rk ) xy(2,node_num)
  real ( kind = rk ) xsi
  real ( kind = rk ) xtab(nquad)
  real ( kind = rk ) ytab(nquad)
!
!  Zero out the matrix.
!
...
!
!    Input, real ( kind = rk ) POINT(DIM_NUM), the data point to be checked.
!
!    Output, integer NEAREST, the index of the center point 
...
!
  ...
  integer j
  integer nearest
  real ( kind = rk ), dimension ( dim_num ) :: point
...
!    Input, integer CLUSTER_NUM, the number of center points.
!
!    Input, real ( kind = rk ) CENTER(DIM_NUM,CLUSTER_NUM), the
!    center points.
!
!    Input, real ( kind = rk ) POINT(DIM_NUM), the data point to be checked.
!
...
!
  ...
  real ( kind = rk ), dimension ( dim_num, cluster_num ) :: center
  real ( kind = rk ) dist
  real ( kind = rk ) dist_new
  integer j
  integer nearest
...
!    Input, integer DIM_NUM, the number of spatial dimensions.
!
!    Input, real ( kind = rk ) R_MIN(DIM_NUM), R_MAX(DIM_NUM), the coordinates
!    of the minimum and maximum corners of the region.
!
!    Input, integer POINT_NUM, the number of points to generate.
!
!    Output, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the coordinates
!    of the points.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A(N), the vector whose 2-norm is desired.
!
!    Output, real ( kind = rk ) R8VEC_NORM2, the 2-norm of A.
!
  implicit none
...
!
!    Output, real ( kind = rk ) VALUE1(DIM_NUM), VALUE2(DIM_NUM), the values
!    entered by the user.
!
!    Output, integer IERROR, an error flag, which is zero if no
!    error occurred.
!
  implicit none
...
!
!    Input, real ( kind = rk ) XSI, the XSI coordinate of the point.
!
!    Input, real ( kind = rk ) ETA, the ETA coordinate of the point.
!
!    Output, real ( kind = rk ) REFQBF, the value of the basis function
!    PSI(IQ)(XSI,ETA).
!
  implicit none
...
!
!    Output, integer IERROR, an error flag, which is zero if 
!    no error occurred.
!
  implicit none
...
!  Parameters:
!
!    Input, integer I, an I4 to be converted.
!
...
!    part of the real ( kind = rk ) number.
!
!    Legal input is:
!
!       1 blanks,
...
!
!    Output, real ( kind = rk ) XTAB(NORDER), YTAB(NORDER), the abscissas.
!
