...
!
!    Input, real ( kind = rk ) UA, UB, the values prescribed for U at 
!    the endpoints.
!
...
!
!    Output, real ( kind = rk ) X(N), the grid points.
!
!    Output, real ( kind = rk ) U(N), the approximation to the solution at 
!    the grid points.
!
  implicit none
  ...
  integer i
  real ( kind = rk ), external :: k
  real ( kind = rk ) rhs(n)
  real ( kind = rk ) tri(3,n)
  real ( kind = rk ) u(n)
  real ( kind = rk ) ua
  real ( kind = rk ) ub
  real ( kind = rk ) x(n)
