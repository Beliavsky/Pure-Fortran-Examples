...
!  Input:
!
!    integer ORDER, the order of the Clenshaw Curtis rule.
!    1 <= ORDER.
!
!    integer I, the index of the desired abscissa.  
!    1 <= I <= ORDER.
!
!  Output:
!
!    real ( kind = rk ) CC_ABSCISSA, the value of the I-th 
...
!
  ...
  if ( i < 1 ) then
    ...
    cc_abscissa = cos ( real ( order - i, kind = rk ) * pi &
...
!  Input:
!
!    integer LEVEL_MAX, determines the number of points 
!    in the grid as 2^LEVEL_MAX + 1.
!
!    integer TEST_NUM, the number of points to be tested.
!
!    integer TEST_VAL(TEST_NUM), the indices of the points
!    to be tested.  Normally, each index would be between 0 and 2^LEVEL_MAX.
!
!  Output:
!
!    integer TEST_LEVEL(TEST_NUM), the level at which the
...
!  Input:
!
!    integer LEVEL_MAX, controls the size of the final 
!    sparse grid.
!
...
!    integer TEST_NUM, the number of points to be tested.
!
!    integer TEST_VAL(DIM_NUM,TEST_NUM), the indices of 
!    the points to be tested.  Normally, each index would be between 0 
!    and 2^LEVEL_MAX.
!
!  Output:
!
!    integer TEST_LEVEL(TEST_NUM), the value of LEVEL
...
!
!    integer ORDER_1D(DIM_NUM), the order of the 
!    Clenshaw-Curtis rule in each dimension.
!
!    integer ORDER_ND, the product of the entries of
!    ORDER_1D.
!
!  Output:
!
!    real ( kind = rk ) POINT(DIM_NUM,ORDER_ND), the points in
...
!
  ...
  integer p
  real ( kind = rk ) point(dim_num,order_nd)
...
!  Output:
!
!    integer INDX(DIM_NUM,ORDER_ND), the indices of the
...
!    integer DIM_NUM, the spatial dimension.
!
!    real ( kind = rk ) Q_MAX, the maximum values of
!    Q, the sum of the weighted orders in each spatial coordinate.
!
!    real ( kind = rk ) ALPHA(DIM_NUM), the weight factors for
!    the orders in each spatial dimension.
!
!    integer ORDER_MIN(DIM_NUM), ORDER_MAX(DIM_NUM), the 
!    minimum and maximum values of the order of the 1D Clenshaw Curtis rule
!    in each spatial dimension.
!
!    integer GRID_NUM, the number of Clenshaw Curtis
...
!  Output:
!
!    integer GRID_ORDER(DIM_NUM,GRID_NUM), contains, for 
!    each grid, the order of the Clenshaw-Curtis rule in each dimension.
!
!    real ( kind = rk ) GRID_POINT(DIM_NUM,POINT_NUM), contains
...
!
  ...
  real ( kind = rk ), dimension ( dim_num ) :: alpha
  integer, dimension ( dim_num, grid_num ) :: grid_order
  real ( kind = rk ),dimension ( dim_num, point_num ) :: grid_point
  logical more
...
!    the orders in each spatial dimension.
!
!    integer ORDER_MIN(DIM_NUM), ORDER_MAX(DIM_NUM), 
...
!    integer DIM_NUM, the spatial dimension.
!
!    integer Q_MIN, Q_MAX, the minimum and maximum values 
...
!  Output:
!
!    integer GRID_ORDER(DIM_NUM,GRID_NUM), contains, 
...
!
  ...
  integer grid_num
  integer, dimension ( dim_num, point_num ) :: grid_order
  real ( kind = rk ), dimension ( dim_num, point_num ) :: grid_point
  logical more
  integer, dimension ( dim_num ) :: order_1d
  integer order_nd
...
!    integer DIM_NUM, the spatial dimension.
!
!    integer LEVEL(DIM_NUM), the nesting levels of the 
!    1D rules.
!
!  Output:
!
!    integer ORDER(DIM_NUM), the order (number of points)
...
!    the orders in each spatial dimension.
!
!    integer LEVEL_MIN(DIM_NUM), LEVEL_MAX(DIM_NUM), the
...
!  Output:
!
!    integer GRID_LEVEL(DIM_NUM,GRID_NUM), contains, 
...
!
  ...
  real ( kind = rk ), dimension ( dim_num ) :: alpha
  integer, dimension ( dim_num, grid_num ) :: grid_level
  real ( kind = rk ),dimension ( dim_num, point_num ) :: grid_point
  logical more
...
!    integer DIM_NUM, the spatial dimension.
!
!    integer LEVEL_MIN, LEVEL_MAX, the minimum and maximum
!    values of LEVEL.
!
!    integer GRID_NUM, the number of Clenshaw Curtis
...
!
  ...
  integer grid_num
  integer, dimension ( dim_num, point_num ) :: grid_level
  integer, dimension ( dim_num, point_num ) :: grid_order
  real ( kind = rk ), dimension ( dim_num, point_num ) :: grid_point
  integer h
  integer level
...
!    integer DIM_NUM, the spatial dimension.
!
!    integer LEVEL_MIN, LEVEL_MAX, the minimum and 
...
!
!    integer I, the index of the desired weight.  
!    1 <= I <= ORDER.
!
!  Output:
!
!    real ( kind = rk ) CC_WEIGHT, the I-th weight in the
...
!
  ...
  do j = 1, ( order - 1 ) / 2
    ...
    angle = real ( 2 * j * ( i - 1 ), kind = rk ) * pi &
          / real ( order - 1, kind = rk )

    value = value - b * cos ( angle ) / real ( 4 * j * j - 1, kind = rk )

  end do

  if ( i == 1 ) then
    value =           value / real ( order - 1, kind = rk )
  else if ( i <= order - 1 ) then
    value = 2.0D+00 * value / real ( order - 1, kind = rk )
  else if ( i == order ) then
    value =           value / real ( order - 1, kind = rk )
  end if
...
!  Output:
!
!    real ( kind = rk )  X(N), W(N), the abscissas and weights
!    of the rule.
!
  implicit none
...
!    the grid.
!
!    real ( kind = rk ) WEIGHT(ORDER_ND), the integration weights
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: w1d
  real ( kind = rk ) weight(*)
  real ( kind = rk ), allocatable, dimension ( : ) :: x1d
...
!  Output:
!
!    real ( kind = rk ) XTAB(ORDER), the abscissas of the rule.
!
...
!
!    logical MORE, set by the user to start the computation.
!
!    integer  H, T, internal parameters needed 
...
!  Output:
!
!    integer A(K), the parts of the updated composition.
!
!    logical MORE, set by the routine to terminate the computation.
!
!    integer  H, T, updated internal parameters needed 
!    for the computation.
!
  implicit none
...
subroutine r8table_write0 ( output_file_name, m, n, table )

!*****************************************************************************80
!
!! r8table_write0() writes a table file with no headers.
!
!  Licensing:
!
!    This code is distributed under the MIT license. 
...
!  Input:
!
!    character ( len = * ) OUTPUT_FILE_NAME, the output file name.
!
...
!
  ...
  if ( output_status /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'r8table_write0 - Fatal error!'
...
!
!    integer A(N).  On the first call, the input value of A doesn't matter.  
!    Thereafter, it should be the same as
...
!
!    real ( kind = rk ) ALPHA(N), the coefficient vector.
!
...
!
!    real ( kind = rk ) Q, the limit on the sum.
!
!    logical MORE.  On if the user has set MORE
...
!  Output:
!
!    integer X(N).  On (with MORE = TRUE), the value of X will be the "next"
