...
!    a set of points in a face centered cubic (FCC) FCC lattice, to 
!    compute the coordinates of those points, and write them to a file.
!
...
!  Author:
!
!    Original Fortran77 version by Furio Ercolessi, SISSA, Trieste, 
...
!  Input:
!
!    character ( len = * ) FILE_NAME, the name of the 
!    output file to be created.
!
!    real ALAT, the lattice spacing.
!
!    integer NX, NY, NZ, the number of cells along the
!    X, Y and Z directions.
!
!    real DISPLAC, the maximum random displacement.
!
  implicit none
  ...
  real ( kind = rk8 ) :: alat
  real ( kind = rk8 ), dimension(3) :: box_size
  real ( kind = rk8 ) :: displac
  ...
  integer ny
  integer nz
  real ( kind = rk8 ), dimension(3) :: rands
  real ( kind = rk8 ), dimension(3,4), parameter :: rcell = reshape ( (/ &
    ...
    do j = 0, ny - 1
      do i = 0, nx - 1
        do l = 1, nbase
          ...
          x = alat * ( real ( i, kind = rk8 ) + rcell(1,l) ) &
            + 2.0D+00 * displac * ( rands(1) - 0.5D+00 )
          y = alat * ( real ( j, kind = rk8 ) + rcell(2,l) ) &
            + 2.0D+00 * displac * ( rands(2) - 0.5D+00 )
          z = alat * ( real ( k, kind = rk8 ) + rcell(3,l) ) &
...
!
  ...
  real ( kind = rk8 ) :: alat
  real ( kind = rk8 ), parameter :: cutoff_al = 5.55805441821810D+00
  real ( kind = rk8 ), parameter :: cutoff_lj = 2.5D+00
  real ( kind = rk8 ) :: displac
  character ( len = * ) :: file_name
  integer ios
  integer nx
