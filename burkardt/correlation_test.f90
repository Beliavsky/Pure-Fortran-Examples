...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'correlation_test():'
  write ( *, '(a)' ) '  Fortran90 version.'
  write ( *, '(a)' ) '  Test correlation().'
...
!
  ...
  integer n2
  real ( kind = rk ), allocatable :: rho(:)
  real ( kind = rk ), parameter :: rho0 = 1.0D+00
  real ( kind = rk ), parameter :: rhomax = 10.0D+00
  real ( kind = rk ), parameter :: rhomin = 0.0D+00
...
!
  call sample_paths_eigen ( n, n2, rhomax, rho0, correlation_besselj, x )
  call paths_plot ( n, n2, rho, x, 'besselj', 'Bessel J correlation' )
!
!  besselk
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_besselk, x )
  call paths_plot ( n, n2, rho, x, 'besselk', 'Bessel K correlation' )
!
!  circular
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_circular, x )
  call paths_plot ( n, n2, rho, x, 'circular', 'Circular correlation' )
!
!  constant
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_constant, x )
  call paths_plot ( n, n2, rho, x, 'constant', 'Constant correlation' )
!
!  cubic
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_cubic, x )
  call paths_plot ( n, n2, rho, x, 'cubic', 'Cubic correlation' )
!
!  damped_cosine
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_damped_cosine, x )
  call paths_plot ( n, n2, rho, x, 'damped_cosine', 'Damped cosine correlation' )
!
!  damped_sine
!  Use EIGEN, because CHOLESKY fails.
!
  call sample_paths_eigen ( n, n2, rhomax, rho0, correlation_damped_sine, x )
  call paths_plot ( n, n2, rho, x, 'damped_sine', 'Damped sine correlation' )
!
!  exponential
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_exponential, x )
  call paths_plot ( n, n2, rho, x, 'exponential', 'Exponential correlation' )
!
!  gaussian
!  Use EIGEN, because CHOLESKY fails.
!
  call sample_paths_eigen ( n, n2, rhomax, rho0, correlation_gaussian, x )
  call paths_plot ( n, n2, rho, x, 'gaussian', 'Gaussian correlation' )
!
!  hole
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_hole, x )
  call paths_plot ( n, n2, rho, x, 'hole', 'Hole correlation' )
!
!  linear
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_linear, x )
  call paths_plot ( n, n2, rho, x, 'linear', 'Linear correlation' )
!
!  matern ( nu = 2.5 )
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_matern, x )
  call paths_plot ( n, n2, rho, x, 'matern', 'Matern correlation (nu=2.5)' )
!
!  pentaspherical
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_pentaspherical, x )
  call paths_plot ( n, n2, rho, x, 'pentaspherical', 'Pentaspherical correlation' )
!
!  power ( e = 2.0 )
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_power, x )
  call paths_plot ( n, n2, rho, x, 'power', 'Power correlation (e=2.0)' )
!
!  rational_quadratic
!  Use EIGEN, because CHOLESKY fails.
!
  call sample_paths_eigen ( n, n2, rhomax, rho0, correlation_rational_quadratic, x )
  call paths_plot ( n, n2, rho, x, 'rational_quadratic', 'Rational quadratic correlation' )
!
!  spherical
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_spherical, x )
  call paths_plot ( n, n2, rho, x, 'spherical', 'Spherical correlation' )
!
!  white_noise
!
  call sample_paths_cholesky ( n, n2, rhomax, rho0, correlation_white_noise, x )
...
!
  ...
  rho0 = 1.0D+00
  allocate ( x(1:n,1:n2) )
  call sample_paths2_cholesky ( n, n2, rhomin, rhomax, rho0, correlation_brownian, x )
  allocate ( rho(1:n) )
  call r8vec_linspace ( n, rhomin, rhomax, rho )
