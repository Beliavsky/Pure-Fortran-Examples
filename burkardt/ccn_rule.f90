...
!*****************************************************************************80
!
!! ccn_rule() computes a nested Clenshaw Curtis quadrature rule.
!
!  Discussion:
!
!    The user specifies:
...
!
!    Output, real ( kind = rk ) X(N), the elements of the sequence.
!
  implicit none
...
!
  ...
  do while ( m < n )
    ...
    do i = 1, k
      if ( mod ( i, 2 ) == 1 ) then
        x(m+i) = real ( tu, kind = rk ) / 2.0D+00 / real ( k, kind = rk )
        tu = tu + 2
      else
        x(m+i) = real ( td, kind = rk ) / 2.0D+00 / real ( k, kind = rk )
        td = td - 2
      end if
    end do
