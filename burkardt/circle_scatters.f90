...
!
  ...
  real ( kind = rk ) t
  real ( kind = rk ) x(n_circle)
  real ( kind = rk ), allocatable :: xy_in(:,:)
  real ( kind = rk ), allocatable :: xy_out(:,:)
  real ( kind = rk ) y(n_circle)
...
!  Read the data.
!
  call r8mat_header_read ( 'circle1_data.txt', m, n_in )

  allocate ( xy_in(1:m,1:n_in) )

  call r8mat_data_read ( 'circle1_data.txt', m, n_in, xy_in )

  call r8mat_header_read ( 'circle2_data.txt', m, n_out )

  allocate ( xy_out(1:m,1:n_out) )

  call r8mat_data_read ( 'circle2_data.txt', m, n_out, xy_out )
!
...
!
  call graf ( -0.1D+00, 1.1D+00, 0.0D+00, 0.1D+00, -0.1D+00, 1.1D+00, 0.0D+00, 0.1D+00 )
!
...
!
  do i = 1, n_in
    call rlcirc ( xy_in(1,i), xy_in(2,i), 0.01D+00 )
  end do
!
...
!
  do i = 1, n_out
    call rlcirc ( xy_out(1,i), xy_out(2,i), 0.01D+00 )
  end do
!
!  Draw a red circle.
!
  do i = 1, n_circle
    t = 0.5D+00 * pi * real ( i - 1, kind = rk ) / real ( n_circle - 1, kind = rk )
    x(i) = cos ( t )
    y(i) = sin ( t )
  end do
  call thkcrv ( 10 )
