...
!
  ...
  integer node_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: node_x
...
!
!    Input/output, integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM),
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) NODE_XY(M,NODE_NUM), the node coordinates.
!
!    Input, integer ELEMENT_ORDER, the order of the elements.
!
!    Input, integer ELEMENT_NUM, the number of elements.
