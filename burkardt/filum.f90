...
!
!    Output, integer CH_INDEX_LAST, the location of the last 
...
!  Parameters:
!
!    Input, integer IUNIT, the unit number associated 
!    with the open file.
!
...
!
!    Output, integer IUNIT, the FORTRAN unit number associated 
!    with the file.
!
!    Output, integer REC_NUM, the number of records (that is, 
...
!
!    Output, integer CHAR_NUM, the number of characters in 
!    the file.
!
  implicit none
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Input, integer COLUMN_NUM, the number of columns assumed 
!    to be in the file.
!
!    Output, real ( kind = rk ) COL_MIN(COLUMN_NUM), COL_MAX(COLUMN_NUM), 
...
!
  ...
  real ( kind = rk ) col_max(column_num)
  real ( kind = rk ) col_min(column_num)
  character ( len = * ) filename
  integer ierror
  integer ios
  integer iunit
...
!  Parameters
!
!    Input, integer FILE_UNIT, the unit number associated with 
!    the file.
!
!    Input/output, logical MORE.  On first call, the user should 
...
!  
!    Output, integer VALUE, the next integer in the file.  However,
...
!    between calls.
!
!    Input/output, integer LINE_NUM, the number of lines read from 
...
!  Parameters:
!
!    Input, integer INPUT_UNIT, the unit number associated 
!    with the input file.
!
!    Input, integer OUTPUT_UNIT, the unit number associated 
!    with the output file.
!
!    Output, integer LINE_NUM, the number of records copied 
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Input, integer LINE_INDEX, the index of the line to be read.
!
...
subroutine file_line_uniform ( filename, line, line_index, line_num )
...
!
  do
    ...
    if ( r * real ( line_num, kind = rk ) <= 1.0D+00 ) then
...
!
!    Output, integer FILE_LINE_WIDTH, the length of the 
!    longest line.
!
  implicit none
...
!    number generator.
!
!    Input, integer N, the number of lines to be extracted.
!
!    Output, character ( len = * ) LINE(N), N random lines from the file.
!
!    Output,  integer LINE_INDEX(N), the indices of the lines.
!
!    Output, integer LINE_NUM, the number of lines in the file.
!
  implicit none
...
!
  do
    ...
    if ( line_num <= n ) then
      ...
      if ( r * real ( line_num, kind = rk ) <= real ( n ) ) then
...
!    be created.
!
!    Output, integer N1, N2, N3, the number of lines of text in the
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, integer PARA_NUM, the number of paragraphs found in 
!    the file.
!
  implicit none
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, integer LINE_NUM, the number of lines found in the 
...
!    the sequence.
!
!    Output, integer FILE_DIM, the dimension of the data 
!    in one file.
!
...
!
!    Output, integer INPUT_LINE_NUM, the number of lines in the 
!    input file.
!
!    Output, integer OUTPUT_LINE_NUM, the number of lines in the 
!    output file.
!
  implicit none
  ...
  character ( len = * ) input_filename
  integer input_line_num
  integer input_unit
  integer ios
  character ( len = 255 ) line
  character ( len = 255 ) line_old
  character ( len = * ) output_filename
  integer output_line_num
  integer output_unit
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, integer WORD_NUM, the number of words in the file.
!
  implicit none
...
!
!    Output, integer I, J, the indices of the first and 
...
!  Input:
!
!    character ( len = * ) FILENAME: a filename
!
!  Output:
!
!    character ( len = * ) FILENAME: the incremented filename.
!
  implicit none
...
!
!    Output, integer LEN_NONNULL, the length of the string, up to 
