...
!  Input:
!
!    real ( kind = rk ) R, the coordinate where an interpolated
!    value is desired.  
!
!    real ( kind = rk ) X0, X1, the data values at the ends of the line.
!
!  Output:
!
!    real ( kind = rk ) X, the interpolated data value at (R).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, the coordinates where an 
!    interpolated value is desired.  
!
!    real ( kind = rk ) X00, X01, X10, X11, the data values 
!    at the corners.
!
!  Output:
!
!    real ( kind = rk ) X, the interpolated data value at (R,S).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, T, the coordinates where an 
!    interpolated value is desired.
!
!    real ( kind = rk ) X000, X001, X010, X011, X100, X101, X110, 
!    X111, the data values at the corners.
!
!  Output:
!
!    real ( kind = rk ) X, the interpolated data value at (R,S,T).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, the coordinates where an interpolated 
!    value is desired.  
!
!    real ( kind = rk ) X00, X01, X10, X11, the data values 
!    at the corners.
!
!    real ( kind = rk ) XR0, XR1, X0S, X1S, the data values at 
...
!  Input:
!
!    real ( kind = rk ) R, S, T, the coordinates where an interpolated
!    value is desired.
!
!    real ( kind = rk ) X000, X001, X010, X011, X100, X101, X110,
!    X111, the data values at the corners.
!
!    real ( kind = rk ) XR00, XR01, XR10, XR11, X0S0, X0S1, X1S0, 
...
!    X1S1, X00T, X01T, X10T, X11T, the data values at points along the edges.
!
!    real ( kind = rk ) X0ST, X1ST, XR0T, XR1T, Xrs0, Xrs1, the 
...
!
!    real ( kind = rk ) X(M).  
!    On X(1) and X(M) contain scalar values which are to be 
...
!
!    integer M, the number of entries in X.
!
!  Output:
!
!    real ( kind = rk ) X(M).  
!    X(2) through X(M-1) have been assigned interpolated values.
!
  implicit none
...
!    X(1,1), X(1,M2), X(M1,1) and X(M1,M2) contain scalar 
!    values which are to be interpolated throughout the table, using 
!    the table indices I and J as independent variables. 
!
!    integer M1, M2, the number of rows and columns in X.
!
!  Output:
!
!    real ( kind = rk ) X(M1,M2).  
!    all entries in X have been assigned a value.
!
  implicit none
...
!
!    real ( kind = rk ) X(M1,M2).  
!    data is contained in the "edge entries" X(1,J), X(I,1), 
...
!    X(M1,J) and X(I,M2), for I = 1 to M1, and J = 1 to M2.
!
!    real ( kind = rk ) R(M1), S(M2), are "coordinates" for the rows and
...
!    X(1,1,1), X(1,M2,1), X(M1,1,1), X(M1,M2,1), X(1,1,M3),
...
!
!    integer M1, M2, M3, the number of rows, columns, 
!    and layers in X.
!
!  Output:
!
...
!
!    real ( kind = rk ) X(M1,M2,M3).  
!    there is already scalar data in the entries X(I,J,K) 
...
!
!    integer M1, M2, M3, the number of rows, columns, and 
!    layers in X.
!
!  Output:
!
!    real ( kind = rk ) X(M1,M2,M3).  
!    all entries in X have been assigned a value, using the
...
!  Input:
!
!    real ( kind = rk ) R, the (R) coordinate of the point to 
!    be evaluated.
!
!    integer N, the dimension of the vector space.
!
!    External BOUND_r, a subroutine which is given (R) coordinates
...
!  Output:
!
!    real ( kind = rk ) X(N), the interpolated value at the point (R).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, the (R,S) coordinates of the point to be 
!    evaluated.
!
!    integer N, the dimension of the vector space.
!
...
!  Output:
!
!    real ( kind = rk ) X(N), the interpolated value at the point (R,S).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, T, the (R,S,T) coordinates of the 
...
!  Output:
!
!    real ( kind = rk ) X(N), the interpolated value at the 
!    point (R,S,T).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) R, S, T, the (R,S,T) coordinates of the point 
!    to be evaluated.
!
!    integer N, the dimension of the vector space.
!
