...
!
  ...
  real ( kind = rk ) y
  real ( kind = rk ) yray(n)
  real ( kind = rk ) zlev(12)
  real ( kind = rk ) zmat(m,n)
  ...
  do i = 1, m
    x = 1.6D+00 * real ( i - 1, kind = rk ) / real ( m - 1, kind = rk )
    xray(i) = x
    do j = 1, n
      y = 1.6D+00 * real ( j - 1, kind = rk ) / real ( n - 1, kind = rk )
      yray(j) = y
...
!
  ...
  do i = 1, 12
    zlev(13-i) = 0.1D+00 + real( i - 1, kind = rk ) * 0.1D+00
  end do
