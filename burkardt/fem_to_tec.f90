...
!
!    Input, integer NODE_DATA_NUM, the number of data items per
!    node.
!
...
!
!    Output, real ( kind = rk ) NODE_DATA(NODE_DATA_NUM,NODE_NUM), the data
...
!
  ...
  real ( kind = rk ), dimension (dim_num,node_num) :: node_coord
  character ( len = * ) node_coord_file_name
  real ( kind = rk ), dimension (node_data_num,node_num) :: node_data
...
!    displayed.)
!
!    Output, integer DIM_NUM, the spatial dimension, inferred 
...
!
!    Output, integer NODE_DATA_NUM, the number of data items per
...
!
  ...
  character ( len = * ) node_coord_file_name
  real ( kind = rk ), allocatable, dimension ( :, : ) :: node_data
  character ( len = * ) node_data_file_name
  integer node_data_num
  integer node_num
...
!    Input, character ( len = * ) FILE_NAME, the name of the file.
!
!    Output, integer LINE_NUM, the number of lines found 
...
!
!    Input, integer TEC_FILE_UNIT, the unit associated with 
!    the file.
!
!    Input, integer DIM_NUM, the spatial dimension.
!
...
!    Input, integer ELEMENT_ORDER, the order of the elements.
!
!    Input, integer NODE_DATA_NUM, the number of data items 
!    per node.
!
!    Input, real ( kind = rk ) NODE_COORD(DIM_NUM,NODE_NUM), the coordinates
!    of nodes.
!
!    Input, integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM);
!    the global index of local node I in element J.
!
!    Input, real ( kind = rk ) NODE_DATA(NODE_DATA_NUM,NODE_NUM), the
...
!
  ...
  real ( kind = rk ) node_data(node_data_num,node_num)
  character ( len = * ) tec_file_name
