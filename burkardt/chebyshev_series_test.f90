...
!*****************************************************************************80
!
!! chebyshev_series_test() tests chebyshev_series().
!
!  Licensing:
!
...
!
  ...
  real ( kind = rk ) s2
  real ( kind = rk ) s3
  real ( kind = rk ), dimension ( nc ) :: table5 = (/ & 
...
!
  ...
  real ( kind = rk ) s1
  real ( kind = rk ) s2
  real ( kind = rk ), dimension ( nc ) :: table1 = (/ &
...
!
  ...
  real ( kind = rk ) s1
  real ( kind = rk ) s2
  real ( kind = rk ), dimension ( nc ) :: table5odd = (/ &
    1.13031820798497005442D+00, &
    0.04433684984866380495D+00, &
    0.00054292631191394375D+00, &
    0.00000319843646240199D+00, &
