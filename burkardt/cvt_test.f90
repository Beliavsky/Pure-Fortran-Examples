...
!
  do rank = 0, n-1
    call tuple_next_fast ( ngrid, dim_num, rank, tuple )
    r(1:dim_num,rank+1) = real ( 2 * tuple(1:dim_num) - 1, kind = rk ) &
...
!
!    Output, real ( kind = rk ) R(DIM_NUM,N), an array of sample points from
!    the region.
!
  implicit none
...
!
  if ( .false. ) then
    init = 4
    do i = 1, n
      r(1,i) = real ( 2 * i - 1, kind = rk ) / real ( 2 * n, kind = rk )
    end do
  end if
