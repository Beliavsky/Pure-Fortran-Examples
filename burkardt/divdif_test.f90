...
!
  ...
  integer, parameter :: maxtab = 10

  real ( kind = rk ) diftab(maxtab)
  real ( kind = rk ) diftab2(maxtab)
  real ( kind = rk ) diftab3(maxtab)
  integer ntab
  integer ntab2
  integer ntab3
  real ( kind = rk ) xtab(maxtab)
  real ( kind = rk ) xtab2(maxtab)
  real ( kind = rk ) xtab3(maxtab)
  real ( kind = rk ) xval
  real ( kind = rk ) ytab(maxtab)
  real ( kind = rk ) yval
...
!
  ...
  do ntab = 1, maxtab
    ...
    err = 2.0D+00 * sqrt ( err ) / real ( ntab, kind = rk )
...
!
  ...
  write ( *, '(a)' ) '      X        Y'
  write ( *, '(a)' ) ' '
  xhi = real ( ntab, kind = rk )
  xlo = 1.0D+00

  do i = 1, nstep

    xval = ( real ( nstep - i,     kind = rk ) * xlo   &
           + real (         i - 1, kind = rk ) * xhi ) &
           / real ( nstep     - 1, kind = rk )
...
!
  ...
  real ( kind = rk ) c(maxtab)
  real ( kind = rk ) diftab1(maxtab)
  real ( kind = rk ) diftab2(maxtab)
  integer ntab
  real ( kind = rk ) xtab1(maxtab)
  real ( kind = rk ) xtab2(maxtab)
  real ( kind = rk ) ytab1(maxtab)
  real ( kind = rk ) ytab2(maxtab)
...
!
  ...
  integer, parameter :: maxtab = 8

  integer j
  integer ntab
  real ( kind = rk ) wtab(maxtab)
  real ( kind = rk ) xtab(maxtab)
...
!
  ...
  integer, parameter :: maxroots = 4

  real ( kind = rk ) c(maxroots+1)
  real ( kind = rk ) diftab(maxroots+1)
  integer nroots
  integer ntab
  real ( kind = rk ) roots(maxroots)
  real ( kind = rk ) xtab(maxroots+1)
...
!
  ...
  integer, parameter :: maxroot = 5

  real ( kind = rk ) c(maxroot+1)
  integer nc
  integer nroots
  real ( kind = rk ) roots(maxroot)
...
!
  ...
  real ( kind = rk ), allocatable :: c0(:)
  real ( kind = rk ), allocatable :: d0(:)
  real ( kind = rk ), allocatable :: d1(:)
  real ( kind = rk ), allocatable :: d2(:)
  real ( kind = rk ), allocatable :: d3(:)
  real ( kind = rk ), allocatable :: d4(:)
  real ( kind = rk ), allocatable :: f0(:)
  integer i
  integer k
  integer n0
  integer n1
...
!
  f0(1:n0) = 1.0D+00
  do i = 4, 1, -1
    f0(1:n0) = f0(1:n0) * x0(1:n0) / real ( i, kind = rk ) + 1.0D+00
  end do
!
