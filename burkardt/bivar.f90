...
!  Input:
!
!    integer MD, mode of computation.   MD must be 1, 2 or 3.
!    1 for new NCP and/or new XD, YD;
!    2 for old NCP, old XD, YD, new XI, YI;
!    3 for old NCP, old XD, YD, old XI, YI.
!
!    integer NCP, the number of additional data points used
!    for estimating partial derivatives at each data point.
!    2 < =  NCP < NDP.  Reasonable values are 3 < =  NCP < =  5.
!
!    integer NDP, the number of data points.
!    4 < =  NDP.
!
!    real ( kind = rk ) XD(NDP), Y(NDP), the X and Y coordinates
!    of the data points.
!
!    real ( kind = rk ) ZD(NDP), the data values at the data points.
!
!    integer NIP, the number of output points at which
!    interpolation is to be performed.
!    1 < =  NIP.
!
!    real ( kind = rk ) XI(NIP), YI(NIP), the coordinates of the
!    points at which interpolation is to be performed.
!
!  Output:
!
!    real ( kind = rk ) ZI(NIP), the interpolated data values.
!
!  Workspace:
!
...
!    of the data points.
!
!    integer NCP, the number of additional data points used for
!    estimating partial derivatives at each data point.
!
!  Output:
!
!    integer IPC(NCP*NDP), the indices of NCP data points closest
...
!    of the data points.
!
!    integer NT, the number of triangles.
!
!    integer IPT(3*NT), the indices of the triangle vertexes.
!
!    integer NL, the number of border line segments.
!
!    integer IPL(3*NL), containing the point numbers of the
!    end points of the border line segments and their respective triangle
!    numbers.
!
!    integer NXI, NYI, the number of grid points in the X and Y
!    coordinates.
!
!    real ( kind = rk ) XI(NXI), YI(NYI), the coordinates of the
!    grid points.
!
!  Output:
!
!    integer NGP(2*(NT+2*NL)) where the
!    number of grid points that belong to each of the
!    triangles or of the border line segments are to be stored.
!
!    integer IGP(NXI*NYI), where the grid point numbers
...
!    integer NT, the number of triangles.
!
!    integer IPT(3*NT), the point numbers of the vertexes of
!    the triangles,
!
!    integer NL, the number of border line segments.
!
!    integer IPL(3*NL), the point numbers of the end points of
!    the border line segments and their respective triangle numbers.
!
!    real ( kind = rk ) XII, YII, the coordinates of the point
!    to be located.
!
!    integer IWK(18*NDP), real ( kind = rk ) WK(8*NDP),
...
!  Output:
!
!    integer ITI, the triangle number, when the point is
...
!    4 < =  NDP.
!
!    real ( kind = rk ) XD(NDP), Y(NDP), Z(NDP), the X, Y and
!    Z coordinates of the data points.
!
!    integer NCP, the number of additional data points used for
!    estimating partial derivatives at each data point,
!
!    integer IPC(NCP*NDP), the indices of NCP data points
!    closest to each of the NDP data points.
!
!  Output:
!
!    real ( kind = rk ) PD(5*NDP), the estimated ZX, ZY, ZXX, ZXY,
...
!  Input:
!
!    real ( kind = rk ) XD(NDP), Y(NDP), Z(NDP), the X, Y and Z
...
!    integer NL, the number of border line segments.
!
!    integer IPL(3*NL), the point numbers of the end points
!    of the border line segments and their respective triangle numbers.
!
!    real ( kind = rk ) PDD(5*NDP). the partial derivatives at
!    the data points,
!
!    integer ITI, triangle number of the triangle in which lies
!    the point for which interpolation is to be performed,
!
!    real ( kind = rk ) XII, YII, the X and Y coordinates of the
!    point for which interpolation is to be performed.
!
!  Output:
!
!    real ( kind = rk ) ZII, the interpolated Z value.
!
  implicit none
...
!  Input:
!
!    integer MD, mode of computation (must be 1, 2, or 3,
...
!
!    integer NCP, the number of data points used to
!    estimate partial derivatives.  2 <= NCP < NDP.  A value between 3 and
!    5 is recommended.
!
!    integer NDP, the number of data points.  NDP must be
!    at least 4.
!
!    real ( kind = rk ) XD(NDP), Y(NDP), Z(NDP), the X, Y and Z
!    coordinates of the data points.
!
!    integer NXI, NYI, the number of output grid points in the
!    X and Y directions.  NXI and NYI must each be at least 1.
!
!    real ( kind = rk ) XI(NXI), YI(NYI), the X and Y coordinates
!    of the grid points.
!
!  Output:
!
!    real ( kind = rk ) ZI(NXI,NYI), contains the interpolated Z
...
!    integer NT, the number of triangles,
!
!    integer IPT(6*NDP-15), where the point numbers of the
!    vertexes of the IT-th triangle are to be stored as entries
!    3*IT-2, 3*IT-1, and 3*IT, for IT = 1 to NT.
!
!    integer NL, the number of border line segments.
!
!    integer IPL(6*NDP), where the point numbers of the end
...
!  Input:
!
!    real ( kind = rk ) X(*), Y(*), the coordinates of the data.
!
!    integer I1, I2, I3, I4, indices of four points
!    P1, P2, P3, and P4 that form a quadrilateral with P3 and P4 connected
!    diagonally.
!
!  Output:
!
!    integer IDXCHG, is 1 if an exchange is recommended,
!    and 0 otherwise.
!
  implicit none
