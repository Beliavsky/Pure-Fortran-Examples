...
!*****************************************************************************80
!
!! FEM3D_SAMPLE samples 3D finite element method data.
!
!  Discussion:
!
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: fem_node_xyz
  ...
  real ( kind = rk ), allocatable :: sample_node_xyz(:,:)
...
!
!    Input, real ( kind = rk ) FEM_NODE_XYZ(3,FEM_NODE_NUM), the coordinates 
!    of the nodes.
!
...
!
!    Input, real ( kind = rk ) SAMPLE_NODE_XYZ(3,SAMPLE_NODE_NUM), 
!    the sample nodes.
!
...
!
  ...
  integer fem_element_neighbor(4,fem_element_num)
  integer fem_element_node(fem_element_order,fem_element_num)
  real ( kind = rk ) fem_node_xyz(3,fem_node_num)
  real ( kind = rk ) fem_value(fem_value_dim,fem_node_num)
  integer i
  integer j
  real ( kind = rk ), dimension ( 3 ) :: p_xyz
  real ( kind = rk ) sample_node_xyz(3,sample_node_num)
  real ( kind = rk ) sample_value(fem_value_dim,sample_node_num)
  integer t
  integer t_node(fem_element_order)
  real ( kind = rk ) t_xyz(3,fem_element_order)

  call i4_fake_use ( fem_element_neighbor(1,1) )
!
