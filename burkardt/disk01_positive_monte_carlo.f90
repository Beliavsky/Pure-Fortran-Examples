function disk01_positive_area ( )

!*****************************************************************************80
!
!! disk01_positive_area() returns the area of the unit positive disk in 2D.
!
!  Licensing:
!
...
!  Parameters:
!
!    Output, real ( kind = rk ) disk01_positive_area, the area.
!
  implicit none
  ...
  disk01_positive_area = 0.25D+00 * r8_pi * r * r

  return
end
subroutine disk01_positive_monomial_integral ( e, integral )
...
!
  ...
  arg = real ( e(1) + 3, kind = rk ) / 2.0D+00
  f1 = gamma ( arg )
  arg = real ( e(2) + 1, kind = rk ) / 2.0D+00
  f2 = gamma ( arg )
  arg = real ( e(1) + e(2) + 4, kind = rk ) / 2.0D+00
  f3 = gamma ( arg )

  integral = f1 * f2 / f3 / 2.0D+00 / real ( 1 + e(1), kind = rk )

  return
end
subroutine disk01_positive_sample ( n, x )

!*****************************************************************************80
!
!! disk01_positive_sample() uniformly samples points from the unit positive disk.
!
!  Licensing:
!
!    This code is distributed under the MIT license. 
