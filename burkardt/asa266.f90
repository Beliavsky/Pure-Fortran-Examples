...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument of the digamma function.
!    0 < X.
!
!    Output, real ( kind = rk ) DIGAMMA, the value of the digamma function at X.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A(N), the probabilities for each component.
...
!    K < N.
!
!    Input, real ( kind = rk ) X(IX,K), contains the N by K array of samples
...
!
!    Input/output, real ( kind = rk ) ALPHA(K).
...
!
!    Output, real ( kind = rk ) RLOGL, the value of the log-likelihood function
!    at the solution point.
!
!    Output, real ( kind = rk ) V(K*(K+1)/2); V(J*(J-1)/2+I) contains the
!    covariance between ALPHA(I) and ALPHA(J), for I = 1 to J, J = 1 to K.
!
!    Output, real ( kind = rk ) G(K), contains an estimate of the derivative of
!    the log-likelihood with respect to each component of ALPHA.
!
!    Output, integer NITER, contains the number of Newton-Raphson
!    iterations performed.
!
!    Output, real ( kind = rk ) S, the value of the chi-squared statistic.
!
!    Output, real ( kind = rk ) EPS, contains the probability that the 
!    chi-squared statistic is less than S.
!
!    Workspace, real ( kind = rk ) WORK(2*K).
!
...
!
  ...
  real ( kind = rk ), parameter :: alpha_min = 0.00001D+00
  real ( kind = rk ) an
  real ( kind = rk ) beta
  real ( kind = rk ) chi2
  real ( kind = rk ) digamma
  real ( kind = rk ) eps
  real ( kind = rk ) g(k)
  real ( kind = rk ), parameter :: gamma = 0.0001D+00
  real ( kind = rk ) gammad
  real ( kind = rk ) gg
  integer i
  integer i2
  ...
  real ( kind = rk ) v((k*(k+1))/2)
  real ( kind = rk ) varp1
  real ( kind = rk ) work(2*k)
  real ( kind = rk ) x(ix,k)
  real ( kind = rk ) x_min
  real ( kind = rk ) x11
  real ( kind = rk ) x12
...
!
  gg = alngam ( rkk / 2.0D+00, ifault )

  chi2 = ppchi2 ( gamma, rkk, gg, ifault )
...
!
    ...
    if ( s < chi2 ) then
      eps = gammad ( s / 2.0D+00, rkk / 2.0D+00, ifault2 )
...
!    Each A(I) should be nonnegative, and at least one should be positive.
!
!    Output, real ( kind = rk ) MEAN(N), the means of the PDF.
!
  implicit none
...
!  Parameters:
!
!    Input, integer COMP_MAX, the leading dimension of A, which
!    must be at least COMP_NUM.
!
!    Input, integer COMP_NUM, the number of components in the
!    Dirichlet mixture density, that is, the number of distinct Dirichlet PDF's
!    that are mixed together.
!
!    Input, integer ELEM_NUM, the number of elements of an
!    observation.
!
!    Input, real ( kind = rk ) A(COMP_MAX,ELEM_NUM), the probabilities for
!    element ELEM_NUM in component COMP_NUM.
!    Each A(I,J) should be greater than or equal to 0.0.
!
!    Input, real ( kind = rk ) COMP_WEIGHT(COMP_NUM), the mixture weights of
!    the densities.
...
!    the sample.
!
!    Output, real ( kind = rk ) MEAN(ELEM_NUM), the means for each element.
!
  implicit none
  ...
  real ( kind = rk ) a(comp_max,elem_num)
  real ( kind = rk ) a_sum(comp_num)
  integer comp_i
  real ( kind = rk ) comp_weight(comp_num)
  real ( kind = rk ) comp_weight_sum
  integer elem_i
...
!    the sample.
!
!    Output, integer COMP, the index of the component of the
!    Dirichlet mixture that was chosen to generate the sample.
!
!    Output, real ( kind = rk ) X(ELEM_NUM), a sample of the PDF.
!
  implicit none
...
!
  do elem_i = 1, elem_num
    call gamma_sample ( a(comp,elem_i), 1.0D+00, x(elem_i) )
  end do
!
...
!    positive.
!
!    Output, real ( kind = rk ) X(N), a sample of the PDF.  The entries 
...
!
  ...
  do i = 1, n
    call gamma_sample ( a(i), 1.0D+00, x(i) )
  end do
!
!  Normalize the result.
!
...
!    Each A(I) should be nonnegative, and at least one should be positive.
!
!    Output, real ( kind = rk ) VARIANCE(N), the variances of the PDF.
!
  implicit none
...
!    0.0 <= CDF <= 1.0.
!
!    Input, real ( kind = rk ) A, B, the parameters of the PDF.
!    0.0 < B.
!
...
!
!    Output, real ( kind = rk ) GAMAIN, the value of the incomplete
!    gamma ratio.
!
  implicit none
  ...
  real ( kind = rk ) a
  real ( kind = rk ), parameter :: acu = 1.0D-08
  real ( kind = rk ) alngam
  real ( kind = rk ) an
  real ( kind = rk ) arg
  real ( kind = rk ) b
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, P, the arguments of the incomplete
...
!
!    Output, real ( kind = rk ) GAMMDS, the value of the incomplete
!    Gamma integral.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) Z, the argument of the Gamma function.
!
!    Output, integer IER, error flag.
!    0, no error occurred.
!    1, Z is less than or equal to 0.
!
!    Output, real ( kind = rk ) LNGAMMA, the logarithm of the gamma 
!    function of Z.
!
  implicit none
  ...
  real ( kind = rk ), parameter :: lnsqrt2pi = 0.9189385332046727D+00

  real ( kind = rk ), parameter, dimension ( 9 ) :: a = (/ &
...
!  Parameters:
!
!    Input, real ( kind = rk ) Z, divides the real line into two semi-infinite
!    intervals, over each of which the standard normal distribution
!    is to be integrated.
!
!    Output, real ( kind = rk ) P, Q, the integrals of the standard normal
!    distribution over the intervals ( - Infinity, Z] and
!    [Z, + Infinity ), respectively.
!
!    Output, real ( kind = rk ) PDF, the value of the standard normal
!    distribution at Z.
!
  implicit none
  ...
  real ( kind = rk ), parameter :: cutoff = 7.071D+00
  real ( kind = rk ), parameter :: p0 = 220.2068679123761D+00
  real ( kind = rk ), parameter :: p1 = 221.2135961699311D+00
  real ( kind = rk ), parameter :: p2 = 112.0792914978709D+00
  real ( kind = rk ), parameter :: p3 = 33.91286607838300D+00
  real ( kind = rk ), parameter :: p4 = 6.373962203531650D+00
  real ( kind = rk ), parameter :: p5 = 0.7003830644436881D+00
  real ( kind = rk ), parameter :: p6 = 0.03526249659989109D+00
  real ( kind = rk ), parameter :: q0 = 440.4137358247522D+00
  real ( kind = rk ), parameter :: q1 = 793.8265125199484D+00
  real ( kind = rk ), parameter :: q2 = 637.3336333788311D+00
  real ( kind = rk ), parameter :: q3 = 296.5642487796737D+00
  real ( kind = rk ), parameter :: q4 = 86.78073220294608D+00
  real ( kind = rk ), parameter :: q5 = 16.06417757920695D+00
  real ( kind = rk ), parameter :: q6 = 1.755667163182642D+00
  real ( kind = rk ), parameter :: q7 = 0.08838834764831844D+00
  real ( kind = rk ), parameter :: root2pi = 2.506628274631001D+00
...
!  Parameters:
!
!    Input, real ( kind = rk ) Z, divides the line into two semi-infinite
!    intervals, over each of which the standard normal distribution
!    is to be integrated.
!
!    Output, real ( kind = rk ) P, Q, the integrals of the standard normal
...
!  Parameters:
!
!    Input, real ( kind = rk ) P,  value of the chi-squared cumulative
!    probability density function.
!    0.000002 <= P <= 0.999998.
!
!    Input, real ( kind = rk ) V, the parameter of the chi-squared probability
!    density function.
!    0 < V.
!
!    Input, real ( kind = rk ) G, the value of log ( Gamma ( V / 2 ) ).
!
...
!
!    Output, real ( kind = rk ) PPCHI2, the value of the chi-squared random
...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ), parameter :: aa = 0.6931471806D+00
  real ( kind = rk ) b
  real ( kind = rk ) c
  real ( kind = rk ), parameter :: c1 = 0.01D+00
  real ( kind = rk ), parameter :: c2 = 0.222222D+00
  real ( kind = rk ), parameter :: c3 = 0.32D+00
  real ( kind = rk ), parameter :: c4 = 0.4D+00
  real ( kind = rk ), parameter :: c5 = 1.24D+00
  real ( kind = rk ), parameter :: c6 = 2.2D+00
  real ( kind = rk ), parameter :: c7 = 4.67D+00
  real ( kind = rk ), parameter :: c8 = 6.66D+00
  real ( kind = rk ), parameter :: c9 = 6.73D+00
  real ( kind = rk ), parameter :: c10 = 13.32D+00
  real ( kind = rk ), parameter :: c11 = 60.0D+00
  real ( kind = rk ), parameter :: c12 = 70.0D+00
  real ( kind = rk ), parameter :: c13 = 84.0D+00
  real ( kind = rk ), parameter :: c14 = 105.0D+00
  real ( kind = rk ), parameter :: c15 = 120.0D+00
  real ( kind = rk ), parameter :: c16 = 127.0D+00
  real ( kind = rk ), parameter :: c17 = 140.0D+00
  real ( kind = rk ), parameter :: c18 = 175.0D+00
  real ( kind = rk ), parameter :: c19 = 210.0D+00
  real ( kind = rk ), parameter :: c20 = 252.0D+00
  real ( kind = rk ), parameter :: c21 = 264.0D+00
  real ( kind = rk ), parameter :: c22 = 294.0D+00
  real ( kind = rk ), parameter :: c23 = 346.0D+00
  real ( kind = rk ), parameter :: c24 = 420.0D+00
  real ( kind = rk ), parameter :: c25 = 462.0D+00
  real ( kind = rk ), parameter :: c26 = 606.0D+00
  real ( kind = rk ), parameter :: c27 = 672.0D+00
  real ( kind = rk ), parameter :: c28 = 707.0D+00
  real ( kind = rk ), parameter :: c29 = 735.0D+00
  real ( kind = rk ), parameter :: c30 = 889.0D+00
  real ( kind = rk ), parameter :: c31 = 932.0D+00
  real ( kind = rk ), parameter :: c32 = 966.0D+00
  real ( kind = rk ), parameter :: c33 = 1141.0D+00
  real ( kind = rk ), parameter :: c34 = 1182.0D+00
  real ( kind = rk ), parameter :: c35 = 1278.0D+00
  real ( kind = rk ), parameter :: c36 = 1740.0D+00
  real ( kind = rk ), parameter :: c37 = 2520.0D+00
  real ( kind = rk ), parameter :: c38 = 5040.0D+00
  real ( kind = rk ) ch
  real ( kind = rk ), parameter :: e = 0.5D-06
  real ( kind = rk ) g
  real ( kind = rk ) gammad
  integer i
  integer if1
  ...
  real ( kind = rk ) ppchi2
  real ( kind = rk ) p
  real ( kind = rk ), parameter :: pmax = 0.999998D+00
  real ( kind = rk ), parameter :: pmin = 0.000002D+00
  real ( kind = rk ) p1
  real ( kind = rk ) p2
  real ( kind = rk ) ppnd
  real ( kind = rk ) q
...
!
!    Output, real ( kind = rk ) PPND, the normal deviate value with the property
...
!    1, P <= 0 or 1 <= P.
!
!    Output, real ( kind = rk ) PPND16, the normal deviate value with the
...
!
  ...
  real ( kind = rk ), parameter :: a0 = 3.3871328727963666080d+00
  real ( kind = rk ), parameter :: a1 = 1.3314166789178437745d+02
  real ( kind = rk ), parameter :: a2 = 1.9715909503065514427d+03
  real ( kind = rk ), parameter :: a3 = 1.3731693765509461125d+04
  real ( kind = rk ), parameter :: a4 = 4.5921953931549871457d+04
  real ( kind = rk ), parameter :: a5 = 6.7265770927008700853d+04
  real ( kind = rk ), parameter :: a6 = 3.3430575583588128105d+04
  real ( kind = rk ), parameter :: a7 = 2.5090809287301226727d+03
  real ( kind = rk ), parameter :: b1 = 4.2313330701600911252d+01
  real ( kind = rk ), parameter :: b2 = 6.8718700749205790830d+02
  real ( kind = rk ), parameter :: b3 = 5.3941960214247511077d+03
  real ( kind = rk ), parameter :: b4 = 2.1213794301586595867d+04
  real ( kind = rk ), parameter :: b5 = 3.9307895800092710610d+04
  real ( kind = rk ), parameter :: b6 = 2.8729085735721942674d+04
  real ( kind = rk ), parameter :: b7 = 5.2264952788528545610d+03
  real ( kind = rk ), parameter :: c0 = 1.42343711074968357734d+00
  real ( kind = rk ), parameter :: c1 = 4.63033784615654529590d+00
  real ( kind = rk ), parameter :: c2 = 5.76949722146069140550d+00
  real ( kind = rk ), parameter :: c3 = 3.64784832476320460504d+00
  real ( kind = rk ), parameter :: c4 = 1.27045825245236838258d+00
  real ( kind = rk ), parameter :: c5 = 2.41780725177450611770d-01
  real ( kind = rk ), parameter :: c6 = 2.27238449892691845833d-02
  real ( kind = rk ), parameter :: c7 = 7.74545014278341407640d-04
  real ( kind = rk ), parameter :: const1 = 0.180625d+00
  real ( kind = rk ), parameter :: const2 = 1.6d+00
  real ( kind = rk ), parameter :: d1 = 2.05319162663775882187d+00
  real ( kind = rk ), parameter :: d2 = 1.67638483018380384940d+00
  real ( kind = rk ), parameter :: d3 = 6.89767334985100004550d-01
  real ( kind = rk ), parameter :: d4 = 1.48103976427480074590d-01
  real ( kind = rk ), parameter :: d5 = 1.51986665636164571966d-02
  real ( kind = rk ), parameter :: d6 = 5.47593808499534494600d-04
  real ( kind = rk ), parameter :: d7 = 1.05075007164441684324d-09
  real ( kind = rk ), parameter :: e0 = 6.65790464350110377720d+00
  real ( kind = rk ), parameter :: e1 = 5.46378491116411436990d+00
  real ( kind = rk ), parameter :: e2 = 1.78482653991729133580d+00
  real ( kind = rk ), parameter :: e3 = 2.96560571828504891230d-01
  real ( kind = rk ), parameter :: e4 = 2.65321895265761230930d-02
  real ( kind = rk ), parameter :: e5 = 1.24266094738807843860d-03
  real ( kind = rk ), parameter :: e6 = 2.71155556874348757815d-05
  real ( kind = rk ), parameter :: e7 = 2.01033439929228813265d-07
  real ( kind = rk ), parameter :: f1 = 5.99832206555887937690d-01
  real ( kind = rk ), parameter :: f2 = 1.36929880922735805310d-01
  real ( kind = rk ), parameter :: f3 = 1.48753612908506148525d-02
  real ( kind = rk ), parameter :: f4 = 7.86869131145613259100d-04
  real ( kind = rk ), parameter :: f5 = 1.84631831751005468180d-05
  real ( kind = rk ), parameter :: f6 = 1.42151175831644588870d-07
  real ( kind = rk ), parameter :: f7 = 2.04426310338993978564d-15
...
!
      ...
      nq = int ( w * 4.0D+00 )
      w = 4.0D+00 * ( w - real ( nq, kind = rk ) * fourth )
!
...
!
!    Input, real ( kind = rk ) A(M,N), the array to be examined.
!
!    Output, real ( kind = rk ) MEAN(N), the means, or averages, of the columns.
!
  implicit none
  ...
  mean(1:n) = mean(1:n) / real ( m, kind = rk  )
...
!    the array.
!
!    Input, real ( kind = rk ) A(M,N), the array whose variances are desired.
!
!    Output, real ( kind = rk ) VARIANCE(N), the variances of the rows.
!
  implicit none
  ...
  do j = 1, n
    ...
    mean = sum ( a(1:m,j) ) / real ( m, kind = rk  )
    ...
    if ( 1 < m ) then
      variance(j) = variance(j) / real ( m - 1, kind = rk )
    else
      variance(j) = 0.0D+00
    end if
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument of the trigamma function.
!    0 < X.
!
!    Output, real ( kind = rk ) TRIGAMMA, the value of the trigamma function 
!    at X.
!
  implicit none
  ...
  real ( kind = rk ), parameter :: b2 =   1.0D+00 / 6.0D+00
  real ( kind = rk ), parameter :: b4 = - 1.0D+00 / 30.0D+00
  real ( kind = rk ), parameter :: b6 =   1.0D+00 / 42.0D+00
  real ( kind = rk ), parameter :: b8 = - 1.0D+00 / 30.0D+00
  real ( kind = rk ) trigamma
  real ( kind = rk ) x
  real ( kind = rk ) y
  real ( kind = rk ) z
