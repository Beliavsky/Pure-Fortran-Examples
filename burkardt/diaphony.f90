...
!
  ...
  integer point_num
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: points
...
!
  ...
  if ( 1 <= arg_num ) then
    ...
    write ( *, '(a)' ) '  Input comes from "' // trim ( input_filename ) // '".'
  else
    write ( *, '(a)' ) ' '
...
!  Read the data.
!
  allocate ( points(1:dim_num,1:point_num) )

  call r8mat_data_read ( input_filename, dim_num, point_num, points )

  if ( minval ( points ) < 0.0D+00 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'DIAPHONY - Fatal error!'
    write ( *, '(a)' ) '  At least one coordinate of a point is less than 0!'
    stop
  else if ( 1.0D+00 < maxval ( points ) ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'DIAPHONY - Fatal error!'
...
!
  ...
  e = 1.0D+00 / sqrt ( real ( point_num, kind = rk8 ) )
  de = d / e
...
!
!    Input, real ( kind = rk8 ) X(DIM_NUM,POINT_NUM), the point set, which is
!    presumed to lie in the DIM_NUM dimensional unit hypercube.
!
!    Output, real ( kind = rk8 ) D, the value of the diaphony.
!
  implicit none
  ...
  real ( kind = rk8 ) r8_modp
  real ( kind = rk8 ) x(dim_num,point_num)
  real ( kind = rk8 ) z(dim_num)
  ...
  do i = 1, point_num
    do j = 1, point_num
      ...
      do k = 1, dim_num
        z(k) = r8_modp ( z(k), 1.0D+00 )
      end do
      d = d + f ( dim_num, z )
    end do
  end do

  bot = ( real ( point_num, kind = rk8 ) )**2 &
...
!    Input, integer DIM_NUM, the spatial dimension.
!
!    Input, real ( kind = rk8 ) X(DIM_NUM), the value of the argument.
!
!    Output, real ( kind = rk8 ) F, the value of the test function.
!
  implicit none
...
subroutine file_column_count ( filename, ncolumn )
...
!
!    Output, integer NCOLUMN, the number of columns.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the number to be divided.
!
!    Input, real ( kind = rk8 ) Y, the number that divides X.
!
!    Output, real ( kind = rk8 ) R8_MODP, the nonnegative remainder
...
!
!    Output, real ( kind = rk8 ) TABLE(M,N), the data.
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) DVAL, the value read from the string.
!
...
!
!    Output, real ( kind = rk8 ) RVEC(N), the values read from the string.
!
!    Output, integer IERROR, error flag.
!    0, no errors occurred.
