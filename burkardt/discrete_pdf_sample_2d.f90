...
!*****************************************************************************80
!
!! discrete_pdf_sample_2d() defines a PDF using discrete 2D data.
!
!  Discussion:
!
...
!  Parameters:
!
!    Local, integer N, the number of sample points to be generated.
!
  implicit none
...
!
  ...
  call discrete_cdf_to_xy ( n1, n2, cdf, n, u, xy )
!
...
subroutine discrete_cdf_to_xy ( n1, n2, cdf, n, u, xy )
...
!    of PDF data.
!
!    Input, real ( kind = rk ) CDF(N1,N2), the CDF values associated with each 
...
!
!    Input, real ( kind = rk ) U(N), N random values.
!
!    Output, real ( kind = rk ) XY(2,N), the sample points in the unit square.
!
  implicit none
    ...
    do i = 1, n1
      ...
      do k = 1, n
        if ( low <= u(k) .and. u(k) <= high ) then
          ...
          xy(1,k) = ( real ( i - 1, kind = rk ) + r(1) ) / real ( n1, kind = rk )
          xy(2,k) = ( real ( j - 1, kind = rk ) + r(2) ) / real ( n2, kind = rk )
        end if
      end do
      low = high
    end do
...
!    of PDF data.
!
!    Output, real ( kind = rk ) PDF(N1,N2).  PDF(I,J) is the discrete PDF 
...
!
  ...
  real ( kind = rk ) pdf(n1,n2)
  real ( kind = rk ), save :: pdf_save(20,20) = reshape ( (/ &
...
!
  ...
  real ( kind = rk ) pdf(n1,n2)
  real ( kind = rk ), save :: pdf_save(12,8) = reshape ( (/ &
...
!    of PDF data.
!
!    Input, real ( kind = rk ) PDF(N1,N2), the discrete PDF for the cell (I,J),
!    normalized so that the sum over all cells is 1.
!
!    Output, real ( kind = rk ) CDF(N1,N2), the discrete CDF for the cell (I,J).
