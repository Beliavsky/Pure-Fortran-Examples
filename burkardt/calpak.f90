...
!  Parameters:
!
!    Input, real ( kind = rk ) DN, a MATLAB DATENUM.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  implicit none
...
!
!    Output, real ( kind = rk ) DATE_NUM, the datenum value.
!
  implicit none
  ...
  integer, save, dimension ( n_max ) :: d_vec = (/ &
    ...
    25, &
     1 /)
  real ( kind = rk ) date_num
  real ( kind = rk ), save, dimension ( n_max ) :: date_num_vec = (/ &
         1.0D+00, &
       367.0D+00, &
     36526.0D+00, &
    365244.0D+00, &
...
!  Parameters:
!
!    Input/output, integer Y, integer M,
!    integer D, a year, month, and day
...
!
  ...
  integer days
  integer m
  integer month_length_alexandrian
  integer y
...
!
  ...
  integer days
  integer m
  integer month_length_gregorian
  integer y
...
!
  ...
  integer days
  integer m
  integer month_length_republican
  integer y
...
!
!    Output, integer DAYS_BEFORE_MONTH_COMMON, the number of
...
!
  ...
  integer days_before_month_common
  integer y
  integer y2
...
!
  ...
  integer, parameter, dimension(12) :: mdays = (/ &
     0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 /)
  integer days_before_month_gregorian
  integer y
  integer y2
...
!    Input, integer M, the number of the month.
!
!    Output, integer DAYS_BEFORE_MONTH_JULIAN, the number of
!    days in the year before the first day of the given month.
!
  implicit none
  ...
  integer, parameter, dimension(12) :: mdays = (/ &
     0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 /)
  integer days_before_month_julian
  integer y
  integer y2
...
!
  ...
  integer d
  integer, dimension ( n_test ) :: d_test = &
    (/  30,    12,    4,  23,   15,   31,   20,   11,   27,   16 /)
  integer i
  integer m
  integer, dimension ( n_test ) :: m_test = &
    (/   3,     4,    4,    4,    4,    3,    4,    4,    3,    4 /)
  character ( len = 20 ) s
  integer y
  integer, dimension ( n_test ) :: y_test = &
...
!  Parameters:
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date of the epoch.
!
  implicit none
...
subroutine epoch_to_jed_byzantine ( jed )

!*****************************************************************************80
!
!! epoch_to_jed_byzantine returns the epoch of the Byzantine calendar as a JED.
!
!  Discussion:
!
...
!  Output:
!
!    real ( kind = rk ) JED, the Julian Ephemeris Date of the epoch.
!
  implicit none
...
!  Input:
!
!    integer Y, M, D, real ( kind = rk ) F, a YMDF date.
!
!  Output:
!
!    integer Y, M, D, real ( kind = rk ) F,
!    the YMDF date after borrowing.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) F, a day fraction between 0.0 and 1.0.
!
...
!
  ...
  f2 = 24.0D+00 * f2
  h = int ( f2 )
  f2 = f2 - real ( h, kind = rk )

  f2 = 60.0D+00 * f2
  m = int ( f2 )
  f2 = f2 - real ( m, kind = rk )

  f2 = 60.0D+00 * f2
  s = int ( f2 )
  f2 = f2 - real ( s, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) F, the number to be written into the string.
...
!  Parameters:
!
!    Input, integer C, integer W, real ( kind = rk ) S,
!    the GPS cycle/week/second date.
!
!    Output, real ( kind = rk ) JED, the corresponding Julian Ephemeris Date.
!
  implicit none
...
!  Parameters:
!
!    Input, integer C,  integer W,
!    real ( kind = rk ) SEC, the GPS cycle/week/second date.
!
...
!
!    Output, integer I4_MODP, the positive remainder
...
!
  ...
  integer d
  integer, dimension ( test_num ) :: d_vec = (/ &
    50, -50, 50, -50 /)
  integer i4_modp
  integer m
  integer n
  integer, dimension ( test_num ) :: n_vec = (/ &
    107, 107, -107, -107 /)
  integer r
  integer test
...
!  Output:
!
!    integer I4_NORMAL_AB, a sample of the normal PDF.
!
  implicit none
...
!  Parameters:
!
!    Input, integer INTVAL, an integer to be converted.  If the
...
!  Output:
!
!    integer Y, M, D, the YMD date after inflation.
!
  implicit none
...
!
  ...
  integer j
  integer y
  integer year_length_days_common
...
!
  ...
  integer j
  integer y
  integer year_length_days_english
...
!
  ...
  integer j
  integer y
  integer year_length_days_gregorian
...
!
  ...
  integer j
  integer y
  integer year_length_days_hebrew
...
!
  ...
  integer j
  integer y
  integer year_length_days_islamic
...
!
  ...
  integer j
  integer y
  integer year_length_days_julian
...
!
  ...
  integer j
  integer y
  integer year_length_days_republican
...
!
  ...
  integer j
  integer y
  integer year_length_days_roman
...
!    Output, integer Y, M, D, the Common Era date.
!
!    Output, real ( kind = rk ) F, the fractional part of the day.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  integer, parameter :: n_max = 51
  ...
  real ( kind = rk ), save, dimension ( n_max ) :: f_vec = (/ &
    ...
    0.50D+00 /)
  real ( kind = rk ) jed
  real ( kind = rk ), save, dimension ( n_max ) :: jed_vec = (/ &
           0.00D+00, &
           1.00D+00, &
      259261.00D+00, &
      347998.50D+00, &
...
!  Parameters:
!
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, logical JED_IS_LEGAL, is TRUE if JED is legal, and 
!    FALSE otherwise.
!
  implicit none
...
!
  ...
  logical jed_is_legal
  integer :: jed_num = 7
  real ( kind = rk ), dimension ( 7 ) :: jed_value = (/ &
...
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) MJD, the Modified Julian Ephemeris Date.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  integer, parameter :: n_max = 33
  ...
  integer n_data
  real ( kind = rk ) mjd
  real ( kind = rk ), save, dimension ( n_max ) :: mjd_vec = (/ &
    -892769.0D+00, &
    -739963.0D+00, &
    -653107.0D+00, &
    -629359.0D+00, &
...
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) RD, the Reingold Dershowitz Date.
!
  implicit none
  ...
  integer n_data
  real ( kind = rk ) rd
  real ( kind = rk ), save, dimension ( n_max ) :: rd_vec = (/ &
    -214193.0D+00, &
     -61387.0D+00, &
      25469.0D+00, &
      49217.0D+00, &
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) DN, a MATLAB DATENUM.
!
  implicit none
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, integer C, W, real ( kind = rk ) S, the
...
!
  ...
  w = int ( d ) / 7
  d = d - real ( 7 * w, kind = rk )

  c = w / 1024
  w = w - 1024 * c

  s = d * real ( 24.0D+00 * 60.0D+00 * 60.0D+00, kind = rk )
...
!
!    Output, real ( kind = rk ) F, the fractional part of the date.
!
  implicit none
  ...
  j = int ( jed - jed_epoch )
  f = ( jed - jed_epoch ) - real ( j, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) JED1, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) JED2, the Julian Ephemeris Date
...
!
  ...
  integer issue
  real ( kind = rk ) jed
  real ( kind = rk ) jed_02_01_1852
  real ( kind = rk ) jed_07_06_1852
  real ( kind = rk ) jed_07_02_1853
  real ( kind = rk ) jed_07_06_1854
  real ( kind = rk ) jed_07_05_1855
  real ( kind = rk ) jed_25_09_1855
  real ( kind = rk ) jed_29_09_1855
  real ( kind = rk ) jed_04_01_1856
  real ( kind = rk ) jed_07_07_1856
  real ( kind = rk ) jed_03_01_1857
  real ( kind = rk ) jed_02_01_1858
  real ( kind = rk ) jed_06_07_1858
  real ( kind = rk ) jed_05_07_1859
  real ( kind = rk ) jed_03_01_1860
  real ( kind = rk ) jed_05_07_1860
  real ( kind = rk ) jed_02_01_1861
  real ( kind = rk ) jed_04_21_1861
  real ( kind = rk ) jed_04_28_1861
  real ( kind = rk ) jed_05_05_1861
  real ( kind = rk ) jed_05_07_1861
  real ( kind = rk ) jed_02_01_1862
  real ( kind = rk ) jed_05_07_1862
  real ( kind = rk ) jed_02_01_1863
  real ( kind = rk ) jed_28_09_1863
  real ( kind = rk ) jed_30_09_1863
  real ( kind = rk ) jed_02_01_1864
  real ( kind = rk ) jed_05_07_1864
  real ( kind = rk ) jed_03_01_1865
  real ( kind = rk ) jed_05_07_1865
  real ( kind = rk ) jed_02_01_1866
  real ( kind = rk ) jed_02_01_1867
  real ( kind = rk ) jed_07_02_1898
  real ( kind = rk ) jed_22_04_1905
  real ( kind = rk ) jed_10_08_1978
  real ( kind = rk ) jed_05_11_1978
  real ( kind = rk ) jed_01_01_2000
  real ( kind = rk ) jed_epoch
  integer m
  integer m2
  integer sundays
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) RD, the RD date.
!
  implicit none
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) S, the corresponding UNIX SS date.
!
  implicit none
...
!
  ...
  f = ( jedmod + 0.5D+00 ) - real ( j, kind = rk )
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) BESSEL, the Bessel year.
!
  implicit none
  ...
  real ( kind = rk ) jed
  real ( kind = rk ) jed_epoch
  real ( kind = rk ), parameter :: year_length_days = 365.242198781D+00
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) JULIAN, the Julian year.
!
  implicit none
  ...
  real ( kind = rk ) jed_epoch
  real ( kind = rk ) julian
  real ( kind = rk ), parameter :: year_length_days = 365.25D+00
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, integer Y, J, real ( kind = rk ) F, the YJF date.
!
  implicit none
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, integer Y, M, D, real ( kind = rk ) F,
!    the YMDF date.
!
  implicit none
...
!    Input, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
!    Output, integer Y, M, D, real ( kind = rk ) F, the YMDF date.
!
  implicit none
...
!
  ...
  real ( kind = rk ) jed_epoch
  integer m
  integer month_length_eg_lunar
  integer ncycle
  integer y
  integer year_length_days_eg_lunar
...
!
  ...
  real ( kind = rk ) f
  real ( kind = rk ) f1
  integer, parameter :: g1 = 365
  integer, parameter :: g4 = 1461
  integer, parameter :: g100 = 36524
  integer, parameter :: g400 = 146097
  integer i4_modp
  integer j
  real ( kind = rk ) jed
  real ( kind = rk ) jed_epoch
  ...
  j = int ( jed - jed_epoch )
  f1 = ( jed - jed_epoch ) - real ( j, kind = rk )
...
!
  ...
  real ( kind = rk ) jf
  integer m
  real ( kind = rk ) month_length_hindu_solar
  integer y
  real ( kind = rk ) year_length_days_hindu_solar
...
!
  ...
  jf = jf - ( real ( m - 1, kind = rk ) * month_length_hindu_solar ( ) )
!
...
!
  ...
  jd = int ( ( real ( j + 1524, kind = rk ) - 122.1D+00 ) / 365.25D+00 )
  je = int ( 365.25D+00 * real ( jd, kind = rk ) )
  jg = int ( real ( j + 1524 - je, kind = rk ) / 30.6001D+00 )
!
...
!
  ...
  integer, parameter :: cycle_length = 1461
  integer d
  integer d0
  integer d1
  integer d2
  ...
  integer y
  integer y1
  integer, parameter :: year_length_days = 365
...
!
  ...
  integer n_data
  integer weekday
  integer, save, dimension ( n_max ) :: weekday_vec = (/ &
    1, 4, 4, 1, 4, &
    2, 7, 1, 1, 6, &
    7, 6, 1, 1, 4, &
    7, 7, 7, 4, 1, &
...
!    defining the Mayan long date.
!
!    Input, real ( kind = rk ) F, the fractional part of the date.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  implicit none
  ...
  jed = jed_epoch + real ( days, kind = rk ) + f
...
!
  ...
  call epoch_to_jed_mayan_long ( jed_epoch )
  jed = jed_epoch + real ( 18980 * y + r, kind = rk ) + f
...
!  Parameters:
!
!    Input, real ( kind = rk ) MJD, the modified Julian Ephemeris Date.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  implicit none
...
!
  ...
  integer months
  integer y
  integer year_length_months_alexandrian
...
!
  ...
  integer months
  integer y
  integer year_length_months_bahai
...
!
  ...
  integer months
  integer y
  integer year_length_months_common
...
!
  ...
  integer months
  integer y
  integer year_length_months_eg_civil
...
!
  ...
  integer months
  integer y
  integer year_length_months_english
...
!
  ...
  integer months
  integer y
  integer year_length_months_gregorian
...
!
  ...
  integer months
  integer y
  integer year_length_months_hebrew
...
!
  ...
  integer months
  integer y
  integer year_length_months_islamic
...
!
  ...
  integer months
  integer y
  integer year_length_months_julian
...
!
  ...
  integer months
  integer y
  integer year_length_months_republican
...
!
  ...
  integer months
  integer y
  integer year_length_months_roman
...
!
  ...
  integer m
  integer m2
  integer, parameter, dimension(13) :: mdays = (/ &
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_BAHAI, the number of
!    days in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_COMMON, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_COPTIC, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_EG_CIVIL, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_EG_LUNAR, the number of days
!    in the month.
!
  implicit none
  ...
  integer year_length_months_eg_lunar
!
!  Copy the input.
!
  m2 = m
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_ENGLISH, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_ETHIOPIAN, the number of days
!    in the month.
!
  implicit none
  ...
  integer, parameter, dimension(13) :: mdays = (/ &
    30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 5 /)
  integer month_length_ethiopian
  integer y
  integer y2
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_GREEK, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_GREGORIAN, the number of days
!    in the month.
!
  implicit none
...
!    returned as 0 days.
!
!    Output, integer MONTH_LENGTH_HEBREW, the number of days
!    in the month.
!
  implicit none

  integer, dimension (6,13) :: a = reshape ( source = (/ &
    30,  30,  30,  30,  30,  30, &
    29,  29,  30,  29,  29,  30, &
    29,  30,  30,  29,  30,  30, &
    29,  29,  29,  29,  29,  29, &
...
!  Parameters:
!
!    Output, real ( kind = rk ) MONTH_LENGTH_HINDU_SOLAR, the number of
!    days in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_IRANIAN, the number of
!    days in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_JULIAN, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, real ( kind = rk ) MONTH_LENGTH_LUNAR, the number of days in
!    the month.
!
  implicit none
  ...
  integer m
  real ( kind = rk ) month_length_lunar
  integer y
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_PERSIAN, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_REPUBLICAN, the number of days
!    in the month.
!
  implicit none
...
!    Input, integer M, the number of the month.
!
!    Output, integer MONTH_LENGTH_ROMAN, the number of days
!    in the month.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) MONTH_LENGTH_SYNODIC, the length of the
!    mean synodic month,
!    in days.
!
  implicit none
  ...
  real ( kind = rk ) month_length_synodic
...
!
!    Output, integer M, the number of the month, or -1 if the name
...
!
  ...
  integer d
  integer m
  integer, parameter, dimension(12) :: ides = (/ &
...
!
  ...
  integer months
  integer y
  integer year_length_months_greek
...
!
  ...
  integer d
  integer m
  integer, parameter, dimension(12) :: nones = (/ &
...
!    3=last quarter.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date on which the
...
!
  ...
  real ( kind = rk ), parameter :: degrees_to_radians = pi / 180.0D+00
...
!
  ...
  jed = jed_epoch - 1 + real ( d + mod ( mod ( d + 5, 7 ), 2 ), kind = rk )
...
!
  ...
  f = real ( mu, kind = rk )
  f = real ( s, kind = rk ) + f / 1000.0D+00
  f = real ( n, kind = rk ) + f / 60.0D+00
  f = real ( h, kind = rk ) + f / 60.0D+00
  f = f / 24.0D+00
...
!
  ...
  f1 = real ( mu1, kind = rk )
  f1 = real ( s1, kind = rk ) + f1 / 1000.0D+00
  f1 = real ( n1, kind = rk ) + f1 / 60.0D+00
  f1 = real ( h1, kind = rk ) + f1 / 60.0D+00
  f1 = f1 / 24.0D+00
...
!  Parameters:
!
!    Input, integer ( kind = rk ) VOLUME, ISSUE, the New York Times
!    volume and issue.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  ...
  integer issue
  real ( kind = rk ) jed
  real ( kind = rk ), parameter :: jed_epoch_50000 = 2449790.5D+00
  integer volume

  if ( 149 < volume ) then
    jed = jed_epoch_50000 + real ( issue - 50000 + 500, kind = rk )
!
...
!
    ...
    jed = jed_epoch_50000 + real ( issue - 50000, kind = rk )
!
!  Factor in the strike of 1978.
!
  else
    jed = jed_epoch_50000 + real ( issue - 50000 - 88, kind = rk )
  end if
...
!
  ...
  integer, parameter :: test_num = 34

  integer, dimension ( test_num ) :: d_test = (/ &
  ...
  real ( kind = rk ) f1
  real ( kind = rk ) f2
  integer, dimension ( test_num ) :: issue_test = (/ &
  ...
  character ( len = 25 ) s2
  integer test
  integer, dimension ( test_num ) :: volume_test = (/ &
  ...
  integer volume1
  integer volume3
  integer, dimension ( test_num ) :: y_test = (/ &
...
!  Parameters:
!
!    Input, real ( kind = rk ) D, the number to be written into the string.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) RD, the RD Date.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  implicit none
...
!    1, an error occurred.
!
!    Output, integer LAST, the last character used to make IVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) S, the UNIX date.
!
!    Output, real ( kind = rk ) JED, the corresponding Julian Ephemeris Date.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date of the date.
!
  implicit none
...
!    Output, integer Y, M, D, the Common Era date.
!
!    Output, integer ( kind = rk ) W, the day of the week.  Sunday = 1.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ALEXANDRIAN, the number of
!    days in the year.
!
  implicit none
  ...
  integer y
  logical year_is_leap_alexandrian
  integer year_length_days_alexandrian
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_BAHAI, the number of
!    days in the year.
!
  implicit none
  ...
  integer y
  logical year_is_leap_bahai
  integer year_length_days_bahai
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_COMMON, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_COPTIC, the number of
!    days in the year.
!
  implicit none
  ...
  integer y
  logical year_is_leap_coptic
  integer year_length_days_coptic
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_EG_CIVIL, the number of
!    days in the year.
!
  implicit none

  integer y
  integer year_length_days_eg_civil
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_EG_LUNAR, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ENGLISH, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ETHIOPIAN, the number of
!    days in the year.
!
  implicit none
  ...
  integer y
  logical year_is_leap_ethiopian
  integer year_length_days_ethiopian
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_GREEK, the number of
!    days in the year.
!
  implicit none
  ...
  logical year_is_embolismic_greek
  logical year_is_leap_greek
  integer year_length_days_greek
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_GREGORIAN, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_HEBREW, the number of
!    days in the year.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) YEAR_LENGTH_DAYS_HINDU_SOLAR, the number of days
!    in the year.
!
  implicit none
  ...
  year_length_days_hindu_solar = real ( 1577917828, kind = rk ) &
    / real ( 4320000, kind = rk )
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ISLAMIC, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_JULIAN, the number of
!    days in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, real ( kind = rk ) YEAR_LENGTH_DAYS_LUNAR, the number of days
!    in the year.
!
  implicit none
  ...
  integer y
  real ( kind = rk ) year_length_days_lunar
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_PERSIAN, the number of days
!    in the year.
!
  implicit none
  ...
  integer y
  logical year_is_leap_persian
  integer year_length_days_persian
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ENGLISH, the number of days
!    in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_DAYS_ROMAN, the number of days
!    in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, real ( kind = rk ) YEAR_LENGTH_DAYS_SOLAR, the number of days
!    in the year.
!
  implicit none
  ...
  integer y
  integer y2
  real ( kind = rk ) year_length_days_solar
  ...
  if ( y2 < - 4000 ) then
    ...
    year_length_days_solar = &
      ( real ( 2000 - y2, kind = rk ) * 365.2424992D+00 &
      + real ( 4000 + y2, kind = rk ) * 365.2421897D+00 ) &
      /        6000.0D+00
  else
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_ALEXANDRIAN, the
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_BAHAI, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_COMMON, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_COPTIC, the number of
!    months in the year.
!
  implicit none

  integer y
  integer year_length_months_coptic
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_EG_CIVIL, the number
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_EG_LUNAR, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_ENGLISH, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_ETHIOPIAN, the number of
!    months in the year.
!
  implicit none

  integer y
  integer year_length_months_ethiopian
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_GREEK, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_GREGORIAN, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_HEBREW, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_HINDU_LUNAR, the number of
!    months in the year.
!
  implicit none

  integer y
  integer year_length_months_hindu_lunar
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_HINDU_SOLAR, the number of
!    months in the year.
!
  implicit none

  integer y
  integer year_length_months_hindu_solar
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_ISLAMIC, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_JULIAN, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_PERSIAN, the number of
!    months in the year.
!
  implicit none

  integer y
  integer year_length_months_persian
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_REPUBLICAN, the number of
!    months in the year.
!
  implicit none
...
!    Input, integer Y, the year to be checked.
!
!    Output, integer YEAR_LENGTH_MONTHS_ROMAN, the number of
!    months in the year.
!
  implicit none
...
!  Parameters:
!
!    Input/output, integer Y, J, real ( kind = rk ) F, the YJF date.
!
...
!  Parameters:
!
!    Input, integer Y1, J1, real ( kind = rk ) F1,
!    the first YJF date.
!
!    Input, integer Y2, J2, real ( kind = rk ) F2,
!    the second YJF date.
!
...
!    the second YJF date.
!
!    Output, real ( kind = rk ) DAYS, the day difference between the two dates.
!
...
!  Parameters:
!
!    Input/output, integer Y1, J1, real ( kind = rk ) F1,
!    the first date.
!
!    Input/output, integer Y2, J2, real ( kind = rk ) F2,
!    the second date.
!
  implicit none
...
!  Parameters:
!
!    Input, integer Y, J, real ( kind = rk ) F, the YJF date.
!
!    Output, real ( kind = rk ) JED, the Julian Ephemeris Date.
!
  implicit none
...
!  Parameters:
!
!    Input, integer Y1, J1, real ( kind = rk ) F1, the YJF date.
!
!    Output, integer Y2, M2, D2, real ( kind = rk ) F2,
!    the YMDF date.
!
  implicit none
...
!    Input, integer Y1, J1, real ( kind = rk ) F1, the YJF date.
!
!    Output, integer Y2, M2, D2, real ( kind = rk ) F2, the
!    YMDF date.
!
  implicit none
...
subroutine yjf_uniform_common ( y1, j1, f1, y2, j2, f2, y, j, f )
...
!    the second YJF date.
!
!    Output, integer Y, J, real ( kind = rk ) F, the random
!    YJF date.
!
  implicit none
...
!    Input, integer Y, M, the YM date.
!
!    Output, real ( kind = rk ) YF, the Decimal date.
!
  implicit none
  ...
  yf = real ( y, kind = rk ) + real ( 2 * m - 1, kind = rk ) / 24.0D+00
...
!
  f = ( real ( days, kind = rk ) + 0.5D+00 ) &
      / real ( day_max, kind = rk )
...
!    Input, integer Y, M, D, the YMD date.
!
!    Output, real ( kind = rk ) JED, the corresponding JED.
!
  implicit none
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 1401 - g, kind = rk ) - 0.5D+00
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 1401, kind = rk ) - 0.5D+00
...
!  Parameters:
!
!    Input/output, integer Y, M, D, real ( kind = rk ) F, the
...
!  Parameters:
!
!    Input, integer Y1, M1, D1, real ( kind = rk ) F1, the
!    first YMDF date.
!
!    Input, integer Y2, M2, D2, real ( kind = rk ) F2, the
!    second YMDF date.
!
!    Output, character CMP:
!    '<' if date 1 precedes date 2;
...
!  Parameters:
!
!    Input, integer Y1, M1, D1, real ( kind = rk ) F1,
!    the first YMDF date.
!
!    Input, integer Y2, M2, D2, real ( kind = rk ) F2,
!    the second YMDF date.
!
!    Output, real ( kind = rk ) DAYS, the number of days between the two dates.
!
!    Output, integer IERROR, is 1 if either date is illegal,
!    0 otherwise.
!
...
!    second YMDF date.
!
!    Output, integer YN, MN, DN, real ( kind = rk ) FN,
...
!    YMDF date.
!
!    Input, real ( kind = rk ) DAYS, the number of days to advance the date.
!
!    Output, integer Y2, M2, D2, real ( kind = rk ) F2, the
...
!  Parameters:
!
!    Input, integer Y, M, D, real ( kind = rk ) F, the YMDF date.
!
!    Output, real ( kind = rk ) JED, the corresponding Julian Ephemeris Date.
!
  implicit none
...
!
  jed = real ( &
    ( 1461 * y_prime ) / 4 + 30 * m_prime + d_prime - 124, kind = rk ) - 0.5D+00
  jed = jed + f
...
!  Convert the computational date to a JED.
!
  jed = real ( 365 * y_prime + 30 * m_prime + d_prime - 317, kind = rk ) &
    - 0.5D+00
  jed = jed + f
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 1412 - g, kind = rk ) - 0.5D+00
  jed = jed + f
...
!  Convert the computational date to a JED.
!
  jed = real ( 365 * y_prime + 30 * m_prime + d_prime - 47 + 1, kind = rk ) &
    - 0.5D+00
  jed = jed + f
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 124, kind = rk ) - 0.5D+00
  jed = jed + f
...
!  Input:
!
!    integer Y, M, D, real ( kind = rk ) F, the YMDF date.
!
!  Output:
!
!    real ( kind = rk ) JED, the corresponding JED.
!
  implicit none
...
!
  do m2 = 1, m - 1
    jed = jed + real ( month_length_hebrew ( y, m2 ), kind = rk )
  end do
!
!  Add on the days.
!
  jed = jed + real ( d - 1, kind = rk )
  jed = jed + f
...
!
  ...
  jed = jed_epoch + &
      real ( d - 1, kind = rk ) &
    + real ( m - 1, kind = rk ) * month_length_hindu_solar ( ) &
    + real ( y, kind = rk ) * year_length_days_hindu_solar ( )
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 7665, kind = rk ) - 0.5D+00
  jed = jed + f
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 7664, kind = rk ) - 0.5D+00
  jed = jed + f
...
!
  jed = real ( ( ( 1461 * ( y3 + 4715 ) ) / 4 ) - 1095 &
    + days_before_month_julian ( y2, m2 ) + d2 - 1, kind = rk ) - 0.5D+00
  jed = jed + f
...
!  Convert the computational date to a JED.
!
  jed = real ( 365 * y_prime + 30 * m_prime + d_prime - 77, kind = rk ) - 0.5D+00
  jed = jed + f
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 111 - g, kind = rk ) - 0.5D+00
  jed = jed + f
...
!
  ...
  jed = real ( j1 + j2 + d_prime - 1348 - g, kind = rk ) - 0.5D+00
  jed = jed + f
...
!
  ...
  integer i4_modp
  integer m
  integer, save, dimension ( 12 ) :: m_table = &
...
!
  ...
  days = d + int ( 2.6D+00 * real ( mm, kind = rk ) - 0.2D+00 ) &
    - 2 * c + yy + ( yy / 4 ) + ( c / 4 )
...
!    the YMDF date.
!
!    Output, integer Y2, J2, real ( kind = rk ) F2, YJF date.
!
  implicit none
...
!    the YMDF date.
!
!    Output, integer Y2, J2, real ( kind = rk ) F2, the YJF date.
!
  implicit none
...
subroutine ymdf_uniform_common ( y1, m1, d1, f1, y2, m2, d2, f2, &
  y, m, d, f )
...
subroutine ymdf_uniform_english ( y1, m1, d1, f1, y2, m2, d2, f2, &
  y, m, d, f )
...
!    the second YMDF date.
!
!    Output, integer Y, M, D, real ( kind = rk ) F, the
!    random YMDF date.
!
  implicit none
...
!
!    Output, integer IERROR, is 1 if either date was illegal, and
!    0 otherwise.
!
  implicit none
...
!    Input, integer Y2, M2, D2, H2, N2, S2, the second date.
!
!    Output, integer NDAY, NHOUR, NMINUTE, NSECOND,
...
!
  f =       real ( s, kind = rk )      / 60.0D+00
  f = ( f + real ( n, kind = rk )    ) / 60.0D+00
  f = ( f + real ( h, kind = rk )    ) / 24.0D+00
  f = ( f + real ( days, kind = rk ) ) / real ( day_max, kind = rk )
...
!
  ...
  integer d
  integer, parameter :: dhi = 1
  integer, parameter :: dlo = 1
  integer h
  integer, parameter :: hhi = 0
  integer, parameter :: hlo = 0
  integer i
  integer m
  integer, parameter :: mhi = 1
  integer, parameter :: mlo = 1
  integer n
  integer, parameter :: nhi = 0
  integer, parameter :: nlo = 0
  character ( len = 22 ) s
  integer ss
  integer, parameter :: shi = 0
  integer, parameter :: slo = 0
  integer y
  real ( kind = rk ) yf
  integer, parameter :: yhi = 1970
  integer, parameter :: ylo = 1960
...
!
  f2 = real ( ( ( h1 * 60 + n1 ) * 60 + s1 ), kind = rk ) &
    / real ( 24 * 60 * 60, kind = rk )
