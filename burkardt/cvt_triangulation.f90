...
!
  ...
  real ( kind = rk ) :: boundary_h = 0.05D+00
  integer iteration
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: point_new
  real ( kind = rk ), allocatable, dimension ( :, : ) :: sample
    ...
    do j = 1, n
      if ( count(j) /= 0 ) then
        point_new(1:dim_num,j) = point_new(1:dim_num,j) &
          / real ( count(j), kind = rk )
      end if
    end do

    energy = energy / real ( sample_num, kind = rk )
...
!
  ...
  real ( kind = rk ) energy
  character ( len = 255 ) file_txt_name
  real ( kind = rk ), allocatable, dimension ( :, : ) :: fixed
  integer fixed_num
  integer iteration
...
!
  ...
  character ( len = 255 ) file_txt_name
  real ( kind = rk ) :: h
  real ( kind = rk ), allocatable, dimension ( : ) :: hi
  integer iteration
  integer, parameter :: iteration_max = 40
  integer j
  real ( kind = rk ), allocatable, dimension ( : ) :: lo
  integer, parameter :: n = 400
  integer, allocatable, dimension ( : ) :: nearest
  real ( kind = rk ), allocatable, dimension ( :, : ) :: point
  real ( kind = rk ), allocatable, dimension ( :, : ) :: point_new
...
!
  ...
  real ( kind = rk ) area_box
  real ( kind = rk ) area_hex_unit
  real ( kind = rk ) :: boundary_h = 0.05D+00
  logical comment
  integer, allocatable, dimension ( : ) :: count
  integer, parameter :: dim_num = 2
...
!
  ...
  h = ( 2.0D+00 / sqrt ( 3.0D+00 ) ) &
    * sqrt ( area_box / ( real ( n_box, kind = rk ) * area_hex_unit ) )
...
!
!    Output, real ( kind = rk ) POINT(DIM_NUM,N), the coordinates
!    of the points.
!
  implicit none
  ...
  real ( kind = rk ) point2(dim_num,batch)
  integer seed
  real ( kind = rk ) test(batch)
  real ( kind = rk ) weight(batch)
  real ( kind = rk ), parameter :: x1 =  0.0D+00
  real ( kind = rk ), parameter :: x2 = +1.0D+00
  real ( kind = rk ), parameter :: y1 =  0.0D+00
  real ( kind = rk ), parameter :: y2 = +1.0D+00
...
!
!    Output, real ( kind = rk ) W(N), the weight associated with each point.
!
  implicit none
...
!
!    Input, real ( kind = rk ) POINT(DIM_NUM,N), the point coordinates.
!
!    Input, real ( kind = rk ) SAMPLE(DIM_NUM,SAMPLE_NUM), the sample
!    coordinates.
!
...
!
  ...
  real ( kind = rk ) point(dim_num,n)
  real ( kind = rk ) sample(dim_num,sample_num)
...
!    calculation.
!
!    Input, real ( kind = rk ) ENERGY, the estimated Voronoi energy.
!
!    Input, real ( kind = rk ) CELL_GENERATOR(DIM_NUM,N), the points.
!
