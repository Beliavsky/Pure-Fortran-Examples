...
!
!    Input, real ( kind = rk ) NODE_X(NODE_NUM), the nodes.  
!
...
!
!    Input, real ( kind = rk ) VALUE(VALUE_DIM,NODE_NUM), the FEM values.
!
...
!
!    Output, real ( kind = rk ) SAMPLE_VALUE(VALUE_DIM,SAMPLE_NODE_NUM),
...
!
  ...
  real ( kind = rk ) sample_value(value_dim,sample_node_num)
  real ( kind = rk ) value(value_dim,node_num)

  call i4_fake_use ( element_num )
!
...
!
!    Input, real ( kind = rk ) T(NT), an array that has been sorted
!    into ascending order.
!
!    Input, integer NS, the number of points to be bracketed.
!
!    Input, real ( kind = rk ) S(NS), values to be bracketed by entries of T.
!
