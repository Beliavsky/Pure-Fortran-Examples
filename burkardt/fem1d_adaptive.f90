...
!
!    integer KOUNT, the number of adaptive steps that have 
!    been taken.
!
!    integer M.
...
!
!    integer NMAX, the maximum number of unknowns that can be
!    handled.
!
!    integer NODE(NL,N).
...
!    and XN(N) is XR.
!
!    real ( kind = rk ) XQUAD(NQUAD,NMAX), the I-th quadrature point
!    in interval J.
!
!    real ( kind = rk ) XQUADY(NQUAD,NMAY ), the I-th quadrature point
...
!    refer to an "NU+1"-th unknown.
!
!    Output, real ( kind = rk ) F(NU).
!    ASSEMBLE stores into F the right hand side of the linear
!    equations.
!    SOLVE replaces those values of F by the solution of the
!    linear equations.
!
!    Input, real ( kind = rk ) H(N)
!    H(I) is the length of subinterval I.  This code uses
!    equal spacing for all the subintervals.
!
!    Input, integer N
...
!    The number of quadrature points used in a subinterval.
!
!    Input, integer NMAX, the maximum number of unknowns that 
!    can be handled.
!
...
!    to have at X = XR.
!
!    Input, real ( kind = rk ) WQUAD(NQUAD).
!    WQUAD(I) is the weight associated with the I-th point
!    of an NQUAD point Gaussian quadrature rule.
!
...
!    and XN(N) is XR.
!
!    Input, real ( kind = rk ) XQUAD(NQUAD,NMAX), the I-th quadrature point
!    in interval J.
!
  implicit none
  ...
  real ( kind = rk ) phij
  real ( kind = rk ) phijx
  real ( kind = rk ), external :: pp
  real ( kind = rk ), external :: qq
  real ( kind = rk ) ul
  real ( kind = rk ) ur
  real ( kind = rk ) wquad(nquad)
  real ( kind = rk ) wquade
...
!    of basis functions.
!
!    Output, real ( kind = rk ) WQUAD(NQUAD).
!    WQUAD(I) is the weight associated with the I-th point
!    of an NQUAD point Gaussian quadrature rule.
!
!    Input, real ( kind = rk ) XN(0:N).
...
!    and XN(N) is XR.
!
!    Output, real ( kind = rk ) XQUAD(NQUAD,NMAX), the I-th quadrature point
!    in interval J.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) BETA, the value of BETA.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the evaluation point.
!
!    Output, real ( kind = rk ) PHII, PHIIX, the value of the basis function
!    and its derivative.
!
!    Input, real ( kind = rk ) XLEFT, XRITE, the endpoints of the interval.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) ADIAG(NU).
!    ADIAG(I) is the "diagonal" coefficient of the I-th
!    equation in the linear system.  That is, ADIAG(I) is
!    the coefficient of the I-th unknown in the I-th equation.
!
!    Input, real ( kind = rk ) ALEFT(NU).
!    ALEFT(I) is the "left hand" coefficient of the I-th
!    equation in the linear system.  That is, ALEFT(I) is the
!    coefficient of the (I-1)-th unknown in the I-th equation.
!    There is no value in ALEFT(1), since the first equation
...
!    in ARITE(NU).
!
!    Input/output, real ( kind = rk ) F(NU).
...
!       at the right endpoint U' has the value UR.
!
!    Input, integer KOUNT, the number of adaptive steps that 
!    have been taken.
!
!    Input, integer N
!    The number of subintervals into which the interval
...
!  Local parameters:
!
!    Local, real ( kind = rk ) ADIAG(NU).
!    ADIAG(I) is the "diagonal" coefficient of the I-th
!    equation in the linear system.  That is, ADIAG(I) is
!    the coefficient of the I-th unknown in the I-th equation.
!
!    Local, real ( kind = rk ) ALEFT(NU).
!    ALEFT(I) is the "left hand" coefficient of the I-th
!    equation in the linear system.  That is, ALEFT(I) is the
!    coefficient of the (I-1)-th unknown in the I-th equation.
!    There is no value in ALEFT(1), since the first equation
...
!    The number of quadrature points used in a subinterval.
!
!    Local, real ( kind = rk ) WQUAD(NQUAD).
!    WQUAD(I) is the weight associated with the I-th point
!    of an NQUAD point Gaussian quadrature rule.
!
!    Local, real ( kind = rk ) XQUAD(NQUAD,NMAX), the I-th quadrature point
!    in interval J.
!
  implicit none
  ...
  real ( kind = rk ) adiag(nmax)
  real ( kind = rk ) aleft(nmax)
  real ( kind = rk ) arite(nmax)
  real ( kind = rk ) f(nmax)
  real ( kind = rk ) h(n)
  integer ibc
  integer indx(0:n)
...
!
  ...
  real ( kind = rk ) adiag(nmay)
  real ( kind = rk ) aleft(nmay)
  real ( kind = rk ) arite(nmay)
  real ( kind = rk ) eta(n)
  real ( kind = rk ) f(nu)
  real ( kind = rk ) fy(nmay)
  real ( kind = rk ) h(n)
  real ( kind = rk ) hy(nmay)
  integer i
  integer ibcy
  integer indy(0:nmay)
  integer j
  ...
  real ( kind = rk ) wquad(nquad)
  real ( kind = rk ) xn(0:n)
  real ( kind = rk ) xquady(nquad,nmay)
  real ( kind = rk ) y
  real ( kind = rk ) yl
  real ( kind = rk ) ym
  real ( kind = rk ) yn(0:nmay)
  real ( kind = rk ) yr
!
...
!
    ...
    do i = 0, ny
      yn(i) = ( real ( ny - i, kind = rk ) * yl   &
              + real (      i, kind = rk ) * ym ) &
              / real ( ny,     kind = rk )
    end do

    do i = ny + 1, m
      yn(i) = ( real ( m - i,      kind = rk ) * ym   &
              + real (     i - ny, kind = rk ) * yr ) &
              / real ( m -     ny, kind = rk )
    end do
!
...
!
    if ( 1 <= j ) then
      ...
      do i = 1, ny
        ...
        ulval = ( real ( ny - i + 1, kind = rk ) * uleft   &
                + real (      i - 1, kind = rk ) * urite ) &
                / real ( ny,         kind = rk )

        urval = ( real ( ny - i, kind = rk ) * uleft   &
                + real (      i, kind = rk ) * urite ) &
                / real ( ny,     kind = rk )
!
...
!
    if ( j <= n - 1 ) then
      ...
      do i = m+1-ny, m
        ...
        ulval = ( real (      m - i + 1, kind = rk ) * uleft   &
                + real ( ny - m + i - 1, kind = rk ) * urite ) &
                / real ( ny,             kind = rk )

        urval = ( real (      m - i, kind = rk ) * uleft   &
                + real ( ny - m + i, kind = rk ) * urite ) &
                / real ( ny,         kind = rk )
!
!  Compute the integral of
!
...
!    and the problem solved again.
!
!    Input/output, real ( kind = rk ) XN(0:N).
!    XN(I) is the location of the I-th node.  XN(0) is XL,
!    and XN(N) is XR.
!
