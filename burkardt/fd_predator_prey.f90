...
!
  ...
  integer j
  integer length
  real ( kind = rk ) pred_init
  real ( kind = rk ) prey_init
  integer step_num
  character ( len = 255 ) string
  real ( kind = rk ), allocatable :: trf(:,:)
...
!
  ...
  t_start = 0.0D+00
  t_stop =  5.0D+00
  dt = ( t_stop - t_start ) / real ( step_num, kind = rk )
...
!
  ...
  do j = 1, step_num
    trf(1,j+1) = trf(1,j) + dt
    trf(2,j+1) = trf(2,j) + dt &
      * (    2.0D+00 * trf(2,j) - 0.001D+00 * trf(2,j) * trf(3,j) )
    trf(3,j+1) = trf(3,j) + dt &
      * ( - 10.0D+00 * trf(3,j) + 0.002D+00 * trf(2,j) * trf(3,j) )
  end do
!
!  Write data file.
!
  call get_unit ( data_unit )
  data_filename = 'fd_predator_prey_data.txt'
  open ( unit = data_unit, file = data_filename, status = 'replace' )
  do j = 1, step_num + 1
    write ( data_unit, '(f10.4,2x,f10.4,2x,f10.4)' ) trf(1:3,j)
  end do
...
!
  ...
  command_filename = 'fd_predator_prey_commands.txt'
  open ( unit = command_unit, file = command_filename, status = 'replace' )

  write ( command_unit, '(a)' ) 'set term png'
  write ( command_unit, '(a)' ) 'set output "fd_predator_prey.png"'
  ...
  write ( command_unit, '(a)' ) 'set title "Predator/Prey Populations"'
  write ( command_unit, '(a,i4,a)' ) 'plot "' // trim ( data_filename ) &
    // '" using 1:2 with lines lw 3 title "prey", \'
  write ( command_unit, '(a)' ) '"" using 1:3 with lines lw 3 title "predator"'
