...
!
!    Output, integer ICC(NCC), the CCS rows.
!
!    Output, integer CCC(N+1), the compressed CCS columns.
!
!    Output, real ( kind = rk ) ACC(NCC), the CCS values.
!
  implicit none
...
!    Input, character ( len = * ) PREFIX, a common prefix for the filenames.
!
!    Output, integer NCC, the number of CCS elements.
!
...
!
!    Input, integer NCC, the number of CCS values.
!
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) X(N), the data.
!
  implicit none
