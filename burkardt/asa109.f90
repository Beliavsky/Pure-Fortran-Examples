...
!  Input:
!
!    real ( kind = rk ) P, Q, the parameters of the incomplete
!    Beta function.
!
!    real ( kind = rk ) BETA, the logarithm of the value of
!    the complete Beta function.
!
!    real ( kind = rk ) ALPHA, the value of the incomplete Beta
!    function.  0 <= ALPHA <= 1.
!
!  Output:
!
...
!    nonzero, an error occurred.
!
!    real ( kind = rk ) XINBTA, the argument of the incomplete
!    Beta function which produces the value ALPHA.
!
!  Local:
!
!    real ( kind = rk ) SAE, requests an accuracy of about 10^SAE.
!
  implicit none
  ...
  real ( kind = rk ) r
  real ( kind = rk ) s
  real ( kind = rk ), parameter :: sae = -30.0D+00
  real ( kind = rk ) sq
  real ( kind = rk ) t
  real ( kind = rk ) tx
  real ( kind = rk ) w
