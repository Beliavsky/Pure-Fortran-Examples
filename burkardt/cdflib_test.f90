...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) ccdf_compute
  real ( kind = rk ) ccdf_lookup
  real ( kind = rk ) cdf_compute
  real ( kind = rk ) cdf_lookup
  integer ierror
  integer n_data
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!
  ...
  real ( kind = rk ) ompr
  integer s
  real ( kind = rk ) s_double
  real ( kind = rk ) pr
  integer x
  real ( kind = rk ) x_double
  ...
  do
    ...
    s_double = real ( s, kind = rk )
    x_double = real ( x, kind = rk )
...
!
  ...
  real ( kind = rk ) cdf_lookup
  integer dfd
  real ( kind = rk ) dfd_double
  integer dfn
  real ( kind = rk ) dfn_double
  integer n_data
  real ( kind = rk ) x
  ...
  do
    ...
    dfn_double = real ( dfn, kind = rk )
    dfd_double = real ( dfd, kind = rk )
...
!
  ...
  real ( kind = rk ) erf_compute
  real ( kind = rk ) erf_lookup
  real ( kind = rk ) erfc_compute
  real ( kind = rk ) erfc_lookup
...
!
  ...
  real ( kind = rk ) gamma
  real ( kind = rk ) gamma_compute
  real ( kind = rk ) gamma_lookup
  integer n_data
  real ( kind = rk ) x
...
!
  ...
  real ( kind = rk ) psi
  real ( kind = rk ) psi_compute
  real ( kind = rk ) psi_lookup
  integer n_data
  real ( kind = rk ) x
