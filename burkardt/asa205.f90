...
!  Local:
!
!    Local, integer BOUND(M_MAX,M_MAX).  BOUND(I,J) is the
!    current upper bound on TABLE2(I,J) to satisfy row and column totals.
!
!    Local, integer M_MAX, the maximum dimension of TABLE2.
!
!    Local, integer MULT2(M_MAX), MULT2(I) is the maximum number 
!    of equivalent tables given the first I rows.
!
!    Local, integer NTOTAL, the total of the table entries.
!
!    Local, real ( kind = rk ) PROB2(M_MAX,M_MAX), the partial sum of terms 
!    in log(p).
!
!    Local, integer REPS(M_MAX), the number of previous rows equal 
!    to row I.
!
!    Local, logical REPT(M_MAX), is true if row totals ROWSUM(I) and N(I-1) 
!    are equal.
!
!    Local, integer TABLE2(M_MAX,M_MAX), the current table.
!
!    Local, integer TOTAL_MAX, the maximum number of observations.
!
!    Local, integer Z(M_MAX), the lower bound on the sum of 
...
!
  ...
  integer n
  integer, parameter :: total_max = 201

  integer bound(m_max,m_max)
  integer colsum(n)
  integer colsum2(m_max)
  external eval
  real ( kind = rk ) factlm(total_max+1)
  real ( kind = rk ) flogm(total_max+1)
  integer i
  logical ieqim
  integer ifault
  integer iflag
  ...
  real ( kind = rk ) prob
  real ( kind = rk ) prob0
  real ( kind = rk ) prob2(m_max,m_max)
  integer reps(m_max)
  integer repsc
  integer repsr
  logical rept(m_max)
  ...
  integer rsum
  integer rowsum(m)
  integer rowsum2(m_max)
  integer table(m,n)
  integer table2(m_max,m_max)
  integer z(m_max)
!
