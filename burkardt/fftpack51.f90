...
!
!    Input/output, complex ( kind = rk ) C(LENC) containing the sequence to be 
!    transformed.
!
...
!
  ...
  else if ( lensav < 2 * n + int ( log ( real ( n, kind = rk ) ) &
    / log ( 2.0D+00 ) ) + 4 ) then
    ier = 2
...
!    array C, of two consecutive elements within the sequence to be transformed.
!
!    Input/output, complex ( kind = rk ) C(LENC) containing the sequence to 
!    be transformed.
!
!    Input, integer LENC, the dimension of the C array.  
!    LENC must be at least INC*(N-1) + 1.
...
!    2, input parameter LENSAV not big enough.
  ...
  if (lensav < 2*n + int(log( real ( n, kind = rk ) )/log( 2.0D+00 )) + 4) then
    ier = 2
...
!
!    Input/output, complex ( kind = rk ) C(LDIM,M), on intput, the array of 
...
!
  ...
  if ( ldim < l ) then
    ...
    return
  else if (lensav < 2*l + int(log( real ( l, kind = rk ))/log( 2.0D+00 )) + &
    2*m + int(log( real ( m, kind = rk ))/log( 2.0D+00 )) +8) then
    ier = 2
...
!
  iw = 2*l+int(log( real ( l, kind = rk ) )/log( 2.0D+00 )) + 3

  call cfftmb(l, 1, m, ldim, c, (l-1) + ldim*(m-1) +1, &
    wsave(iw), 2*m + int(log( real ( m, kind = rk ))/log( 2.0D+00 )) + 4, &
    work, 2*l*m, ier1)
...
!
  ...
  call cfftmb (m, ldim, l, 1, c, (m-1)*ldim + l, &
    wsave(iw), 2*l + int(log( real ( l, kind = rk ) )/log( 2.0D+00 )) + 4, &
    work, 2*m*l, ier1)
...
!
!    Input/output, complex ( kind = rk ) C(LDIM,M), on input, the array of two 
...
!
  ...
  if ( ldim < l ) then
    ...
    2*l + int(log( real ( l, kind = rk ))/log( 2.0D+00 )) + &
    2*m + int(log( real ( m, kind = rk ))/log( 2.0D+00 )) +8) then
    ier = 2
...
!
  ...
  if ( lensav < 2 * l + int ( log ( real ( l, kind = rk ) ) &
    / log ( 2.0D+00 ) ) + 2 * m + int ( log ( real ( m, kind = rk ) ) &
    / log ( 2.0D+00 ) ) + 8 ) then
    ier = 2
  ...
  call cfftmi ( l, wsave(1), 2 * l + int ( log ( real ( l, kind = rk ) ) &
    / log ( 2.0D+00 ) ) + 4, ier1 )
  ...
  call cfftmi ( m, &
    wsave(2*l+int(log( real ( l, kind = rk ) )/log( 2.0D+00 )) + 3), &
    2*m + int(log( real ( m, kind = rk ) )/log( 2.0D+00 )) + 4, ier1)
...
!    transformed. 
!
!    Input/output, complex ( kind = rk ) C(LENC), an array containing LOT 
...
!    transformed.
!
!    Input/output, complex ( kind = rk ) C(LENC), array containing LOT sequences,
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    2*n + int(log( real ( n, kind = rk ) )/log( 2.0D+00 )) +4) then
    ier = 2
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real ( kind = rk ) WSAVE(LENSAV), containing the prime factors of N
...
!
  ...
  lnsv = n + int(log( real ( n, kind = rk ) )/log( 2.0D+00 )) +4
...
!
!    Input/output, real ( kind = rk ) R(LENR), array containing LOT sequences, 
...
!    in array R, of two consecutive elements within the sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), containing the sequence to 
!     be transformed.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1)+ 1.
...
!
  ...
  lnsv = nm1 + int(log( real ( nm1, kind = rk ) )/log( 2.0D+00 )) +4
...
!
  ...
  124 lenx = inc*(nm1-1)  + 1
   ...
   30 fnm1s4 = real ( nm1, kind = rk ) / 4.0D+00
...
!
!    Output, real ( kind = rk ) FAC(15), factorization information.  FAC(1) is 
...
!
 ...
 102  srt3s2 = sqrt( 3.0D+00 )/ 2.0D+00 
  ...
  124 do m=1,lot
      ...
      lnsv = np1 + int(log( real ( np1, kind = rk ))/log( 2.0D+00 )) + 4
      lnwk = lot*np1
...
!
 ...
 102  ssqrt3 = 1.0D+00 / sqrt ( 3.0D+00 )
  ...
  127 continue 
      ...
      xh(m,np1) = xh(m,np1)+xh(m,np1)
   20 continue
   30 sfnp1 = 1.0D+00 / real ( np1, kind = rk )
      m1 = 0
      do 125 m=1,lj,jump
         m1 = m1+1
...
!
!    Input, real ( kind = rk ) R(LDR,M), the copied information.
!
!    Output, real (  kind = rk ) W(LDW,M), the original information.
!
  implicit none
  ...
  integer l
  real ( kind = rk ) r(ldr,m)
  real ( kind = rk ) w(ldw,m)
...
!
  ...
  else if (lensav < n + int(log( real ( n, kind = rk ) )/log( 2.0D+00 )) +4) then
    ier = 2
...
!
  ...
  if (lensav < n + int(log( real ( n, kind = rk ) )/log( 2.0D+00 )) +4) then
    ier = 2
...
!    is most efficient when M is a product of small primes.
!
!    Input/output, real ( kind = rk ) R(LDIM,M), the real array of two 
...
!    + INT(LOG(REAL(M))) + 8.
!
!    Workspace, real ( kind = rk ) WORK(LENWRK).  WORK provides workspace, and 
...
!  verify lensav
!
  lwsav =   l+int(log( real ( l, kind = rk ) )/log( 2.0D+00 ))+4
  mwsav =   2*m+int(log( real ( m, kind = rk ) )/log( 2.0D+00 ))+4
  mmsav =   m+int(log( real ( m, kind = rk ) )/log( 2.0D+00 ))+4
  modl = mod(l,2)
  modm = mod(m,2)
...
!
      ...
      call rfftmb(m,ldim,l,1,r,m*ldim,wsave(1), &
           l+int(log( real ( l, kind = rk ) )/log( 2.0D+00 ))+4,work,lenwrk,ier1)
...
!    + INT(LOG(REAL(M))) + 8. 
!
!    Workspace, real ( kind = rk ) WORK(LENWRK), provides workspace, and its 
...
!    array R, of two consecutive elements within the same sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), real array containing LOT 
...
!    LENSAV must be at least N + INT(LOG(REAL(N))) + 4.
!
!    Output, real ( kind = rk ) WSAVE(LENSAV), work array containing the prime 
...
!    array R, of two consecutive elements within the sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), on input, the sequence to be 
...
!
  ...
  real ( kind = rk ) wsave(lensav)
      ...
      if (lensav < 2*n + int(log( real ( n, kind = rk ) ) &
        /log( 2.0D+00 )) +4) then
        ier = 2
...
!    array R, of two consecutive elements within the same sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), containing LOT sequences, each 
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    return
  else if ( lensav < n / 2 + n + int ( log ( real ( n, kind = rk ) ) &
    / log ( 2.0D+00 ) ) + 4 ) then
    ier = 2
...
!
  ...
  if (lensav < n/2 + n + int(log( real ( n, kind = rk ) ) &
    /log( 2.0D+00 )) +4) then
    ier = 2
...
!
 ...
 30   fnp1s4 = real ( np1, kind = rk ) / 4.0D+00
         x(1,1) = fnp1s4*xh(1)
         dsum = x(1,1)
      do i=3,n,2
            x(1,i-1) = fnp1s4*xh(i)
...
!    of information to be copied.
!
!    Output, real ( kind = rk ) R(LDR,M), the copied information.
!
!    Input, real (  kind = rk ) W(LDW,M), the original information.
!
  implicit none
