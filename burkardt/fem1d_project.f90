...
!*****************************************************************************80
!
!! FEM1D_PROJECT uses projection on finite element method data.
!
!  Discussion:
!
...
!
  ...
  real ( kind = rk ), allocatable :: fem_node_x(:,:)
  real ( kind = rk ), allocatable :: fem_value(:,:)
  ...
  real ( kind = rk ), allocatable :: sample_node_x(:,:)
  integer sample_value_dim
  integer sample_value_num
  real ( kind = rk ), allocatable :: sample_value(:,:)
...
!
!    Input, real ( kind = rk ) SAMPLE_NODE_X(SAMPLE_NODE_NUM), the sample nodes.
!
!    Input, real ( kind = rk ) SAMPLE_VALUE(VALUE_DIM,SAMPLE_NODE_NUM),
!    the values at sample nodes.
!
!    Input, integer FEM_NODE_NUM, the number of FEM nodes.
!
!    Input, real ( kind = rk ) FEM_NODE_X(FEM_NODE_NUM), the FEM nodes.
!
...
!
!    Output, real ( kind = rk ) FEM_VALUE(FEM_VALUE_DIM,FEM_VALUE_NUM),
!    the FEM values.
!
  implicit none
  ...
  integer fem_value_dim
  integer fem_value_num
  integer, parameter :: quad_num = 2
  integer sample_node_num
  integer sample_value_dim

  real ( kind = rk ) a(3,fem_node_num)
  real ( kind = rk ) a1
  real ( kind = rk ) b(fem_node_num,fem_value_dim)
  real ( kind = rk ) b1
  integer dim
  integer fem_element_num
  integer fem_element_order
  real ( kind = rk ) fem_node_x(fem_node_num)
  real ( kind = rk ) fem_value(fem_value_dim,fem_value_num)
  integer i
  real ( kind = rk ) integral
  integer l
  integer phi_num
  real ( kind = rk ) phi_v(3)
  real ( kind = rk ) phi_x(3)
  real ( kind = rk ) phil
  real ( kind = rk ) phir
  integer quad
  real ( kind = rk ) :: quad_x(quad_num) = (/ &
    -0.577350269189625764509148780502D+00, &
     0.577350269189625764509148780502D+00 /)
  real ( kind = rk ) :: quad_w(quad_num) = (/ 1.0D+00, 1.0D+00 /)
  integer r
  real ( kind = rk ) sample_node_x(sample_node_num)
  real ( kind = rk ) sample_value(sample_value_dim,sample_node_num)
  real ( kind = rk ) wq
  real ( kind = rk ) x(fem_value_num,fem_value_dim)
  real ( kind = rk ) xl
  real ( kind = rk ) xq
  real ( kind = rk ) xr

  call i4_fake_use ( fem_element_num )
  call i4_fake_use ( fem_element_order )
!
!  Set up the matrix A.
!
...
!
!    Output, real ( kind = rk ) QUAD, the integral of F(X) * G(X)
!    from A to B.
!
  implicit none
