...
!*****************************************************************************80
!
!! fem2d_poisson_rectangle() solves the Poisson equation on a rectangle.
!
!  Discussion:
!
!    This program solves
...
!  Local:
!
!    real ( kind = rk ) A(3*IB+1,NUNK), the coefficient matrix.
!
!    real ( kind = rk ) EH1, the H1 seminorm error.
!
!    real ( kind = rk ) EL2, the L2 error.
!
!    real ( kind = rk ) ELEMENT_AREA(ELEMENT_NUM), the area of elements.
!
...
!
!    real ( kind = rk ) F(NUNK), the right hand side.
!
!    integer IB, the half-bandwidth of the matrix.
!
!    integer INDX(NODE_NUM), gives the index of the
!    unknown quantity associated with the given node.
!
!    integer NNODES, the number of nodes used to form
!    one element.
!
!    integer NQ, the number of quadrature points used
!    for assembly.
!
!    integer NQE, the number of quadrature points used
!    for error estimation.
!
!    integer NUNK, the number of unknowns.
!
!    integer NX, the number of points in the X direction.
!
!    integer NY, the number of points in the Y direction.
!
!    real ( kind = rk ) WQ(NQ), quadrature weights.
!
!    real ( kind = rk ) NODE_XY(2,NODE_NUM), the
!    coordinates of nodes.
!
!    real ( kind = rk ) XL, XR, YB, YT, the X coordinates of
!    the left and right sides of the rectangle, and the Y coordinates
!    of the bottom and top of the rectangle.
!
!    real ( kind = rk ) XQ(NQ,ELEMENT_NUM), YQ(NQ,ELEMENT_NUM), the
...
!
  ...
  integer, dimension(nnodes,element_num) :: element_node
  real ( kind = rk ), allocatable, dimension (:) :: f
  integer ib
  integer ierr
  integer, dimension(node_num) :: indx
  integer job
  ...
  real ( kind = rk ), dimension(nq) :: wq
  real ( kind = rk ), parameter :: xl = 0.0D+00
  real ( kind = rk ), dimension(nq,element_num) :: xq
  real ( kind = rk ), parameter :: xr = 1.0D+00
  real ( kind = rk ), parameter :: yb = 0.0D+00
  real ( kind = rk ), dimension(nq,element_num) :: yq
...
!
!    Input, integer NNODES, the number of nodes used to form
!    one element.
!
!    Input, integer ELEMENT_NUM, the number of elements.
!
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Input, integer NQ, the number of quadrature points used 
!    in assembly.
!
!    Input, real ( kind = rk ) WQ(NQ), quadrature weights.
!
!    Input, real ( kind = rk ) XQ(NQ,ELEMENT_NUM), YQ(NQ,ELEMENT_NUM), the
!    coordinates of the quadrature points in each element.
!
!    Input, real ( kind = rk ) ELEMENT_AREA(ELEMENT_NUM), the area of elements.
!
!    Input, integer INDX(NODE_NUM), gives the index of the 
!    unknown quantity associated with the given node.
!
...
!
!    Output, real ( kind = rk ) A(3*IB+1,NUNK), the NUNK by NUNK
!    coefficient matrix, stored in a compressed format.
!
!    Output, real ( kind = rk ) F(NUNK), the right hand side.
!
!  Local parameters:
!
...
!
  ...
  real ( kind = rk ), dimension(3*ib+1,nunk) :: a
  real ( kind = rk ) aij
  integer basis
  real ( kind = rk ) bi
  real ( kind = rk ) bj
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, integer INDX(NODE_NUM), the index of the unknown in the
...
!    Input, integer NUNK, the number of unknowns.
!
!    Input/output, real ( kind = rk ) A(3*IB+1,NUNK), the NUNK by NUNK
!    coefficient matrix, stored in a compressed format.  On output,
!    A has been adjusted for boundary conditions.
!
!    Input/output, real ( kind = rk ) F(NUNK), the right hand side.
...
!    finite element linear system.
!
!    Input, integer NUNK, the number of unknowns in the finite
!    element system.
!
!    Input, real ( kind = rk ) F(NUNK), the solution vector of the finite
!    element system.
!
  implicit none
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Input, integer INDX(NODE_NUM), the index of the unknown
!    quantity associated with the given node.
!
!    Input, real ( kind = rk ) NODE_XY(2,NODE_NUM), the X and Y
!    coordinates of nodes.
!
!    Input, real ( kind = rk ) F(NUNK), the coefficients of the solution.
!
!    Input, integer ELEMENT_NUM, the number of elements.
!
!    Input, integer NNODES, the number of nodes used to form
!    one element.
!
!    Input, integer NQE, the number of points in the quadrature
!    rule.  This is actually fixed at 13.
!
!    Input, integer NUNK, the number of unknowns.
!
!    Input, integer NODE_NUM, the number of nodes.
...
!    derivatives evaluated at a particular quadrature point.
!
!    Local, real ( kind = rk ) WQE(NQE), stores the quadrature weights.
!
!    Local, real ( kind = rk ) X, Y, the coordinates of a particular
!    quadrature point.
!
!    Local, real ( kind = rk ) XQE(NQE), YQE(NQE), stores the location
...
!
  ...
  real ( kind = rk ) :: u
  real ( kind = rk ) :: uh
  real ( kind = rk ), dimension(nqe) :: wqe
  real ( kind = rk ) :: x
  real ( kind = rk ), dimension(nqe) :: xqe
  real ( kind = rk ) :: y
  real ( kind = rk ), dimension(nqe) :: yqe
...
!
!    Output, real ( kind = rk ) U, DUDX, DUDY, the value of
...
!  Parameters:
!
!    Input, integer NX, NY, controls the number of elements
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Output, integer INDX(NODE_NUM), the index of the unknown
!    in the finite element linear system.
!
!    Output, integer NUNK, the number of unknowns.
!
  implicit none
...
!    rule.  This is actually fixed at 13.
!
!    Output, real ( kind = rk ) WQE(13), the quadrature weights.
!
!    Output, real ( kind = rk ) XQE(NQE), YQE(NQE), the X and Y coordinates
...
!    Input, real ( kind = rk ) F(NUNK), the coefficients of the solution.
!
!    Input, integer INDX(NODE_NUM), gives the index of the unknown
!    quantity associated with the given node.
!
!    Input, integer NODE_NUM, the number of nodes.
!
...
!
!    Input, integer TRIANGLE_NODE(6,TRI_NUM), lists, for
