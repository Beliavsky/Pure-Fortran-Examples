...
!
  do i = 1, nip
    xi(i) = real ( i - 1, kind = rk ) / real ( nip - 1, kind = rk )
  end do
...
!
  do i = 1, nxi
    xi(i) = real ( i - 1, kind = rk ) / real ( nxi - 1, kind = rk )
  end do
 
  do j = 1, nyi
    yi(j) = real ( j - 1, kind = rk ) / real ( nyi - 1, kind = rk )
  end do
!
...
!
  ...
  integer, parameter :: ncp = 4
  integer, parameter :: ndp = 30
  integer, parameter :: nip = 1
  integer, parameter :: nxi = 6
  integer, parameter :: nyi = 5

  real ( kind = rk ), dimension ( nxi, nyi ) :: dzi1
  real ( kind = rk ), dimension ( nxi, nyi ) :: dzi2
  integer idp
  integer iwk1(max(31,27+ncp)*ndp+nip)
  integer iwk2(max(31,27+ncp)*ndp+nxi*nyi)
  integer ixi
  integer iyi
  integer md
  real ( kind = rk ) wk(8*ndp)
  ...
  real ( kind = rk ), dimension ( nyi ) :: yi = (/ &
    0.00D+00,  5.00D+00, 10.00D+00, 15.00D+00, 20.00D+00 /)
  real ( kind = rk ), dimension ( ndp ) :: zd = (/ &
  ...
  real ( kind = rk ), dimension ( nxi, nyi ) :: zi = reshape ( (/ &
    ...
    (/ nxi, nyi /) )
  real ( kind = rk ), dimension ( nxi, nyi ) :: zi1
  real ( kind = rk ), dimension ( nxi, nyi ) :: zi2  
