...
!
      if (msglvl > 2) then
          call svout1 ( logfil, 1, rnorm0, ndigit, &
                      '_getv0: re-orthonalization ; rnorm0 is')
          call svout1 ( logfil, 1, rnorm, ndigit, &
...
!
         if (msglvl > 2 ) then
            call ivout1 (logfil, 1, jj, ndigit, &
                     '_napps: shift number.')
            call svout1 ( logfil, 1, sigmar, ndigit, &
                     '_napps: The real part of the shift ')
            call svout1 ( logfil, 1, sigmai, ndigit, &
...
!
            ...
            if( abs( h( i+1,i ) )<=max( ulp*tst1, smlnum ) ) then
               if (msglvl > 0) then
                  call ivout1 (logfil, 1, i, ndigit, &
                       '_napps: matrix splitting at row/column no.')
                  call ivout1 (logfil, 1, jj, ndigit, &
                       '_napps: matrix splitting with shift number.')
                  call svout1 ( logfil, 1, h(i+1,i), ndigit, &
...
!
         if (msglvl > 2) then
             call ivout1 (logfil, 1, istart, ndigit, &
                         '_napps: Start of current block ')
             call ivout1 (logfil, 1, iend, ndigit, &
...
!
      if (msglvl > 1) then
         call svout1 ( logfil, 1, q(kplusp,kev), ndigit, &
              '_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}')
         call svout1 ( logfil, 1, h(kev+1,kev), ndigit, &
              '_napps: betak = e_{kev+1}^T*H*e_{kev}')
         call ivout1 (logfil, 1, kev, ndigit, &
...
!
      if (msglvl > 0) then
         call ivout1 (logfil, 1, mxiter, ndigit, &
...
!
         if (msglvl > 2) then
             call ivout1 (logfil, 1, numcnv, ndigit, &
...
!
      if (msglvl > 0) then
         call ivout1 (logfil, 1, kev, ndigit, '_ngets: KEV is')
         call ivout1 (logfil, 1, np, ndigit, '_ngets: NP is')
         ...
         do 30 i = istart, kplusp-1
            ...
            if (h(i+1,1) <= epsmch*big) then
               if (msglvl > 0) then
                  ...
                  call svout1 ( logfil, 1, h(i+1,1), ndigit, &
...
!
      if (msglvl > 1) then
         ...
         call svout1 ( logfil, 1, h(kev+1,1), ndigit, &
...
!
      if (msglvl > 0) then
         call ivout1 (logfil, 1, kev, ndigit, '_sgets: KEV is')
         call ivout1 (logfil, 1, np, ndigit, '_sgets: NP is')
...
!*****************************************************************************80
!
!! ivout1() calls ivout with a scalar argument.
!
!  Discussion:
!
!    Changes in the Fortran standard mean a scalar can't "pretend" to
!    be a vector any more.
!
!  Modified:
!
!    13 July 2024
...
!
      ...
      call ivout ( lout, n, ixvec, idigit, ifmt )
...
!-----------------------------------------------------------------------
!
!! svout1() call svout() with a scalar value.
!
      ...
      sxvec(1) = sx
      call svout ( lout, n, sxvec, idigit, ifmt )
