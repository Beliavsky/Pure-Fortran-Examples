...
!
!    integer NODE_XYZ(3,NODE_NUM), the node coordinates.
!
!    integer RECTANGLE_NUM, the number of rectangles.
!
!    integer RECTANGLE_NODE(16,RECTANGLE_NUM), the nodes that make up
!    each rectangle.
!
!    integer PATCH, the index of the Bezier patch.
!
!    integer POINT_NUM, the number of points at which evaluation
!    is desired.
!
!    real ( kind = rk ) POINT_UV(2,POINT_NUM), the (U,V) parameter
!    coordinates of the points.
!
!  Output:
!
!    real ( kind = rk ) POINT_XYZ(3,POINT_NUM), the (X,Y,Z)
...
!
  ...
  integer patch
  real ( kind = rk ) patch_xmat(4,4)
  real ( kind = rk ) patch_ymat(4,4)
  real ( kind = rk ) patch_zmat(4,4)
  integer point
...
!  Output:
!
!    integer RECTANGLE_NEIGHBOR(4,RECTANGLE_NUM), the indices of
...
!    integer NODE_NUM, the number of nodes.
!
!    real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates of the
!    nodes.
!
  implicit none
...
!  Input:
!
!    character ( len = * ) NODE_FILE_NAME, the name of the node file.
!
!    integer NODE_NUM, the number of nodes.
!
!  Output:
...
!    integer RECTANGLE_NUM, the number of rectangles.
!
!    integer RECTANGLE_NODE(16,RECTANGLE_NUM), the nodes
...
!  Input:
!
!    character ( len = * ) RECTANGLE_FILE_NAME, the name of the
!    rectangle file.
!
!    integer RECTANGLE_NUM, the number of rectangles.
!
...
!  Output:
!
!    integer NODE_NUM, the number of rectangles.
!
  implicit none
...
!    rectangle file.
!
!    integer RECTANGLE_NUM, the number of rectangle.
!
!    integer RECTANGLE_NODE(16,RECTANGLE_NUM), the nodes that
...
!  Input:
!
!    character ( len = * ) INPUT_FILENAME, the name of the file.
!
!  Output:
!
...
!
!    integer I1, I2, the two rows to swap.
!
!  Output:
!
!    integer A(M,N), an array after row swapping.
!
  implicit none
...
!    1, an error occurred.
!
!    integer LENGTH, the number of characters of S used to make IVAL.
!
  implicit none
...
!  Output:
!
!    integer IVEC(N), the values read from the string.
!
...
!  Output:
!
!    real ( kind = rk ) RVEC(N), the values read from the string.
!
!    integer IERROR, error flag.
!    0, no errors occurred.
!    -K, could not read data for entries -K through N.
...
!  Output:
!
!    integer NWORD, the number of "words" in the string.
