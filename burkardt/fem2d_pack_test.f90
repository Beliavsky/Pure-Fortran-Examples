...
!
  ...
  integer, allocatable, dimension ( : ) :: var_node
  integer var_num
...
!
  ...
  integer, parameter :: nelemx = 7
  integer, parameter :: nelemy = 5

  real ( kind = rk ) angle
  real ( kind = rk ), allocatable, dimension ( : ) :: c
  integer col
  real ( kind = rk ), allocatable, dimension ( : ) :: dcdx
  real ( kind = rk ) dcdx_exact
  real ( kind = rk ), allocatable, dimension ( : ) :: dcdy
  real ( kind = rk ) dcdy_exact
...
!
  ...
  do row = 0, nelemy
    ...
    r = ( real ( nelemy - row, kind = rk ) *  1.0D+00   &
        + real (        + row, kind = rk ) *  3.0D+00 ) &
        / real ( nelemy,       kind = rk )

    do col = 0, nelemx

      node = node + 1
      angle = ( real ( nelemx - col, kind = rk ) * 135.0D+00   &
              + real (        + col, kind = rk ) *  45.0D+00 ) &
              / real ( nelemx,       kind = rk )
...
!
  ...
  real ( kind = rk ) x(m,n)
  real ( kind = rk ) xm(m-1,n-1)
  real ( kind = rk ) y(m,n)
  real ( kind = rk ) ym(m-1,n-1)
  ...
  do i = 1, m
    do j = 1, n
      ...
      y(i,j) = ( real ( m - i,     kind = rk ) * q(2,1)   &
               + real (     i - 1, kind = rk ) * q(2,3) ) &
               / real ( m     - 1, kind = rk )

      x(i,j) = ( real ( n - j,     kind = rk ) * q(1,1)   &
               + real (     j - 1, kind = rk ) * q(1,3) ) &
               / real ( n     - 1, kind = rk )
    end do
  end do
...
!
  do i = 1, m-1
    ...
    do j = 1, n-1
      ...
      ym(i,j) = ( real ( 2 * m - 2 * i - 1, kind = rk ) * q(2,1)   &
                + real (         2 * i - 1, kind = rk ) * q(2,3) ) &
                / real ( 2 * m         - 2, kind = rk )

      xm(i,j) = ( real ( 2 * n - 2 * j - 1, kind = rk ) * q(1,1)   &
...
!
  do i = 1, m
    do j = 1, n
      ...
      x(i,j) = ( &
               real ( n - j,     kind = rk )  * ( real ( m - i,     kind = rk ) * q(1,1)   &
                                               + real (     i - 1, kind = rk ) * q(1,2) ) &
                                               / real ( m     - 1, kind = rk )            &
             + real (     j - 1, kind = rk )  * ( real ( m - i,     kind = rk ) * q(1,4)   &
                                               + real (     i - 1, kind = rk ) * q(1,3) ) &
                                               / real ( m     - 1, kind = rk )            &
           ) / real ( n     - 1, kind = rk )

      y(i,j) = ( &
               real ( n - j,     kind = rk )  * ( real ( m - i,     kind = rk ) * q(2,1)   &
                                               + real (     i - 1, kind = rk ) * q(2,2) ) &
                                               / real ( m     - 1, kind = rk )            &
             + real (     j - 1, kind = rk )  * ( real ( m - i,     kind = rk ) * q(2,4)   &
                                               + real (     i - 1, kind = rk ) * q(2,3) ) &
                                               / real ( m     - 1, kind = rk )            &
           ) / real ( n     - 1, kind = rk )
...
!
  ...
  do row = 0, nelemy
    ...
    y = ( real ( 3*nelemy - row, kind = rk ) *  0.0D+00   &
        + real (          + row, kind = rk ) *  6.0D+00 ) &
    ...
    do col = 0, nelemx
      ...
      x = ( real ( 2*nelemx - col, kind = rk ) * 0.0D+00   &
          + real (          + col, kind = rk ) * 6.0D+00 ) &
...
!
  ...
  integer, parameter :: nelemx = 6
  integer, parameter :: nelemy = 6
...
!
  ...
  do row = 0, 2*nelemy
    ...
    y = ( real ( 2*nelemy - row, kind = rk ) *  0.0D+00   &
        + real (          + row, kind = rk ) *  6.0D+00 ) &
...
!
  ...
  integer, parameter :: num_x = 5
  integer, parameter :: num_y = 3

  integer, parameter :: node_num = num_x * num_y
...
!
  ...
  integer, parameter :: element_order = 3
  integer, parameter :: nelemx = 8
  integer, parameter :: nelemy = 8
  ...
  integer row
  real ( kind = rk ) x
  real ( kind = rk ) :: x_max = 8.0D+00
  real ( kind = rk ) :: x_min = 0.0D+00
  real ( kind = rk ) y
  real ( kind = rk ) :: y_max = 8.0D+00
  real ( kind = rk ) :: y_min = 0.0D+00
...
!
  ...
  do row = 0, nelemy
    ...
    y = ( real ( nelemy - row, kind = rk ) *  y_min   &
        + real (        + row, kind = rk ) *  y_max ) &
        / real ( nelemy,       kind = rk )

    do col = 0, nelemx

      node = node + 1
      x = ( real ( nelemx - col, kind = rk ) * x_min   &
          + real (        + col, kind = rk ) *  x_max ) &
          / real ( nelemx,       kind = rk )
...
!
  ...
  integer, parameter :: element_num = 8
  integer, parameter :: element_order = 3
  ...
  real ( kind = rk ), dimension ( 2, node_num ) :: node_xy = reshape ( (/ &
    0.0D+00, 0.0D+00, &
    0.0D+00, 0.5D+00, &
    0.0D+00, 1.0D+00, &
    0.5D+00, 0.0D+00, &
...
!
  ...
  integer, parameter :: element_num = 2
  integer, parameter :: element_order = 6
  integer, parameter :: node_num = 9
...
!
  ...
  do j = 1, n
    ...
    call random_number ( harvest = ref(1:2,j) )
...
!
  ...
  integer j
  real ( kind = rk ) phy(2,n)
  real ( kind = rk ), dimension (2,n) :: ref = reshape ( (/ &
    0.00D+00, 0.00D+00, &
    1.00D+00, 0.00D+00, &
    0.00D+00, 1.00D+00, &
    0.50D+00, 0.00D+00, &
  ...
  real ( kind = rk ), dimension(2,6) :: t = reshape ( (/ &
    0.0D+00, 0.0D+00, &
    2.0D+00, 0.0D+00, &
    0.0D+00, 4.0D+00, &
    1.0D+00, 0.0D+00, &
...
!
  ...
  integer, parameter :: element_num = 40
  integer, parameter :: psi_num = element_num + 1
  integer, parameter :: quad_num = 5
  ...
  real ( kind = rk ) x
  real ( kind = rk ), parameter :: x_max = 1.0D+00
  real ( kind = rk ), parameter :: x_min = 0.0D+00
  real ( kind = rk ) xl
  real ( kind = rk ) xr
...
!
  do i = 1, psi_num
    node_x(i) = ( real ( psi_num - i,     kind = rk ) * x_min   &
                + real (           i - 1, kind = rk ) * x_max ) &
                / real ( psi_num     - 1, kind = rk )
  end do
!
...
!
  ...
  integer, parameter :: nelemx = 3
  integer, parameter :: nelemy = 4
  integer node
  integer node_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: node_xyz
...
!
  ...
  integer i
  integer j
  integer, parameter :: nelemx = 2
  integer, parameter :: nelemy = 2
  integer node
  integer node_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: node_xyz
...
!
  ...
  integer, parameter :: order_max = 64
