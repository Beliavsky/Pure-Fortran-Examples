...
!*****************************************************************************80
!
!! circle_packing() reads a point dataset, and applies the circle test.
!
!  Usage:
!
!    circle_packing point_file
!
!  Licensing:
!
...
!
  ...
  real ( kind = rk ) circle_volume
  integer dim_num
  character ( len = 255 ) input_file_name
  integer point_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: quasi
  logical walls

  call timestamp ( )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'circle_packing():'
  ...
  if ( dim_num <= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'circle_packing(): Fatal error!'
  ...
  walls = .false.
  call circle_packing_test ( dim_num, point_num, quasi, walls, circle_volume )
...
subroutine circle_packing_test ( dim_num, point_num, quasi, walls, circle_volume )

!*****************************************************************************80
!
!! circle_packing_test() performs the circle packing test on a set of points.
!
!  Discussion:
!
...
!
!    Input, real ( kind = rk ) QUASI(DIM_NUM,POINT_NUM), the points.
!
...
!  Output:
!
!    Output, real ( kind = rk ) CIRCLE_VOLUME, the amount of volume taken up
...
!
  ...
  real ( kind = rk ) circle_volume
  integer i
  real ( kind = rk ), dimension ( dim_num, point_num ) :: quasi
  real ( kind = rk ), dimension ( point_num ) :: radius
  real ( kind = rk ) radius_ave
  real ( kind = rk ) radius_max
  real ( kind = rk ) radius_min
  ...
  if ( verbose ) then
    ...
    radius_ave = sum ( radius(1:point_num) ) / real ( point_num, kind = rk )
...
!
!    Output, integer NCOLUMN, the number of columns assumed
!    to be in the file.
!
  implicit none
...
!    Input, integer POINT_NUM, the number of points.
!
!    Input, real ( kind = rk ) COORD(DIM_NUM,POINT_NUM), the point coordinates.
...
!
!    Output, real ( kind = rk ) RADIUS(POINT_NUM), the radius of the sphere
!    around each point.
!
  implicit none
  ...
  real ( kind = rk ) coord(dim_num,point_num)
  real ( kind = rk ) distance_j
  integer, parameter :: FIXED = 0
  integer, parameter :: FREE = 1
  integer i
  integer j
  integer next
  real ( kind = rk ) radius(point_num)
  real ( kind = rk ) radius_i
  real ( kind = rk ) radius_min
  integer status(point_num)
  logical walls
...
!
!    Output, real ( kind = rk ) QUASI(NDIM,NPOINT), the points.
!
  implicit none
  ...
  integer ios
  integer j
  real ( kind = rk ) quasi(ndim,npoint)
