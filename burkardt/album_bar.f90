...
!
  ...
  real ( kind = rk ) yorg
  real ( kind = rk ) ystp
  real ( kind = rk ), allocatable :: zero(:)

  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'album_bar:'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Use DISLIN to make a bar chart of music album sales'
!
!  Read the data.
!
  call r8mat_header_read ( 'album_data.txt', m, n )

  allocate ( xy(1:m,1:n) )
  allocate ( zero(1:n) )

  call r8mat_data_read ( 'album_data.txt', m, n, xy )
!
...
!
  ...
  call graf ( xmin, xmax, xorg, xstp, ymin, ymax, yorg, ystp )
!
!  BEGIN LEVEL 2 COMMANDS.
!
!  Define the title.
!
  call titlin ( 'Music album sales, all formats', 1 )
  call title ( )
!
