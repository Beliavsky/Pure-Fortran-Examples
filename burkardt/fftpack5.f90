...
!
!    Input/output, complex C(LENC) containing the sequence to be 
!    transformed.
!
!    Input, integer LENC, the dimension of the C array.  
!    LENC must be at least INC*(N-1) + 1.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be 
...
!    array C, of two consecutive elements within the sequence to be transformed.
!
!    Input/output, complex C(LENC) containing the sequence to 
!    be transformed.
!
!    Input, integer LENC, the dimension of the C array.  
!    LENC must be at least INC*(N-1) + 1.
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), containing the prime factors 
...
!
!    Input/output, complex C(LDIM,M), on intput, the array of 
...
!
!    Input/output, complex C(LDIM,M), on input, the array of two 
...
!    + INT(LOG(REAL(M))) + 8.
!
!    Output, real WSAVE(LENSAV), contains the prime factors of L 
...
!
  ...
  call cfftmi ( m, wsave(iw), 2*m + int(log( real ( m ))) + 4, ier1 )
...
!    transformed. 
!
!    Input/output, complex C(LENC), an array containing LOT 
...
!    transformed.
!
!    Input/output, complex C(LENC), array containing LOT sequences,
...
!
!    Input/output, real R(LENR); on input, containing the sequence 
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), containing the prime factors of N
...
!
!    Input/output, real R(LENR), array containing LOT sequences, 
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), containing the prime factors of 
...
!    in array R, of two consecutive elements within the sequence.
!
!    Input/output, real R(LENR), containing the sequence to 
!     be transformed.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1)+ 1.
...
!
  ...
  real ( kind = rk ) cc(in1,ido,2,l1)
  real ( kind = rk ) ch(in2,ido,l1,2)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) ch(in2,ido,2,l1)
  real ( kind = rk ) cc(in1,ido,l1,2)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) arg
  real ( kind = rk ) cc(in1,ido,3,l1)
  real ( kind = rk ) ch(in2,ido,l1,3)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) arg
  real ( kind = rk ) cc(in1,ido,l1,3)
  real ( kind = rk ) ch(in2,ido,3,l1)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) cc(in1,ido,4,l1)
  real ( kind = rk ) ch(in2,ido,l1,4)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) cc(in1,ido,l1,4)
  real ( kind = rk ) ch(in2,ido,4,l1)
  real ( kind = rk ) hsqt2
  integer i
  integer ic
  integer idp2
...
!
  ...
  real ( kind = rk ) arg
  real ( kind = rk ) cc(in1,ido,5,l1)
  real ( kind = rk ) ch(in2,ido,l1,5)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) arg
  real ( kind = rk ) cc(in1,ido,l1,5)
  real ( kind = rk ) ch(in2,ido,5,l1)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  real ( kind = rk ) ar2h
  real ( kind = rk ) arg
  real ( kind = rk ) c1(in1,ido,l1,ip)
  real ( kind = rk ) c2(in1,idl1,ip)
  real ( kind = rk ) cc(in1,ido,ip,l1)
  real ( kind = rk ) ch(in2,ido,l1,ip)
  real ( kind = rk ) ch2(in2,idl1,ip)
  real ( kind = rk ) dc2
  real ( kind = rk ) dcp
  real ( kind = rk ) ds2
  real ( kind = rk ) dsp
...
!
!    Input/output, real ( kind = rk ) R(LENR); on input, containing the sequence 
!    to be transformed, and on output, containing the transformed sequence.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1)+ 1.
!
!    Input, real ( kind = rk ) WSAVE(LENSAV).  WSAVE's contents must be 
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Workspace, real ( kind = rk ) WORK(LENWRK).
!
!    Input, integer LENWRK, the dimension of the WORK array.  
!    LENWRK must be at least N.
!
...
!
  ...
  if ( lensav < 2 * n + int ( log ( real ( n, kind = rk ) ) ) + 4 ) then
    ier = 2
...
!  Parameters:
!
!    Input, integer N, the number of elements to be transformed in 
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real ( kind = rk ) WSAVE(LENSAV), containing the prime factors of 
!    N and also containing certain trigonometric values which will be used 
...
!
  ...
  lnsv = n + int ( log ( real ( n, kind = rk ) ) ) + 4
...
!    array R, of two consecutive elements within the sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), containing the sequence 
!    to be transformed.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1)+ 1.
...
!    LENSAV must be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real ( kind = rk ) WSAVE(LENSAV), containing the prime factors 
!    of N and also containing certain trigonometric values which will be used 
...
!
  ...
  lnsv = nm1 + int ( log ( real ( nm1, kind = rk ) ) ) + 4
...
!
  ...
  fnm1s2 = real ( nm1, kind = rk ) / 2.0D+00
  dsum = 0.5D+00 * dsum
  x(1,1) = fnm1s2 * x(1,1)
  ...
  fnm1s4 = real ( nm1, kind = rk ) / 4.0D+00
...
!    in array R, of two consecutive elements within the sequence. 
!
!    Input/output, real ( kind = rk ) R(LENR), on input, the data to be 
...
!
  ...
  if ( lensav < n + int ( log ( real ( n, kind = rk ) ) ) + 4 ) then
    ier = 2
...
!    in array R, of two consecutive elements within the sequence. 
!
!    Input/output, real ( kind = rk ) R(LENR), on input, contains the sequence
!    to be transformed, and on output, the transformed data.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1) + 1.
...
!
  ...
  sn = 1.0D+00 / real ( n, kind = rk )
  tsn = 2.0D+00 / real ( n, kind = rk )
  tsnm = - tsn
  modn = mod ( n, 2 )
  nl = n - 2
...
!
!    Output, real ( kind = rk ) WA(N), trigonometric information.
!
!    Output, real ( kind = rk ) FAC(15), factorization information.  
...
!
  ...
  fac(1) = real ( n, kind = rk )
  fac(2) = real ( nf, kind = rk )
  tpi = 8.0D+00 * atan ( 1.0D+00 )
  argh = tpi / real ( n, kind = rk )
  is = 0
  nfm1 = nf - 1
  l1 = 1
  ...
  do k1 = 1, nfm1
    ...
    do j = 1, ipm
      ...
      do ii = 3, ido, 2
        ...
        arg = fi * argld
        wa(i-1) = real ( cos ( arg ), kind = rk )
        wa(i) = real ( sin ( arg ), kind = rk )
      end do
      is = is + ido
    end do
    l1 = l2
...
!    in array R, of two consecutive elements within the sequence.
!
!    Input/output, real ( kind = rk ) R(LENR), on input, contains the 
...
!
  ...
  if ( lensav < n / 2 + n + int ( log ( real ( n, kind = rk ) ) ) + 4 ) then
    ier = 2
...
!
  ...
  lnsv = np1 + int ( log ( real ( np1, kind = rk ) ) ) + 4
...
!
  ...
  fnp1s4 = real ( np1, kind = rk ) / 4.0D+00
  x(1,1) = fnp1s4 * xh(1)
  dsum = x(1,1)
...
!
  ...
  sfnp1 = 1.0D+00 / real ( np1, kind = rk )
  x(1,1) = 0.5D+00 * xh(1)
  dsum = x(1,1)
...
!
!    Output, real FAC(15), factorization information.  FAC(1) is 
...
!    Output, integer NF, the number of factors.
!
!    Output, real ( kind = rk ) FAC(*), a list of factors of N.
!
  implicit none
...
!    in array R, of two consecutive elements within the sequence. 
!
!    Input/output, real R(LENR), on input, the data to be 
!    transformed, and on output, the transformed data.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1) + 1. 
...
!    array R, of two consecutive elements within the sequence. 
!
!    Input/output, real R(LENR), on input, contains the sequence 
!    to be transformed, and on output, the transformed data.
!
!    Input, integer LENR, the dimension of the R array.  
!    LENR must be at least INC*(N-1) + 1.
...
!    is most efficient when M is a product of small primes.
!
!    Input/output, real R(LDIM,M), the real array of two 
...
!    + INT(LOG(REAL(M))) + 8.
!
!    Workspace, real WORK(LENWRK).  WORK provides workspace, and 
...
!    + INT(LOG(REAL(M))) + 8. 
!
!    Workspace, real WORK(LENWRK), provides workspace, and its 
...
!    array R, of two consecutive elements within the same sequence.
!
!    Input/output, real R(LENR), real array containing LOT 
...
!    LENSAV must be at least N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), work array containing the prime 
...
!    array R, of two consecutive elements within the sequence.
!
!    Input/output, real R(LENR), on input, the sequence to be 
...
!    array R, of two consecutive elements within the same sequence.
!
!    Input/output, real R(LENR), containing LOT sequences, each 
...
!
  ...
  real ( kind = rk ) cc(in1,l1,ido,2)
  real ( kind = rk ) ch(in2,l1,2,ido)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  integer i
  integer k
...
!
  ...
  real ( kind = rk ) cc(in1,l1,ido,3)
  real ( kind = rk ) ch(in2,l1,3,ido)
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
  real ( kind = rk ) cr2
  real ( kind = rk ) cr3
...
!
  ...
  real ( kind = rk ) cc(in1,l1,ido,4)
  real ( kind = rk ) ch(in2,l1,4,ido)
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
  real ( kind = rk ) ci4
  real ( kind = rk ) cr2
...
!
  ...
  real ( kind = rk ) cc(in1,l1,ido,5)
  real ( kind = rk ) ch(in2,l1,5,ido)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
  ...
  real ( kind = rk ) ti4
  real ( kind = rk ) ti5
  real ( kind = rk ), parameter :: ti11 =  0.9510565162951536D+00
  real ( kind = rk ), parameter :: ti12 =  0.5877852522924731D+00
  real ( kind = rk ) tr2
  real ( kind = rk ) tr3
  real ( kind = rk ) tr4
  real ( kind = rk ) tr5
  real ( kind = rk ), parameter :: tr11 =  0.3090169943749474D+00
  real ( kind = rk ), parameter :: tr12 = -0.8090169943749474D+00
  real ( kind = rk ) wa(ido,4,2)
...
!
  ...
  real ( kind = rk ) cc(in1,l1,ip,ido)
  real ( kind = rk ) cc1(in1,lid,ip)
  real ( kind = rk ) ch(in2,l1,ido,ip)
  real ( kind = rk ) ch1(in2,lid,ip)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  integer i
  integer idlj
...
!    array C, of two consecutive elements within the sequence to be transformed.
!
!    Input/output, complex ( kind = ck ) C(LENC) containing the sequence to 
!    be transformed.
!
!    Input, integer LENC, the dimension of the C array.  
!    LENC must be at least INC*(N-1) + 1.
...
!    transform is most efficient when M is a product of small primes. 
!
!    Input/output, complex ( kind = ck ) C(LDIM,M), on intput, the array of two
...
!
  ...
  if ( lensav < 2 * l + int ( log ( real ( l, kind = rk ) ) ) + &
                2 * m + int ( log ( real ( m, kind = rk ) ) ) + 8 ) then
    ier = 2
...
!  On 10 May 2010, the value of IW was modified.
!
  iw = 2 * l + int ( log ( real ( l, kind = rk ) ) ) + 5

  call zfftmb ( l, 1, m, ldim, c, (l-1)+ldim*(m-1) +1, &
    wsave(iw), 2*m + int(log(real ( m, kind = rk ))) + 4, work, 2*l*m, ier1 )
...
!
  ...
  call zfftmb ( m, ldim, l, 1, c, (m-1)*ldim + l, wsave(iw), &
    2*l + int(log(real ( l, kind = rk ))) + 4, work, 2*m*l, ier1 )
...
!    transform is most efficient when M is a product of small primes. 
!
!    Input/output, complex ( kind = ck ) C(LDIM,M), on input, the array of two 
!    dimensions containing the (L,M) subarray to be transformed.  On output, 
!    the transformed data.
!
!    Input, real ( kind = rk ) WSAVE(LENSAV). WSAVE's contents must be 
...
!
  ...
  call zfftmf ( l, 1, m, ldim, c, (l-1) + ldim*(m-1) +1, wsave(iw), &
    2*m + int(log(real ( m, kind = rk ))) + 4, work, 2*l*m, ier1 )
...
!    + INT(LOG(REAL(M))) + 8.
!
!    Output, real ( kind = rk ) WSAVE(LENSAV), contains the prime factors of L 
!    and M, and also certain trigonometric values which will be used in 
...
!
  ...
  call zfftmi ( l, wsave(1), 2*l + int(log(real ( l, kind = rk ))) + 4, ier1 )
...
!
  ...
  call zfftmi ( m, wsave(iw), 2*m + int(log(real ( m, kind = rk ))) + 4, ier1 )
...
!    transformed. 
!
!    Input/output, complex ( kind = ck ) C(LENC), an array containing LOT 
!    sequences, each having length N, to be transformed.  C can have any 
!    number of dimensions, but the total number of locations must be at least 
!    LENC.  On output, C contains the transformed sequences.
!
...
!    transformed.
!
!    Input/output, complex ( kind = ck ) C(LENC), array containing LOT sequences,
!    each having length N, to be transformed.  C can have any number of 
!    dimensions, but the total number of locations must be at least LENC.
!
!    Input, integer LENC, the dimension of the C array.  
...
!
  ...
  real ( kind = rk ) cc(2,in1,l1,ido,2)
  real ( kind = rk ) ch(2,in2,l1,2,ido)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  integer i
  integer im1
...
!
  ...
  real ( kind = rk ) cc(2,in1,l1,ido,3)
  real ( kind = rk ) ch(2,in2,l1,3,ido)
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
  real ( kind = rk ) cr2
  real ( kind = rk ) cr3
...
!
  ...
  real ( kind = rk ) cc(2,in1,l1,ido,4)
  real ( kind = rk ) ch(2,in2,l1,4,ido)
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
  real ( kind = rk ) ci4
  real ( kind = rk ) cr2
...
!
  ...
  real ( kind = rk ) cc(2,in1,l1,ido,5)
  real ( kind = rk ) ch(2,in2,l1,5,ido)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  real ( kind = rk ) ci2
  real ( kind = rk ) ci3
...
!
  ...
  real ( kind = rk ) cc(2,in1,l1,ip,ido)
  real ( kind = rk ) cc1(2,in1,lid,ip)
  real ( kind = rk ) ch(2,in2,l1,ido,ip)
  real ( kind = rk ) ch1(2,in2,lid,ip)
  real ( kind = rk ) chold1
  real ( kind = rk ) chold2
  integer i
  integer idlj
