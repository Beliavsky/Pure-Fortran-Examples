...
!*****************************************************************************80
!
!! fem2d_heat_rectangle() solves the heat equation in a rectangle.
!
!  Discussion:
!
...
!  Local:
!
!    Local, real ( kind = rk ) A(3*IB+1,NODE_NUM), the coefficient matrix.
!
!    Local, real ( kind = rk ) EH1, the H1 seminorm error.
!
!    Local, real ( kind = rk ) EL2, the L2 error.
!
!    Local, real ( kind = rk ) ELEMENT_AREA(ELEMENT_NUM), the area of elements.
!
...
!
!    Local, real ( kind = rk ) F(NODE_NUM), the right hand side.
!
...
!
!    Local, integer ELEMENT_ORDER, the number of nodes used to 
!    form one element.
!
!    Local, real ( kind = rk ) NODE_XY(2,NODE_NUM), the
...
!
!    Local, integer QUAD_NUM, the number of quadrature points used
!    for assembly.
!
!    Local, real ( kind = rk ) TIME, the current time.
!
!    Local, real ( kind = rk ) TIME_FINAL, the final time.
!
!    Local, real ( kind = rk ) TIME_INIT, the initial time.
!
!    Local, real ( kind = rk ) TIME_OLD, the time at the previous time step.
!
!    Local, integer TIME_STEP_NUM, the number of time steps
!    to take.
!
!    Local, real ( kind = rk ) TIME_STEP_SIZE, the size of the time steps.
!
!    Local, real ( kind = rk ) U(NODE_NUM), the finite element coefficients
!    defining the solution at the current time.
!
!    Local, real ( kind = rk ) WQ(QUAD_NUM), quadrature weights.
!
!    Local, real ( kind = rk ) XL, XR, YB, YT, the X coordinates of
!    the left and right sides of the rectangle, and the Y coordinates
!    of the bottom and top of the rectangle.
!
!    Local, real ( kind = rk ) XQ(QUAD_NUM,ELEMENT_NUM),
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: dudx_exact
  real ( kind = rk ), allocatable, dimension ( : ) :: dudy_exact
  real ( kind = rk ) eh1
  real ( kind = rk ) el2
  real ( kind = rk ), dimension(element_num) :: element_area
  ...
  logical node_label
  integer node_show
  real ( kind = rk ), dimension(2,node_num) :: node_xy
  integer, allocatable, dimension (:) :: pivot
  real ( kind = rk ) time
  character ( len = 255 ) time_file_name
  real ( kind = rk ) time_final
  real ( kind = rk ) time_init
  real ( kind = rk ) time_old
  integer time_step
  integer time_step_num
  real ( kind = rk ) time_step_size
  integer time_unit
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: u_exact
  character ( len = 255 ) u_file_name
  real ( kind = rk ), allocatable, dimension ( : ) :: u_old
  real ( kind = rk ), dimension(quad_num) :: wq
  real ( kind = rk ), parameter :: xl = 0.0D+00
  real ( kind = rk ), dimension(quad_num,element_num) :: xq
  real ( kind = rk ), parameter :: xr = 1.0D+00
  real ( kind = rk ), parameter :: yb = 0.0D+00
  real ( kind = rk ), dimension(quad_num,element_num) :: yq
  real ( kind = rk ), parameter :: yt = 1.0D+00
...
!
  ...
  time_final = 0.5D+00
  time_step_num = 10
  time_step_size = ( time_final - time_init ) / real ( time_step_num, kind = rk )
...
!
  ...
  do time_step = 1, time_step_num
    ...
    time = ( real ( time_step_num - time_step, kind = rk ) * time_init    &
           + real (                 time_step, kind = rk ) * time_final ) &
           / real ( time_step_num,             kind = rk )
!
...
!
!    Input, real ( kind = rk ) NODE_XY(2,NODE_NUM), the coordinates of nodes.
!
!    Input, integer ELEMENT_ORDER, the number of nodes used to 
!    form one element.
!
...
!    used in assembly.
!
!    Input, real ( kind = rk ) WQ(QUAD_NUM), quadrature weights.
!
!    Input, real ( kind = rk ) XQ(QUAD_NUM,ELEMENT_NUM),
!    YQ(QUAD_NUM,ELEMENT_NUM), the
!    coordinates of the quadrature points in each element.
!
!    Input, real ( kind = rk ) ELEMENT_AREA(ELEMENT_NUM), the area of elements.
!
!    Input, integer IB, the half-bandwidth of the matrix.
!
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Input, real ( kind = rk ) TIME_STEP_SIZE, the size of the time step.
!
!    Input, real ( kind = rk ) U_OLD(NODE_NUM), the finite element
!    coefficients for the solution at the previous time.
!
!    Input/output, real ( kind = rk ) A(3*IB+1,NODE_NUM), the NODE_NUM
!    by NODE_NUM coefficient matrix, stored in a compressed format.
!
!    Input/output, real ( kind = rk ) F(NODE_NUM), the right hand side.
!
  implicit none
  ...
  real ( kind = rk ), dimension(3*ib+1,node_num) :: a
  integer basis
  real ( kind = rk ) bi
  real ( kind = rk ) bj
  real ( kind = rk ) dbidx
  ...
  real ( kind = rk ), dimension(element_num) :: element_area
  integer, dimension(element_order,element_num) :: element_node
  real ( kind = rk ), dimension(node_num) :: f
  integer j
  integer node
  real ( kind = rk ), dimension(2,node_num) :: node_xy
  integer quad
...
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Input/output, real ( kind = rk ) A(3*IB+1,NODE_NUM), the NODE_NUM by
...
!
  ...
  real ( kind = rk ), dimension(3*ib+1,node_num) :: a
  real ( kind = rk ) dudx_exact(node_num)
  real ( kind = rk ) dudy_exact(node_num)
  real ( kind = rk ), dimension(node_num) :: f
  integer j
  integer jhi
  integer jlo
...
!
!    Input, integer ELEMENT_ORDER, the number of local nodes 
!    per element.
!
!    Input, integer ELEMENT_NUM, the number of elements.
!
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Output, real ( kind = rk ) ELEMENT_AREA(ELEMENT_NUM), the area of elements.
!
  implicit none
...
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Output, real ( kind = rk ) A(3*IB+1,NODE_NUM), the NODE_NUM by NODE_NUM
!    coefficient matrix, stored in a compressed format.
!
!    Output, real ( kind = rk ) F(NODE_NUM), the right hand side.
!
!  Local parameters:
!
!    Local, real ( kind = rk ) BB, BX, BY, the value of some basis function
!    and its first derivatives at a quadrature point.
!
!    Local, real ( kind = rk ) BBB, BBX, BBY, the value of another basis
...
!    coordinates of the nodes.
!
!    Input, real ( kind = rk ) TIME, the curent time.
!
!    Input, real ( kind = rk ) U(NODE_NUM), the solution vector of the finite
!    element system.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) A(2*ML+MU+1,N), on input, the matrix
...
!
!    Input, real ( kind = rk ) A(2*ML+MU+1,N), the DGB matrix.
!
...
!    ML and MU must be nonnegative, and no greater than N-1.
!
!    Input, real ( kind = rk ) A(2*ML+MU+1,N), the LU factors from DGB_FA.
!
!    Input, integer PIVOT(N), the pivot vector from DGB_FA.
!
!    Input/output, real ( kind = rk )l B(N).
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Input, real ( kind = rk ) NODE_XY(2,NODE_NUM), the X and Y
!    coordinates of nodes.
!
!    Input, real ( kind = rk ) U(NODE_NUM), the coefficients of the solution.
!
!    Input, integer ELEMENT_NUM, the number of elements.
!
!    Input, integer ELEMENT_ORDER, the number of nodes used to 
!    form one element.
!
!    Input, integer QUAD2_NUM, the number of points in the 
...
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Output, real ( kind = rk ) EL2, the L2 error.
!
!    Output, real ( kind = rk ) EH1, the H1 seminorm error.
!
!  Local Parameters:
!
!    Local, real ( kind = rk ) AR, the weight for a given quadrature point
!    in a given element.
!
!    Local, real ( kind = rk ) BB, BX, BY, a basis function and its first
...
!    Local, real ( kind = rk ) EL2, the L2 error.
!
!    Local, real ( kind = rk ) UEX, UEXX, UEXY, the exact solution and its first
!    derivatives evaluated at a particular quadrature point.
!
!    Local, real ( kind = rk ) UH, UHX, UHY, the computed solution and its first
!    derivatives evaluated at a particular quadrature point.
!
!    Local, real ( kind = rk ) WQE(QUAD2_NUM), stores the quadrature weights.
!
!    Local, real ( kind = rk ) X, Y, the coordinates of a particular
!    quadrature point.
!
!    Local, real ( kind = rk ) XQE(QUAD2_NUM), YQE(QUAD2_NUM), the
...
!
  ...
  real ( kind = rk ) dbidx
  real ( kind = rk ) dbidy
  real ( kind = rk ) dudx_exact(1)
  real ( kind = rk ) dudxh
  real ( kind = rk ) dudy_exact(1)
  real ( kind = rk ) dudyh
  real ( kind = rk ) eh1
  real ( kind = rk ) el2
  integer element
  ...
  integer quad
  real ( kind = rk ) time
  real ( kind = rk ), dimension(node_num) :: u
  real ( kind = rk ) u_exact(1)
  real ( kind = rk ) uh
  real ( kind = rk ), dimension(quad2_num) :: wqe
  real ( kind = rk ) x
  real ( kind = rk ), dimension(quad2_num) :: xqe
  real ( kind = rk ) xy(2)
  real ( kind = rk ) y
  real ( kind = rk ), dimension(quad2_num) :: yqe
...
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Output, real ( kind = rk ) U(NODE_NUM), the exact solution.
!
!    Output, real ( kind = rk ) DUDX(NODE_NUM), DUDY(NODE_NUM),
...
! real ( kind = rk ) dudt(node_num)
  real ( kind = rk ) dudx(node_num)
  real ( kind = rk ) dudy(node_num)
  integer node
  real ( kind = rk ) node_xy(2,node_num)
...
!  Parameters:
!
!    Input, integer NX, NY, controls the number of elements along
...
!  Parameters:
!
!    Input, integer NX, NY, the number of elements in the X 
!    and Y directions.
!
!    Input, integer NODE_NUM, the number of nodes.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, the (global) coordinates of the point
!    at which the basis function is to be evaluated.
!
!    Input, integer ELEMENT, the index of the element which 
!    contains the point.
!
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Output, real ( kind = rk ) B, DBDX, DBDY, the value of the basis function
...
!  Parameters:
!
!    Input, real ( kind = rk ) NODE_XY(2,NODE_NUM), the X and
...
!    form one element.
!
!    Output, real ( kind = rk ) WQ(3), quadrature weights.
!
!    Output, real ( kind = rk ) XQ(3,ELEMENT_NUM), YQ(3,ELEMENT_NUM), the
!    coordinates of the quadrature points in each element.
!
  implicit none
  ...
  integer ip3
  real ( kind = rk ), dimension(2,node_num) :: node_xy
  real ( kind = rk ), dimension(3) :: wq
  real ( kind = rk ) x1
  real ( kind = rk ) x2
  real ( kind = rk ) x3
  real ( kind = rk ), dimension(3,element_num) :: xq
  real ( kind = rk ) y1
  real ( kind = rk ) y2
  real ( kind = rk ) y3
  real ( kind = rk ), dimension(3,element_num) :: yq
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Input, integer ELEMENT, the index of the element for which
...
!    quadrature rule.  This is actually fixed at 13.
!
!    Output, real ( kind = rk ) WQE(QUAD2_NUM), the quadrature weights.
!
!    Output, real ( kind = rk ) XQE(QUAD2_NUM), YQE(QUAD2_NUM), the
!    quadrature points.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, a point in the region.
!
!    Input, real ( kind = rk ) TIME, the current time.
!
!    Output, real ( kind = rk ) RHS, the value of the right
...
!
!    Input, integer TRIANGLE_NODE(6,TRI_NUM), lists, for each
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) XL, XR, YB, YT, the X coordinates of
!    the left and right sides of the rectangle, and the Y coordinates
!    of the bottom and top of the rectangle.
!
!    Output, real ( kind = rk ) NODE_XY(2,NODE_NUM),
  ...
  do j = 1, 2 * ny - 1
    do i = 1, 2 * nx - 1
      ...
      node_xy(1,i+(j-1)*(2*nx-1)) =         &
        ( real ( 2 * nx - i - 1, kind = rk ) * xl   &
        + real (          i - 1, kind = rk ) * xr ) &
        / real ( 2 * nx     - 2, kind = rk )

      node_xy(2,i+(j-1)*(2*nx-1)) =         &
        ( real ( 2 * ny - j - 1, kind = rk ) * yb   &
        + real (          j - 1, kind = rk ) * yt ) &
        / real ( 2 * ny     - 2, kind = rk )
