...
!*****************************************************************************80
!
!! bump() solves a steady flow problem in a channel with a bump obstruction.
!
!  Discussion:
!
...
!
!    Local, integer IBUMP, determines where isoparametric elements
!    will be used.
...
!
!    Local, integer INDX(MAXNP,2), contains, for each node I, the
!    index of the U and V velocities at that node, or 0.
!
!    Local, integer INSC(NP), contains, for each node I, the
!    index of the pressure P at that node, or 0.
!
!    Local, integer ISOTRI(NELEMN), contains, for each element,
!    a 1 if the element is isoparametric, or 0 otherwise.
!
!    Local, integer IWRITE, controls the amount of output printed.
!
!    Local, logical LONG.
!    .TRUE. if region is "long and thin", and
!    .FALSE. if region is "tall and skinny".
!
!    Local, integer MAXELM, the maximum number of elements.
!    This is actually simply the first dimension of the array NODE.
!
!    Local, integer MAXEQN, the maximum number of equations,
!    and functions.
!
!    Local, integer MAXNEW, the maximum number of Newton steps 
!    to take in one iteration.
!
!    Local, integer MAXNP, the maximum number of nodes.
!    This is actually simply the first dimension of the array INDX.
!
!    Local, integer MAXROW, the first dimension of the matrix A.
!
!    Local, integer MAXSEC, the maximum number of secant steps 
!    to take.
!
!    Local, integer MX, the number of nodes in the X direction.
!
!    Local, integer MY, the number of nodes in the Y direction.
!
!    Local, integer NBAND, the bandwidth of the linear system.
!
!    Local, integer NBLEFT, the column at which the left corner 
!    of the bump lies.
!
!    Local, integer NBRITE, the column at which the right corner 
!    of the bump lies.
!
!    Local, integer NELEMN, the number of elements.
!
!    Local, integer NEQN, the number of equations, and functions.
!
!    Local, integer NLBAND, the lower bandwidth of the matrix.
!
!    Local, integer NNODES, the number of nodes per element, which
!    is 6 for the quadratic triangular elements in use here.
!
!    Local, integer NODE(MAXELM,NNODES), contains, for each 
!    element, the global node indices of the element nodes.
!
!    Local, integer NODEX0, the node whose Y coordinate is zero,
...
!
!    Local, integer NQUAD, the number of quadrature points per
!    element.
!
!    Local, integer NROW, the number of rows need to store the
!    matrix A.
!
!    Local, integer NUMNEW, total number of Newton steps taken.
!
!    Local, integer NX, controls the spacing of nodes and elements 
!    in the X direction.  There are 2*NX+1 nodes in the X
!    direction.
!
!    Local, integer NY, controls the spacing of nodes and elements 
...
!
!    Local, integer XY_UNIT, the unit to which XY graphics 
!    is written.
!
...
!  Parameters:
!
!    Input, integer IT, the element in which the basis function 
!    is defined.
!
!    Input, integer IQ, the index (1, 2 or 3) of the local node at
!    which the basis function is to be evaluated.
!
!    Input, integer ID, the index (1, 2 or 3) of the local node
...
!
!    Input, integer ILINE(MY), the index of the finite element 
!    coefficient associated with each node on the profile.
!
!    Input, integer MY, the number of nodes along the profile line.
!
!    Input, integer NEQN, the number of finite element 
!    coefficients.
!
...
!
!    Input, integer ILINE(MY), the list of global unknown numbers
!    along the profile line.
!
!    Input, integer INDX(NP,2), contains, for each node I, the
!    index of the U and V velocities at that node, or 0.
!
!    Input, integer IWRITE, controls the amount of output printed.
!
!    Input, integer MY, the number of nodes along the profile line.
!
!    Input, integer NELEMN, the number of elements.
!
!    Input, integer NNODES, the number of nodes per element, which
!    is 6 for the quadratic triangular elements in use here.
!
!    Input, integer NODE(NELEMN,NNODES), contains, for each
!    element, the global node indices of the element nodes.
!
...
!  Parameters:
!
!    Input, integer I, the global unknown number.
!
!    Input, integer ILINE(MY), the list of global unknown numbers
!    along the profile line.
!
!    Input, integer MY, the number of nodes on the profile line.
!
!    Output, integer IGETL, the index of the node in the profile 
...
!
!    Input, integer INDX(MAXNP,2), contains, for each node I, the
!    index of the U and V velocities at that node, or 0.
!
!    Input, integer NNODES, the number of nodes per element, which
!    is 6 for the quadratic triangular elements in use here.
...
!  Local Parameters:
!
!    Local, integer IPIVOT(NEQN), pivoting space.
!
  implicit none
...
!  Parameters:
!
!    Input, integer MX, the number of nodes in the X direction.
!
!    Input, integer MY, the number of nodes in the Y direction.
!
!    Input, integer NNODES, the number of nodes per element, which
!    is 6 for the quadratic triangular elements in use here.
!
...
!
!    Input, integer IQ, the index of a basis function 
...
!    the reference triangle.
!
!    Input, integer IN, the index of a basis function 
!    in the reference triangle.
!
...
!
!    Input, integer IT, the index of the element 
!    containing (XQ,YQ).
!
!    Input, integer NELEMN, the number of elements.
!
...
!    is 6 for the quadratic triangular elements in use here.
!
!    Input, integer NODE(NELEMN,NNODES), contains the indices of
...
!  Parameters:
!
!    Input, integer IUK, the index of the unknown.
...
!    index of the U and V velocities at that node, or 0.
!
!    Input, integer UV_UNIT, the FORTRAN unit number associated
...
!  Parameters:
!
!    Input, integer XY_UNIT, the FORTRAN unit number associated 
!    with the output file.
!
!    Input, integer NP, the number of nodes.
!
