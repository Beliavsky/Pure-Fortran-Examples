...
!  Local:
!
!    real ( kind = rk ) A(3*IB+1,NODE_NUM), the coefficient matrix.
!
!    integer DIM_NUM, the spatial dimension, which is 2.
!
!    character ( len = 255 ) ELEMENT_FILE_NAME, the name of the
...
!
!    integer ELEMENT_ORDER, the order of each element.
!
!    real ( kind = rk ) F(NODE_NUM), the right hand side.
!
!    integer IB, the half-bandwidth of the matrix.
!
!    logical NODE_BOUNDARY(NODE_NUM), is TRUE if a given
!    node is on the boundary.
!
!    integer NODE_CONDITION(NODE_NUM), indicates the type of
...
!
!    character ( len = 255 ) NODE_FILE_NAME, the name of the
!    input file containing the node coordinate information.
!
!    integer NODE_NUM, the number of nodes.
!
!    real ( kind = rk ) NODE_XY(2,NODE_NUM), the coordinates of nodes.
!
!    integer QUAD_NUM, the number of quadrature points used 
...
!
!    real ( kind = rk ) U(NODE_NUM), the finite element coefficients
!    defining the solution at the current time.
!
!    real ( kind = rk ) U_OLD(NODE_NUM), the finite element coefficients
...
!
!    Input, integer ELEMENT_ORDER, the number of nodes used
!    to form one element.
!
...
!
  ...
  real ( kind = rk ), dimension(2,quad_num) :: phys_xy
  integer quad
  real ( kind = rk ), dimension (quad_num) :: quad_w
  real ( kind = rk ), dimension (2,quad_num) :: quad_xy
  real ( kind = rk ) t3(2,3)
  real ( kind = rk ) t6(2,6)
  integer test
  real ( kind = rk ) time
  real ( kind = rk ) time_step_size
  real ( kind = rk ) triangle_area_2d
  real ( kind = rk ), dimension(node_num) :: u_old
  real ( kind = rk ), dimension(quad_num) :: w
...
!
  ...
  real ( kind = rk ), dimension(3*ib+1,node_num) :: a
  real ( kind = rk ) bc_value(node_num)
  integer column
  integer column_high
  integer column_low
  ...
  real ( kind = rk ), dimension(node_num) :: f
  integer node
  integer node_condition(node_num)
  real ( kind = rk ), dimension(2,node_num) :: node_xy
  real ( kind = rk ) time
...
!  Local parameters:
!
!    Local, real ( kind = rk ) BI, DBIDX, DBIDY, the value of some basis function
!    and its first derivatives at a quadrature point.
!
!    Local, real ( kind = rk ) BJ, DBJDX, DBJDY, the value of another basis
...
!
  ...
  integer i
  integer j
  real ( kind = rk ) k_value
  integer node_condition(node_num)
  real ( kind = rk ), dimension(2,node_num) :: node_xy
  real ( kind = rk ) phys_xy(2,quad_num)
  integer quad
  real ( kind = rk ), dimension(quad_num) :: quad_w
  real ( kind = rk ), dimension(2,quad_num) :: quad_xy
  real ( kind = rk ) :: rhs_value
  real ( kind = rk ), dimension (2,3) :: t3
  real ( kind = rk ), dimension (2,6) :: t6
  integer test
  real ( kind = rk ) time
  integer element
  real ( kind = rk ) triangle_area_2d
  real ( kind = rk ) w(quad_num)

  call i4_fake_use ( node_condition(1) )
!
...
!
!    Output, integer COLUMN_NUM, the number of columns
!    in the file.
!
  implicit none
...
!
!    Output, real ( kind = rk ) QUAD_XY(2,QUAD_NUM),
...
!
  ...
  real ( kind = rk ) h
  real ( kind = rk ), dimension(quad_num) :: quad_w
  real ( kind = rk ), dimension(dim_num,quad_num) :: quad_xy
  real ( kind = rk ) t
  real ( kind = rk ) u
  real ( kind = rk ) v
  real ( kind = rk ) w
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) U(NODE_NUM), the coefficients of
!    the solution.
!
...
!
  ...
  integer solution_file_status
  integer solution_file_unit
  real ( kind = rk ) time
...
!
!    Output, real ( kind = rk ) TRIANGLE_AREA_2D, the absolute area of
!    the triangle.
!
  implicit none
...
!
!    Input, integer TRIANGLE_NODE(6,TRIANGLE_NUM), the nodes
...
!
  ...
  integer triangle_node(6,triangle_num)
...
!    Input, integer TRIANGLE_NUM, the number of triangles.
!
!    Input, integer TRIANGLE_NODE(6,TRIANGLE_NUM), lists,
