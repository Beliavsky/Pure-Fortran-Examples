...
!  Parameters:
!
!    real ( kind = rk ) ADIAG(NU), the "diagonal" coefficients. 
!    That is, ADIAG(I) is the coefficient of the I-th unknown in 
!    the I-th equation.
!
!    real ( kind = rk ) ALEFT(NU), the "left hand" coefficients.  That is, 
...
!
  ...
  real ( kind = rk ) adiag(nsub+1)
  real ( kind = rk ) aleft(nsub+1)
  real ( kind = rk ) arite(nsub+1)
  real ( kind = rk ) f(nsub+1)
  real ( kind = rk ) h(nsub)
  integer ibc
  integer indx(0:nsub)
  integer node(nl,nsub)
...
!  Parameters:
!
!    Output, real ( kind = rk ) ADIAG(NU), the "diagonal" coefficients. 
!    That is, ADIAG(I) is the coefficient of the I-th unknown in 
!    the I-th equation.
!
!    Output, real ( kind = rk ) ALEFT(NU), the "left hand" coefficients.
!    ALEFT(I) is the coefficient of the (I-1)-th unknown in the I-th equation.
!    There is no value in ALEFT(1), since the first equation
!    does not refer to a "0-th" unknown.
!
...
!    refer to an "NU+1"-th unknown.
!
!    Output, real ( kind = rk ) F(NSUB+1) or F(NU).
!    ASSEMB stores into F the right hand side of the linear
!    equations.
!    SOLVE replaces those values of F by the solution of the
!    linear equations.
!
!    Input, real ( kind = rk ) H(NSUB)
!    H(I) is the length of subinterval I.  This code uses
!    equal spacing for all the subintervals.
!
!    Input, integer INDX(0:NSUB).
!    For a node I, INDX(I) is the index of the unknown
!    associated with node I.
!    If INDX(I) is equal to -1, then no unknown is associated
!    with the node, because a boundary condition fixing the
...
!    to have at X = XR.
!
!    Input, real ( kind = rk ) XN(0:NSUB).
!    XN(I) is the location of the I-th node.  XN(0) is XL,
!    and XN(NSUB) is XR.
!
!    Input, real ( kind = rk ) XQUAD(NSUB)
!    XQUAD(I) is the location of the single quadrature point
!    in interval I.
!
  implicit none
...
!
!    Output, real ( kind = rk ) FF, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) PP, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) QQ, the value of the function.
!
  implicit none
