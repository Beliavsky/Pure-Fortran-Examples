...
!
  ...
  real ( kind = rk8 ), parameter :: a = 10.0D+00
  real ( kind = rk8 ), parameter :: b = 20.0D+00
  integer bin
  real ( kind = rk8 ) c
  real ( kind = rk8 ) cmax
  real ( kind = rk8 ) cmin
  ...
  real ( kind = rk8 ), parameter :: rmax = 23.0D+00
  real ( kind = rk8 ), parameter :: rmin = 8.0D+00
  integer seed
...
!
  ...
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: a = (/  5.0D+00,  0.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: b = (/ 15.0D+00, 20.0D+00 /)
  integer bin(2)
  real ( kind = rk8 ) c(2)
  real ( kind = rk8 ) cmax(2)
  real ( kind = rk8 ) cmin(2)
  integer i
  integer, parameter :: nbin = 7
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: rmin = (/  &
    3.0D+00, -2.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: rmax = (/ &
    23.0D+00, 21.0D+00 /)
  integer seed
...
!
  ...
  integer, parameter, dimension ( 1 ) :: nbin = (/ 4 /)

  real ( kind = rk8 ) a(2,n)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: amin = (/ &
    8.0D+00, 3.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: amax = (/ &
    23.0D+00, 12.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: bin_min = (/ &
    10.0D+00, 5.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: bin_max = (/ &
    20.0D+00, 10.0D+00 /)
  integer bin_last(nbin(1),nbin(1))
  integer bin_next(n)
  integer bin_start(nbin(1),nbin(1))
  integer i1
  integer i2
  integer j
  integer k
...
!
  ...
  real ( kind = rk8 ) a(2,n)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: alo = (/  0.0D+00, 2.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 2 ) :: ahi = (/ 10.0D+00, 3.0D+00 /)
  integer l
  integer r
...
!
  ...
  real ( kind = rk8 ) a(3,n)
  real ( kind = rk8 ), parameter, dimension ( 3 ) :: alo = (/ &
     0.0D+00, 2.0D+00, 1.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( 3 ) :: ahi = (/ &
...
!
  ...
  real ( kind = rk8 ) a(n)
  real ( kind = rk8 ), parameter :: amax = 23.0D+00
  real ( kind = rk8 ), parameter :: amin = 8.0D+00
  real ( kind = rk8 ), parameter :: bin_max = 20.0D+00
  real ( kind = rk8 ), parameter :: bin_min = 10.0D+00
...
!
  ...
  integer, parameter, dimension ( 1 ) :: nbin = (/ 10 /)
  integer, parameter, dimension ( ndim ) :: nbin2 = (/ 20, 5 /)
  integer, parameter :: nset = 1000
  integer, parameter :: ntest = 10

  real ( kind = rk8 ), parameter, dimension ( ndim ) :: bin_min = (/  &
    0.0D+00,  0.0D+00 /)
  real ( kind = rk8 ), parameter, dimension ( ndim ) :: bin_max = (/  &
    20.0D+00, 5.0D+00 /)
  integer bin_last(nbin(1),nbin(1))
...
!
  ...
  do i = 1, ntest
    ...
    call points_nearest_point_bins_2d ( nset, pset, nbin, bin_min, bin_max, &
      bin_start, bin_next, p, i_min, d_min, compares )
    ...
    call points_nearest_point_bins_2d_3 ( nset, pset2, nbin2, bin_min, &
      bin_max, bin_start2, bin_next2, p, i_min, d_min, compares )
...
!
  ...
  real ( kind = rk8 ) d_min0(ntest)
  real ( kind = rk8 ) d_min1(ntest)
  real ( kind = rk8 ) d_min2(ntest)
  real ( kind = rk8 ) d_min3(ntest)
  real ( kind = rk8 ) eps
  integer i
...
!
  ...
  call points_nearest_points_bins_2d_3 ( nset, pset, nbin2, bin_min, bin_max, &
    bin_start2, bin_next2, ntest, ptest, i_min3, d_min3, compares )
...
!
  ...
  write ( *, '(a,g14.6)' ) '  Naive code time = ', &
    real ( clock_count2 - clock_count1, kind = rk8 ) &
    / real ( clock_rate, kind = rk8 )

  write ( *, '(a,g14.6)' ) '  Bin code time =	', &
    real ( clock_count3 - clock_count2, kind = rk8 ) &
    / real ( clock_rate, kind = rk8 )

  write ( *, '(a,g14.6)' ) '  Bin2 code time =   ', &
    real ( clock_count4 - clock_count3, kind = rk8 ) &
    / real ( clock_rate, kind = rk8 )

  write ( *, '(a,g14.6)' ) '  Bin3 code time =   ', &
    real ( clock_count6 - clock_count5, kind = rk8 ) &
    / real ( clock_rate, kind = rk8 )
...
!
  ...
  integer, allocatable :: bin_last(:,:,:)
  integer bin_next(nset)
  integer, allocatable :: bin_start(:,:,:)
  integer clock_count1
  integer clock_count2
  integer clock_count3
  integer clock_max
  ...
  integer i_min2(ntest)
  integer n_different
  real ( kind = rk8 ), allocatable :: pset(:,:)
  real ( kind = rk8 ) ptest(ndim,ntest)
  integer :: seed = 123456789

  allocate ( bin_last(1:nbin,1:nbin,1:nbin) )
  allocate ( bin_start(1:nbin,1:nbin,1:nbin) )
  allocate ( pset(1:ndim,1:nset) )
...
!
  ...
  real ( kind = rk8 ) dnear1
  real ( kind = rk8 ) dnear2
  integer itest
  ...
  integer nnear1
  integer nnear2
  integer nod_tri(3,3*num_pts)
  integer num_tri
  integer :: seed = 123456789
  integer td(ntest)
  integer tnbr(3,3*num_pts)
  real ( kind = rk8 ), dimension (2,num_pts) :: xc = reshape ( (/ &
       0.0D+00, 0.0D+00, &
       2.0D+00, 2.0D+00, &
      -1.0D+00, 3.0D+00, &
      -2.0D+00, 2.0D+00, &
...
!
  ...
  integer i_min
  integer i_test
  real ( kind = rk8 ), dimension ( ndim, n ) :: x = &
    reshape ( &
      (/  7.0D+00,  3.0D+00, &
          4.0D+00,  7.0D+00, &
          5.0D+00, 13.0D+00, &
  ...
  real ( kind = rk8 ), dimension ( ndim, test_num ) :: x_test = &
    reshape ( &
      (/  7.0D+00,  1.0D+00, &
          4.0D+00,  7.0D+00, &
