...
!
!    Input, real ( kind = rk ) RHO(N), the arguments.
!
!    Input, real ( kind = rk ) RHO0, the correlation length.
!
!    Output, real ( kind = rk ) C(N), the correlations.
!
  implicit none
...
!
!    Input, real ( kind = rk ) S(M), T(N), two samples.
!    0 <= S(*), T(*).
!
!    Input, real ( kind = rk ) RHO0, the correlation length.
!
!    Output, real ( kind = rk ) C(M,N), the correlations.
!
  implicit none
...
!    0.0 < RHO0.
!
!    Input, real ( kind = rk ) NU, the smoothness parameter.
...
!
  ...
  do i = 1, n
    ...
    if ( rho1 == 0.0D+00 ) then
      ...
      c(i) = rho1 ** nu * r8_besk ( nu, rho1 ) / gamma ( nu ) &
        / 2.0 ** ( nu - 1.0D+00 )
    end if
...
!    Input, real ( kind = rk ) RHO(N), the arguments.
!
!    Input, real ( kind = rk ) C(N), the correlations.
!
...
!  Parameters:
!
!    Input, integer N, the number of values of RHO.
!
!    Input, integer N2, the number of values of RHO0.
!
!    Input, real ( kind = rk ) RHO(N), the independent value.
!
!    Input, real ( kind = rk ) RHO0(N2), the correlation lengths.
!
!    Input, real ( kind = rk ) C(N,N2), the correlations.
!
!    Input, character ( len = * ) HEADER, an identifier for the files.
!
!    Input, character ( len = * ) TITLE, a title for the plot.
!
  ...
  integer i
  real ( kind = rk ) rho(n)
  real ( kind = rk ) rho0(n2)
  character ( len = * ) title

  call r8_fake_use ( rho0(1) )
...
!    0.0 < RHO0.
!
!    Input, real ( kind = rk ) E, the exponent.
...
!
!    Input, real ( kind = rk ) C(N,N), the correlation matrix.
!
!    Input, real ( kind = rk ) SIGMA(N), the standard deviations.
!
!    Output, real ( kind = rk ) K(N,N), the covariance matrix.
!
  implicit none
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) K(N,N), the covariance matrix.
!
!    Output, real ( kind = rk ) C(N,N), the correlation matrix.
!
!    Output, real ( kind = rk ) SIGMA(N), the standard deviations.
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the number of points in each path.
!
!    Input, integer N2, the number of paths.
!
!    Input, real ( kind = rk ) RHO(N), the independent value.
!
!    Input, real ( kind = rk ) X(N,N2), the path values.
!
!    Input, character ( len = * ) HEADER, an identifier for the files.
!
!    Input, character ( len = * ) TITLE, a title for the plot.
...
!
!    Output, real ( kind = rk ) AMPL, THETA, the modulus and phase.
!
  implicit none
  ...
  real ( kind = rk ) ampl
  real ( kind = rk ) bm0cs(37)
  real ( kind = rk ) bm02cs(40)
  real ( kind = rk ) bt02cs(39)
  real ( kind = rk ) bth0cs(44)
  real ( kind = rk ) eta
  integer nbm0
  integer nbm02
  integer nbt02
  integer nbth0
  real ( kind = rk ) pi4
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) theta
  real ( kind = rk ) x
  real ( kind = rk ) xmax
  real ( kind = rk ) z
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESI1, the Bessel function I of order 1 of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) bi1cs(17)
  integer nti1
  real ( kind = rk ) r8_besi1
  real ( kind = rk ) r8_besi1e
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESI1E, the exponentially scaled Bessel 
!    function I1(X).
!
  implicit none
  ...
  real ( kind = rk ) ai12cs(69)
  real ( kind = rk ) ai1cs(46)
  real ( kind = rk ) bi1cs(17)
  real ( kind = rk ) eta
  integer ntai1
  integer ntai12
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESJ0, the Bessel function J of order 0 of X.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) NU, the order.
!
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESK, the Bessel function K of order NU at X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), allocatable :: bke(:)
  integer nin
  real ( kind = rk ) nu
  real ( kind = rk ) r8_besk
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESK1, the Bessel function K of order 1 of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) bk1cs(16)
  integer ntk1
  real ( kind = rk ) r8_besi1
  real ( kind = rk ) r8_besk1
  real ( kind = rk ) r8_besk1e
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESK1E, the exponentially scaled Bessel 
!    function K1(X).
!
  implicit none
  ...
  real ( kind = rk ) ak12cs(33)
  real ( kind = rk ) ak1cs(38)
  real ( kind = rk ) bk1cs(16)
  real ( kind = rk ) eta
  integer ntak1
  integer ntak12
...
!
!    Output, real ( kind = rk ) BKE(abs(NIN)), the exponentially scaled 
!    K Bessel functions.
!
  implicit none
  ...
  vend = abs ( xnu + real ( nin, kind = rk ) ) - 1.0D+00
...
!    Input, integer NIN, indicates the number of terms to compute.
!
!    Output, real ( kind = rk ) BK(abs(NIN)), the K Bessel functions.
!
  implicit none
...
!
!    Input, real ( kind = rk ) CS(N), the Chebyshev coefficients.
!
!    Input, integer N, the number of Chebyshev coefficients.
!
!    Output, real ( kind = rk ) R8_CSEVL, the Chebyshev series evaluated at X.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) XMIN, XMAX, the bounds.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) DOS(NOS), the Chebyshev coefficients.
!
!    Input, integer NOS, the number of coefficients.
!
!    Input, real ( kind = rk ) ETA, the desired accuracy.
!
!    Output, integer R8_INITS, the number of terms of the 
...
!  Parameters:
!
!    Input, real ( kind = rk ) XNU, the order parameter.
!
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) BKNU, BKNU1, the two K Bessel functions.
!
...
!
  ...
  real ( kind = rk ) alnsml
  real ( kind = rk ) alnz
  real ( kind = rk ) alpha(32)
  real ( kind = rk ) an
  real ( kind = rk ) b0
  real ( kind = rk ) beta(32)
  real ( kind = rk ) bknu
  real ( kind = rk ) bknu0
  real ( kind = rk ) bknu1
  real ( kind = rk ) bknud
...
!
    ...
    a0 = 0.5D+00 * gamma ( 1.0D+00 + v )
    b0 = 0.5D+00 * gamma ( 1.0D+00 - v )
    c0 = - euler
    ...
    do inu = 1, 2
      ...
      if ( a(2) == 0.0D+00 ) then
        ...
        do i = 4, nterms
          ...
          n = i - 1
          x2n = real ( 2 * n - 1, kind = rk )

          a(i) = ( x2n + 2.0D+00 ) ** 2 - xmu
          qq = 16.0D+00 * x2n / a(i)
          p1 = - x2n * ( real ( 12 * n * n - 20 * n, kind = rk ) - a(1) ) &
            / ( ( x2n - 2.0D+00 ) * a(i) ) - qq * x
          p2 = ( real ( 12 * n * n - 28 * n + 8, kind = rk ) - a(1) ) / a(i) &
            - qq * x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_LGMC, the correction factor.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) algmcs(15)
  integer nalgm
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_lgmc
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
  real ( kind = rk ) xbig
  real ( kind = rk ) xmax
...
!*****************************************************************************80
!
!! R8_MACH returns real ( kind = rk ) real machine-dependent constants.
!
!  Discussion:
!
...
!
!    Output, real ( kind = rk ) R8_MACH, the value of the constant.
!
  implicit none
...
!
!    Output, real ( kind = rk ) ERROR_FROBENIUS, measures the 
...
subroutine sample_paths_cholesky ( n, n2, rhomax, rho0, correlation, x )
...
!    Input, integer N2, the number of paths.
!
!    Input, real ( kind = rk ) RHOMAX, the maximum value of RHO.
!
!    Input, real ( kind = rk ) RHO0, the correlation length.
!
...
!    the correlation.
!
!    Output, real ( kind = rk ) X(N,N2), the sample paths.
!
  implicit none
  ...
  real ( kind = rk ) cor(n,n)
  real ( kind = rk ) cor_vec(n)
...
subroutine sample_paths_eigen ( n, n2, rhomax, rho0, correlation, x )
...
!    Input, integer N2, the number of paths.
!
!    Input, real ( kind = rk ) RHOMIN, RHOMAX, the range of RHO.
!
!    Input, real ( kind = rk ) RHO0, the correlation length.
!
...
subroutine sample_paths2_eigen ( n, n2, rhomin, rhomax, rho0, correlation2, x )
