...
!
!    Input, real ( kind = rk ) P(3,N), natural coordinates of evaluation
!    points.
!
!    Output, real ( kind = rk ) PHI(8,N), the basis function values.
!
  implicit none
...
!
  ...
  call random_number ( harvest = p(1:3,1:n) )
...
!    points.
!
!    Output, real ( kind = rk ) PHI(20,N), the basis function values.
!
  implicit none
...
!    points.
!
!    Output, real ( kind = rk ) PHI(27,N), the basis function values.
!
  implicit none
...
!    Input, integer N, the number of evaluation points.
!
!    Input, real ( kind = rk ) P(3,N), the points where the basis functions
!    are to be evaluated.
!
...
!
  ...
  integer j
  real ( kind = rk ) p(3)
  real ( kind = rk ) phi1(node_num,1)
  real ( kind = rk ) phi1_sum
  real ( kind = rk ) phi4(node_num,4)
  real ( kind = rk ), dimension ( 3, node_num ) :: t
  integer test
  integer, parameter :: test_num = 5
  ...
  do test = 1, test_num
    ...
    call random_number ( harvest = c(1:4 ) )
...
!    are to be evaluated.
!
!    Output, real ( kind = rk ) PHI(10,N), the value of the basis functions
!    at the evaluation points.
!
  implicit none
  ...
  real ( kind = rk ) p(3,n)
  real ( kind = rk ) phi(10,n)
  real ( kind = rk ) phi_linear(4,n)
  real ( kind = rk ) t(3,4)
...
!
  ...
  integer j
  real ( kind = rk ) p(3)
  real ( kind = rk ) p10(3,10)
  real ( kind = rk ) phi1(10,1)
  real ( kind = rk ) phi1_sum
  real ( kind = rk ) phi10(10,10)
  ...
  call random_number ( harvest = t(1:3,1:4) )
...
!  Parameters:
!
!    Output, real ( kind = rk ) P(3,8), the coordinates.
!
  implicit none
  ...
  real ( kind = rk ) p(3,8)
  real ( kind = rk ), save :: p_save(3,8) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) P(3,20), the coordinates.
!
  implicit none
  ...
  real ( kind = rk ) p(3,20)
  real ( kind = rk ), save :: p_save(3,20) = reshape ( (/ &
    -1.0D+00, -1.0D+00, -1.0D+00, &
    +1.0D+00, -1.0D+00, -1.0D+00, &
    +1.0D+00, +1.0D+00, -1.0D+00, &
    -1.0D+00, +1.0D+00, -1.0D+00, &
...
!  Parameters:
!
!    Output, real ( kind = rk ) P(3,27), the coordinates.
!
  implicit none
  ...
  real ( kind = rk ) p(3,27)
  real ( kind = rk ), save :: p_save(3,27) = reshape ( (/ &
    -1.0D+00, -1.0D+00, -1.0D+00, &
    +1.0D+00, -1.0D+00, -1.0D+00, &
    +1.0D+00, +1.0D+00, -1.0D+00, &
    -1.0D+00, +1.0D+00, -1.0D+00, &
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(3,4), the coordinates of the vertices of the
...
!
!    Input/output, real ( kind = rk ) B(N,NB).
...
subroutine reference_tet4_sample ( n, ref )
...
!
!    Output, real ( kind = rk ) REF(3,N), points in the reference tetrahedron.
!
  implicit none
...
subroutine reference_tet4_uniform ( n, x )
...
subroutine reference_tet4_uniform2 ( n, x )
