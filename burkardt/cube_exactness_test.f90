...
!  Parameters:
!
!    Input, real ( kind = rk ) A(3), B(3), the lower and upper integration
!    limits.
!
!    Input, integer NX, NY, NZ, the orders in the X and Y directions.
!    These orders must be between 1 and 10.
!
!    Output, real ( kind = rk ) X(N), Y(N), Z(N), the abscissas.
!
...
!
  ...
  integer k
  integer l
  real ( kind = rk ) w(nx*ny*nz)
  real ( kind = rk ) wx(nx)
  real ( kind = rk ) wy(ny)
  real ( kind = rk ) wz(nz)
  real ( kind = rk ) x(nx*ny*nz)
  real ( kind = rk ) xx(nx)
  real ( kind = rk ) y(nx*ny*nz)
  real ( kind = rk ) yy(ny)
  real ( kind = rk ) z(nx*ny*nz)
  real ( kind = rk ) zz(nz)
!
