...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, BETA, scalars that define A.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, BETA, scalars that define the matrix.
!
!    Input, integer N, the order of the matrix.
!
!    Output, real ( kind = rk ) A(N,N), the matrix.
!
  ...
  bot = alpha * ( alpha + real ( n, kind = rk ) * beta )

  do j = 1, n
    do i = 1, n

      if ( i == j ) then
        a(i,j) = ( alpha + real ( n - 1, kind = rk ) * beta ) / bot
      else
        a(i,j) = - beta / bot
      end if
...
!
!    Output, real ( kind = rk ) COND, the estimated L1 condition.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) A(N,N).  On input, a matrix to be factored.
...
!    Output, real ( kind = rk ) COND, the estimated L1 condition.
!
!    Output, real ( kind = rk ) Z(N), a work vector whose contents are 
...
!
  ...
  real ( kind = rk ) a(n,n)
  real ( kind = rk ) a_norm
  real ( kind = rk ) ainv_norm
  real ( kind = rk ) ax(n)
  real ( kind = rk ) ax_norm
  real ( kind = rk ) cond
  integer i
  integer m
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, the scalar defining A.  
...
!
!    Output, real ( kind = rk ) A(3,3), the matrix.
!
  implicit none
...
!
  ...
  real ( kind = rk ) a(4,4)
  real ( kind = rk ), dimension ( 4, 4 ) :: a_save = reshape ( (/ &
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, the scalar that defines A.  A typical
