...
!*****************************************************************************80
!
!! fem2d_sample() evaluates a finite element function at sample points.
!
!  Discussion:
!
...
!
  ...
  integer, allocatable, dimension ( :, : ) :: fem_element_neighbor
  integer, allocatable, dimension ( :, : ) :: fem_element_node
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: fem_node_xy
  character ( len = 255 ) fem_prefix
  real ( kind = rk ), allocatable, dimension ( :, : ) :: fem_value
  ...
  real ( kind = rk ), allocatable :: sample_node_xy(:,:)
  character ( len = 255 ) sample_prefix
  real ( kind = rk ), allocatable, dimension ( :, : ) :: sample_value
...
!   call getarg ( iarg, fem_prefix )
    call get_command_argument ( iarg, fem_prefix, length, status )
...
!    are legal.
!
!    Output, integer DIGIT, the corresponding value.  If CH was
...
!
!    Input, real ( kind = rk ) FEM_NODE_XY(2,FEM_NODE_NUM), the coordinates 
!    of the nodes.
!
...
!
!    Input, real ( kind = rk ) FEM_VALUE(FEM_VALUE_DIM,FEM_NODE_NUM), the 
!    finite element coefficient values at each node.
!
!    Input, integer SAMPLE_NODE_NUM, the number of sample nodes.
!
!    Input, real ( kind = rk ) SAMPLE_NODE_XY(2,SAMPLE_NODE_NUM), 
!    the sample nodes.
!
!    Output, real ( kind = rk ) SAMPLE_VALUE(FEM_VALUE_DIM,SAMPLE_NODE_NUM),
!    the sampled values.
!
  implicit none
  ...
  real ( kind = rk ) b(fem_element_order)
  real ( kind = rk ) dbdx(fem_element_order)
  real ( kind = rk ) dbdy(fem_element_order)
  integer edge
  integer fem_element_neighbor(3,fem_element_num)
  integer fem_element_node(fem_element_order,fem_element_num)
  real ( kind = rk ) fem_node_xy(2,fem_node_num)
  real ( kind = rk ) fem_value(fem_value_dim,fem_node_num)
  integer i
  integer j
  real ( kind = rk ), dimension ( 2 ) :: p_xy
  real ( kind = rk ) sample_node_xy(2,sample_node_num)
  real ( kind = rk ) sample_value(fem_value_dim,sample_node_num)
  integer t
  integer t_node(fem_element_order)
  real ( kind = rk ) t_xy(2,fem_element_order)
!
...
!
!    Input, integer TRIANGLE_NODE(3,TRIANGLE_NUM), the nodes 
...
!    Input, integer TRIANGLE_NUM, the number of triangles.
!
!    Input, integer TRIANGLE_ORDER(6,TRIANGLE_NUM), the nodes 
!    that make up each triangle.
!
!    Output, integer TRIANGLE_NEIGHBOR(3,TRIANGLE_NUM), the three
!    triangles that are direct neighbors of a given triangle.  
