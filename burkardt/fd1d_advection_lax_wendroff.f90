...
!
  ...
  data_filename = 'fd1d_advection_lax_wendroff_data.txt'
...
!
  ...
  command_filename = 'fd1d_advection_lax_wendroff_commands.txt'
  open ( unit = command_unit, file = command_filename, status = 'replace' )

  write ( command_unit, '(a)' ) 'set term png'
  write ( command_unit, '(a)' ) 'set output "fd1d_advection_lax_wendroff.png"'
...
!
!    Input, real ( kind = rk ) X(NX), the coordinates of the nodes.
!
!    Output, real ( kind = rk ) U(NX), the value of the initial condition.
!
  implicit none
