...
!
  ...
  integer c_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: center
  integer c_hi
  integer c_lo
  real ( kind = rk ), allocatable, dimension ( : ) :: e
  integer ierror
  integer it_hi
  integer it_lo
  ...
  integer point_dist
  integer point_num
  real ( kind = rk ), allocatable, dimension ( : ) :: r_max
  real ( kind = rk ), allocatable, dimension ( : ) :: r_min
  integer seed
  real ( kind = rk ) t
  ...
  do c_num = c_lo, c_hi
    ...
    call cluster_iteration ( dim_num, point_num, c_num, point, r_min, &
      r_max, seed, center, e(c_num), it_hi  )
  ...
  do c_num = c_lo, c_hi
    t = e(c_num) / real ( point_num, kind = rk )
...
!
!    Input, integer C_NUM, the number of clusters.
!
...
!    number generator.
!
!    Output, real ( kind = rk ) CENTER(DIM_NUM,C_NUM), the centers associated
!    with the minimal energy clustering.
!
!    Output, real ( kind = rk ) E, the energy associated with the minimal 
!    energy clustering.
!
!    Input, integer IT_HI, the number of energy iterations.
!
  implicit none
  ...
  real ( kind = rk ), dimension ( dim_num, c_num ) :: center
  real ( kind = rk ), dimension ( dim_num, c_num ) :: centroid
...
!
!    Input, real ( kind = rk ) CENTER(DIM_NUM,C_NUM), the center points.
!
!    Input, integer CLUSTER(POINT_NUM), indicates the cluster 
!    to which each data point belongs.
!
!    Output, real ( kind = rk ) E, the energy of the clustering.
!
  implicit none
...
!    Input, integer POINT_NUM, the number of data points.
!
!    Input, integer C_LO, C_HI, the minimum and maximum number of
!    clusters to consider.
!
!    Input, integer IT_LO, IT_HI, the least and most number of
!    energy iterations.
!
  implicit none
...
!  Parameters:
!
!    Output, integer POINT_DIST, the point distribution to use.
!    1, use a uniform random distribution.
!    2, use a uniform grid of points.  (This hasn't been set up properly
!       except for 1D!).
!
!    Input, integer DIM_NUM, the number of spatial dimensions.
!
!    Output, real ( kind = rk ) R_MIN(DIM_NUM), R_MAX(DIM_NUM), the coordinates
!    of the minimum and maximum corners of the region.
!
!    Output, integer POINT_NUM, the number of data points.
!
!    Output, integer C_LO, C_HI, the minimum and maximum number of
!    clusters to consider.
!
!    Output, integer IT_LO, IT_HI, the least and most number of
!    energy iterations.
!
  implicit none
...
!    Input, integer DIM_NUM, the number of spatial dimensions.
!
!    Input, integer C_NUM, the number of center points.
!
!    Input, real ( kind = rk ) CENTER(DIM_NUM,C_NUM), the coordinates of the
!    center points.
!
...
!
!    Output, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the coordinates of
!    the points.
!
  implicit none
  ...
  if ( point_dist == 1 ) then
    ...
    do i = 1, point_num
      ...
      if ( 1 < point_num ) then
        r = real ( i - 1, kind = rk ) / real ( point_num - 1, kind = rk )
      else
        r = 0.5D+00
      end if
