...
!
!    Input, real ( kind = rk ) C(2), the coordinates of the center.
!
!    Input, real ( kind = rk ) A(2), the angle of the first and last
!    points, in DEGREES.
!
!    Input, integer N, the number of points.
!
!    Output, real ( kind = rk ) XY(2,N), the grid points.
!
  implicit none
  ...
  do j = 1, n
    ...
    aj = ( real ( n - j,     kind = rk ) * a(1)   &
         + real (     j - 1, kind = rk ) * a(2) ) &
         / real ( n     - 1, kind = rk )
