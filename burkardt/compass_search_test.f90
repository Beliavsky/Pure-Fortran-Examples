...
!
  ...
  real ( kind = rk ) delta
  real ( kind = rk ) delta_tol
  real ( kind = rk ), external :: extended_rosenbrock
  real ( kind = rk ) fx
  integer k
  integer k_max
  real ( kind = rk ) x(m)
...
!
  ...
  real ( kind = rk ) delta_tol
  real ( kind = rk ) fx
  real ( kind = rk ), external :: goldstein_price
  integer k
  integer k_max
  real ( kind = rk ) x(m)
  real ( kind = rk ) x0(m)
...
!
  ...
  real ( kind = rk ) delta_tol
  real ( kind = rk ) fx
  real ( kind = rk ), external :: himmelblau
  integer k
  integer k_max
  real ( kind = rk ) x(m)
  real ( kind = rk ) x0(m)
...
!
  ...
  integer k
  integer k_max
  real ( kind = rk ), external :: mckinnon
  real ( kind = rk ) phi
  real ( kind = rk ) tau
  real ( kind = rk ) theta
  real ( kind = rk ) x(m)
...
!
  ...
  call compass_search ( rosenbrock, m, x0, delta_tol, delta, k_max, x, fx, k )
  call r8vec_print ( m, x, '  Estimated minimizer X1:' )
  write ( *, '(a)' ) ' '
  write ( *, '(a,g14.6,a,i12)' ) '  F(X1) = ', fx, ' number of steps = ', k
!
...
!
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) BEALE, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) BOHACH1, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) BOHACH2, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) BROYDEN, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) EXTENDED_ROSENBROCK, the value of the function at X.
!
  implicit none
  ...
  real ( kind = rk ) extended_rosenbrock
  real ( kind = rk ) f
  real ( kind = rk ) f1
  real ( kind = rk ) f2
  integer i
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) GOLDSTEIN_PRICE, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) HIMMELBLAU, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) LOCAL, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) MCKINNON, the value of the function at X.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) PHI, TAU, THETA, the parameter values.
!
  implicit none
  ...
  real ( kind = rk ) phi
  real ( kind = rk ), save :: phi_save = 60.0D+00
  real ( kind = rk ) tau
  real ( kind = rk ), save :: tau_save = 2.0D+00
  real ( kind = rk ) theta
  real ( kind = rk ), save :: theta_save = 6.0D+00
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) POWELL, the value of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M), the argument of the function.
!
!    Output, real ( kind = rk ) ROSENBROCK, the value of the function at X.
!
  implicit none
