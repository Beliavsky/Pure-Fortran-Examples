...
!  Parameters:
!
!    User input, real ( kind = rk ) A, the left endpoints of the interval.
!
!    User input, real ( kind = rk ) ALPHA, the value of the parameter ALPHA.
!
!    User input, real ( kind = rk ) B, the right endpoints of the interval.
!
!    User input, real ( kind = rk ) BETA, the value of the parameter BETA.
!
!    Local parameter, integer BIGJ, the number of intervals in 
!    either spatial coordinate.
!
!    User input, real ( kind = rk ) DELT, the time step to use.
!
!    User input, real ( kind = rk ) DELTA, the value of the parameter DELTA.
!
!    Local parameter, integer DIMJ, the number of nodes in either
!
!    User input, real ( kind = rk ) GAMMA, the value of the parameter GAMMA.
!
!    User input, real ( kind = rk ) H, the "space step", the desired spacing
!    between nodes in [A,B].
!
!    Local parameter, integer N, the total number of spatial nodes.
!
!    Local parameter, integer SOLVE, 0 for GMRES routine, 
!    1 for Jacobi.
!
!    User input, real ( kind = rk ) T, the final time.  The problem is to be
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: b_u
  real ( kind = rk ), allocatable, dimension ( : ) :: b_v
  real ( kind = rk ) beta
  integer bigj
  real ( kind = rk ) delt
  real ( kind = rk ) delta
  ...
  real ( kind = rk ) gamma
  real ( kind = rk ) h
  real ( kind = rk ), allocatable, dimension ( : ) :: hhat
  integer i
  integer i2
  integer idummy(1)
  integer ierr
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: rgwk
  integer solve
  real ( kind = rk ) t
  integer time_step
  integer time_steps
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: u_grid
  real ( kind = rk ) u_max
  real ( kind = rk ), allocatable, dimension ( :, : ) :: u0
  real ( kind = rk ), allocatable, dimension ( : ) :: v
  real ( kind = rk ), allocatable, dimension ( :, : ) :: v_grid
  real ( kind = rk ) v_max
  real ( kind = rk ), allocatable, dimension ( :, : ) :: v0
  real ( kind = rk ), allocatable, dimension ( : ) :: x
  real ( kind = rk ), allocatable, dimension ( :, : ) :: x_grid
  real ( kind = rk ), allocatable, dimension ( : ) :: y
  real ( kind = rk ), allocatable, dimension ( :, : ) :: y_grid
  real ( kind = rk ), allocatable, dimension ( : ) :: y1
  real ( kind = rk ), allocatable, dimension ( : ) :: y2
...
!
  ...
  call ppma_write ( 'u2d.ppm', dimj, dimj, r_ppm, g_ppm, b_ppm, ierror )
  ...
  call ppma_write ( 'v2d.ppm', dimj, dimj, r_ppm, g_ppm, b_ppm, ierror )
!
!  Free memory.
!
  deallocate ( b_i )
...
!
!    Input/output, real ( kind = rk ) A(NZ_NUM), the nonzero elements
!    of the matrix.
!
!    Input/output, real ( kind = rk ) B(NZ_NUM), the nonzero elements
...
!
!    Input/output, integer A1(N), integer A2(N), the data 
!    to be sorted.
!
!    Input/output, real ( kind = rk ) A3(N), A4(N), the data to be rearranged
...
!    are stored.
!
!    Input, real ( kind = rk ) RWORK(*), integer IWORK(*), arrays that can be
...
!
  ...
  call i4_fake_use ( ia(1) )
  call i4_fake_use ( isym )
  call i4_fake_use ( iwork(1) )
  call i4_fake_use ( ja(1) )
  call r8_fake_use ( rwork(1) )
...
!    and columns of data.
!
!    Input, integer R(ROW_NUM,COL_NUM), G(ROW_NUM,COL_NUM),
...
!    columns of data.
!
!    Input, integer RGB_MAX, the maximum value of any
!    data component.
!
!    Output, integer IERROR, an error flag.
!    0, no error.
...
!
!    Input, real ( kind = rk ) V, the value.
!
...
!
!    Input/output, integer B_I(*), B_J(*), 
!    real ( kind = rk ) B_V(*).  These vectors store the sparse matrix data.  
