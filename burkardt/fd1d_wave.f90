...
!
!    Input, real ( kind = rk ) X1, X2, the first and last X coordinates.
!
!    Input, integer T_NUM, the number of time steps, including the 
!    initial condition.
!
!    Input, real ( kind = rk ) T1, T2, the first and last T coordinates.
!
!    Input, real ( kind = rk ) C, a parameter which gives the speed of waves.
!
!    Output, real ( kind = rk ) ALPHA, the stability coefficient.
!
  implicit none
  ...
  real ( kind = rk ) t1
  real ( kind = rk ) t2
  real ( kind = rk ) x_delta
  integer x_num
  real ( kind = rk ) x1
  real ( kind = rk ) x2

  t_delta = ( t2 - t1 ) / real ( t_num - 1, kind = rk )
  x_delta = ( x2 - x1 ) / real ( x_num - 1, kind = rk )
...
!
!    Input, real ( kind = rk ) T, the time after the first step has been taken.
!    In other words, T = T1 + T_DELTA.
!
!    Input, real ( kind = rk ) T_DELTA, the time step.
!
!    Input, real ( kind = rk ) ALPHA, the stability coefficient, computed 
!    by FD1D_WAVE_ALPHA.
!
!    Input, real ( kind = rk ) U_X1(T), U_X2(T), functions for the left and 
!    right boundary conditions.
!
!    Input, real ( kind = rk ) UT_T1(X), the function that evaluates dUdT at the 
!    initial time.
!
...
!    Input, integer X_NUM, the number of nodes in the X direction.
!
!    Input, real ( kind = rk ) T, the new time, that is, the current 
!    time + T_DELTA.
!
!    Input, real ( kind = rk ) ALPHA, the stability coefficient, computed 
!    by FD1D_WAVE_ALPHA.
...
!    right boundary conditions.
!
!    Input, real ( kind = rk ) U1(X_NUM), the solution at the old time.
!
!    Input, real ( kind = rk ) U2(X_NUM), the solution at the current time.
!
!    Output, real ( kind = rk ) U3(X_NUM), the solution at the new time.
!
  implicit none
...
!
!    Input, real ( kind = rk ) XV(NV), the evaluation arguments.
!
!    Output, real ( kind = rk ) YV(NV), the values.
!
  implicit none
