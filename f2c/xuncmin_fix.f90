...
iretcd = -1
icscmx = 0
! Avoid compiler-dependent use of an undefined counter in OPTSTP.
mxtake = .false.
! Avoid compiler-dependent logical state before the first step update.
amu = zero
dltp = zero
phi = zero
phip0 = zero
! These are passed through the trust-region routines on later iterations.
epsm = EPSILON( 1.D0 )
...
!
! Changes from xuncmin.f90:
! 1. Keep the full optimization package and Rosenbrock callbacks unchanged.
! 2. Replace the original multi-method OPTDRV demo with a deterministic
!    one-step Newton test. The full trust-region driver still relies on
!    runtime behavior that is not yet reproduced faithfully by xf2c.
! 3. Initialize every scalar used by the test explicitly, so the Fortran
!    and generated C runs follow the same path.

USE unconstrained_min
IMPLICIT NONE
REAL (dp) :: x(2), xnew(2), g(2), h(2,2), p(2), rhs(2), sx(2), udiag(2)
REAL (dp) :: f0, f1, epsm
INTEGER   :: nr, n
...
rhs = -g
epsm = EPSILON(one)
CALL chlhsn(n, h, epsm, sx, udiag)
CALL lltslv(n, h, p, rhs)

xnew = x + p
CALL fcn(n, xnew, f1)

WRITE(*, '(a, g13.5)') 'f(x0): ', f0
WRITE(*, '(a, 2g13.5)') 'g(x0): ', g
WRITE(*, '(a, 4g13.5)') 'H(x0): ', h(1,1), h(1,2), h(2,1), h(2,2)
WRITE(*, '(a, 2g13.5)') 'Newton step: ', p
WRITE(*, '(a, 2g13.5)') 'x1 = x0 + p: ', xnew
WRITE(*, '(a, g13.5)') 'f(x1): ', f1
